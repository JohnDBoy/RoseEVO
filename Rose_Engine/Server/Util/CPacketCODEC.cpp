...
CpC::CpC ()
{
	...
	m_dwMaxPacketSize = MAX_PACKET_SIZE;
	m_pCR = new CRandom;
...
WORD CpC::ESCP (t_PACKETHEADER *pHEADER, int &iSendSEQ)
{
	...
	short nReaminLen = m_dwMaxPacketSize - pHEADER->m_nSize;
	if ( nReaminLen >= 7 )
...
WORD CpC::DRSH(t_PACKETHEADER *pHEADER)
{
	...
	// 패킷 크기 검증..
	if ( wPacketSize > m_dwMaxPacketSize || wPacketSize < sizeof(t_PACKETHEADER) ) {
		return 0;
	}
...
WORD CpC::DRCH(t_PACKETHEADER *pHEADER, int &iRecvSEQ)
{
	...
	// 패킷 크기 검증..
	if ( wPacketSize > m_dwMaxPacketSize ||
