...
#ifdef	__ZONE_EXP_SETUP
//	존경험치 얻기
int		CZoneTHREAD::Get_Exp_LV()
{
	//	경치 레벨 오류 처리.
	if(m_Exp_Lv < 10) m_Exp_Lv = 100;
	return m_Exp_Lv;
}
...
void CZoneTHREAD::Execute()
{
    ...
    while( !this->Terminated ) {
		...
		// (20061129:남병철) : 시간제 아이템 사용 만료 채크
		//--------------------------------------------------
		// 라이처스 훈련장(6) 여기가 가장 한가한 존이어서... 사용자 채크를 함;
		// 전체 구조의 효율성이 확인되지 않아 임시로 6번 존 쓰레드를 사용
		if( this->Get_ZoneNO() == 6 )
		{
			g_pUserLIST->CheckAllUser();
		}
		//--------------------------------------------------
#endif
...
//	LogString( LOG_NORMAL, "Left_OBJECT( Type:%d, IDX:%d:%s ) \n", pObj->Get_TYPE(), pObj->Get_INDEX(), pObj->Get_NAME() );
    ...
    if ( pObj->IsA( OBJ_MOB ) ) {
		if ( bSubFromREGEN && ((CObjMOB*)pObj)->m_pRegenPOINT )
			((CObjMOB*)pObj)->m_pRegenPOINT->SubLiveCNT (m_Timer.GetCurrentTIME());
    }
...
void CZoneTHREAD::RegenCharacter( CRegenPOINT *pRegen, int iIndex, int iCount )
{
	...
	if(pRegen->GetLiveCNT()>=pRegen->m_iLimitCNT)
		return;
	...
	for (int iC=0; iC<iCount; iC++)
	{
...
#ifdef __EVENT_MOB_REGEN
		// 특정 확률
		int iRAND = 1 + RANDOM( 100 );
		if( iRAND <= __EVENT_PERCENT )
		{
			// 전체맵 이벤트 몹 적용
			if( __EVENT_ZONE_NO == 0 )
			{
				if ( !pNewOBJ->Init( this, __EVENT_MOB_NO, fXPos, fYPos, TEAMNO_MOB, pRegen ) ) 
				{
					delete pNewOBJ;
					return;
				}
			...
			{
				// 특정 존에만 이벤트 몹 적용
				if( pRegen->m_pZONE->Get_ZoneNO() == __EVENT_ZONE_NO )
				{
					// __EVENT_ZONE_NO 존에 이벤트 몹 적용
					if ( !pNewOBJ->Init( this, __EVENT_MOB_NO, fXPos, fYPos, TEAMNO_MOB, pRegen ) ) 
					{
						delete pNewOBJ;
						return;
		...
		if( pRegen->GetLiveCNT() >= pRegen->GetLimitLiveCount(pRegen->m_iVar) )
			break;
	}
}
#else
...
//	팀에 소속된 모든 몬스터를 죽인다.
//	김영환 : 추가
void CZoneTHREAD::Kill_AllMOB_Team(classUSER *pUSER,DWORD	p_Team_N)
{
    classDLLNODE< CGameOBJ* > *pObjNODE;

	uniDAMAGE sDamage;
	sDamage.m_wVALUE = MAX_DAMAGE;
	...
	for (pObjNODE=m_ObjLIST.GetHeadNode(); pObjNODE;  ) 
	{
		if ( pObjNODE->DATA->IsA( OBJ_MOB ) ) 
		{
			Temp_MOB = (CObjMOB*)pObjNODE->DATA;
			if ( Temp_MOB->Get_HP() > 0 )
			{
				//	해당 팀 몹 모두 죽임
				if(p_Team_N == 0)
				{
					//	체력 제거.
					Temp_MOB->Sub_HP(sDamage.m_wVALUE);
					//	AI 죽임
					Temp_MOB->Do_DeadAI((CObjCHAR *)Temp_MOB,sDamage.m_wVALUE);
					//	NPC 이벤트 체크 전송 : 무시
					//	아이템 드럽하지 않음.
					//	몬스터 
					Temp_MOB->Dead(NULL);
					sDamage.m_wACTION = 0x00000018;	// 액션 번호 넣기. [테스트]
					//	몬스터 정보 필드에 전송
					Temp_MOB->Send_gsv_DAMAGE2Sector(0,sDamage.m_wDamage,NULL);
				}
				else
				{
					if(Temp_MOB->Get_TeamNO() == p_Team_N )
					{
						//	체력 제거.
						Temp_MOB->Sub_HP(sDamage.m_wVALUE);
						//	AI 죽임
...
//	해당 존의 모든 몬스터의 HP 회복
BOOL	CZoneTHREAD::Set_MAX_HP_All_MOB(DWORD p_Team_N)
{
	//	오브젝의 노드 포인터 설정.
    classDLLNODE< CGameOBJ* > *pObjNODE;

	//	임시 몬스터 포인터
	CObjMOB	*Temp_MOB = NULL;
	...
	for (pObjNODE=m_ObjLIST.GetHeadNode(); pObjNODE;  ) 
	{
		...
		if ( pObjNODE->DATA->IsA( OBJ_MOB ) ) 
		{
			...
			if ( Temp_MOB->Get_HP() > 0 )
			{
				...
				{
					//	HP 복구한다.
					Temp_MOB->Add_HP(Temp_MOB->Get_MaxHP());
					//	복구된 HP를 필드에 전달한다.
					//	우선 HP만 채운다.
				}
			}
...
//	존의 몬스터 개수 얻기
DWORD	CZoneTHREAD::Get_Zone_MOB_Number()
{
	DWORD Return_DW = 0;
	//	노드 연결
    classDLLNODE< CGameOBJ* > *pObjNODE;
	// m_ObjLIST 에서 노드의 해더 얻기
	for (pObjNODE=m_ObjLIST.GetHeadNode(); pObjNODE;  )
	{
		//	해당 노드의 게임 오브젝인 몬스터 인 경우
		if ( pObjNODE->DATA->IsA( OBJ_MOB ) ) 
		{
			Return_DW++;
		}
...
DWORD	CZoneTHREAD::Get_Zone_Team_User_No(DWORD p_Team_N)
{
	DWORD Return_DW = 0;
	//	노드 연결
    classDLLNODE< CGameOBJ* > *pObjNODE;
	...
	for (pObjNODE=m_ObjLIST.GetHeadNode(); pObjNODE;  )
	{
		//	해당 노드의 게임 오브젝인 유저인 경우
		if ( pObjNODE->DATA->IsA( OBJ_USER ) ) 
		{
			//	팀 번호 확인.
			if(((CObjCHAR *)pObjNODE)->Get_TeamNO() == p_Team_N )
			{
				Return_DW++;
			}
		}
...
BOOL	CZoneTHREAD::All_User_Warp(DWORD pType)
{
	switch(pType)
	{
		//	클랜 필드의 경우에 워프 처리 루틴.
	case 0:
		{
			...
			for (pObjNODE=m_ObjLIST.GetHeadNode(); pObjNODE;  )
			{
				...
				{
					//	유저할 유저 포인트 저장.
					p_User = (classUSER *)pObjNODE->DATA;
				}
				// 다음노드 찾기.
				pObjNODE = m_ObjLIST.GetNextNode( pObjNODE );
				//	유저 워프
				if(p_User != NULL)
				{
					GF_CF_End_User_Warp(p_User,true);
				}
			}
			break;
		}
