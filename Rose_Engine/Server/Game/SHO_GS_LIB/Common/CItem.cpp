...
bool tagBaseITEM::check_timeExpired()
{
	...
	time_t start_t, end_t, new_end_t, current_t, down_start_t, down_end_t;
	start_t = get_basicTime() + (time_t)dwPickOutTime;
	end_t = get_basicTime() + (time_t)dwPickOutTime + ((time_t)wPeriod * 3600);	// 기간은 시간단위이므로 초단위로 환산		

	// 서버다운으로 인한 만료시간 보정
	new_end_t = end_t;

	if(g_mapServerDownTime.empty() != true)
	{
		for(int i=1; i<1000; i++)
		{
			if(g_mapServerDownTime[i].down_start_t == 0)
				break;

			new_end_t = calc_endTime(start_t, new_end_t, g_mapServerDownTime[i].down_start_t, g_mapServerDownTime[i].down_end_t, g_mapServerDownTime[i].nValue);

			if(new_end_t > time(&current_t))	return false;		// 아직 많이 남아있다.
		}
	}

	// 만료시간에서 분단위시간은 무조건 올림으로 처리, 정시에 만료되도록 한다.
	new_end_t = new_end_t + (3600 - 1);			// 1시간 - 1초를 더한후
	struct tm *t = localtime(&new_end_t);	
	t->tm_min = 0; t->tm_sec = 0;	// 분,초를 0으로...
	new_end_t = mktime( t);	

	if(new_end_t < time(&current_t))	return true;		// 기간만료
	else							return false;
}
