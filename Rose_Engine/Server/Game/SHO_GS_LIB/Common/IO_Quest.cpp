...
static void Set_QuestVAR (tQST_PARAM *pPARAM, STR_QUEST_DATA *pDATA, int iValue)
{
	switch( pDATA->m_wVarTYPE ) {
		...
		case QST_VARTYPE_CLANWAR:
			...
			if(pDATA->m_wVarNO < 4)
			{
				if(((classUSER *)pPARAM->m_pOWNER)->Quest_Get_Number() == QUEST_PER_PLAYER )
				{
					//	클랜전 신청 오류 처리. 퀘스트를 받을 수 없다.
					pPARAM->m_pOWNER->Quest_SetClanWarVAR(pDATA->m_wVarNO,0);
					//	오류 메시지 전송
					((classUSER *)pPARAM->m_pOWNER)->Send_gsv_CLANWAR_Err(CWAR_QUEST_LIST_FULL);
					////	수정된 유저 퀘스트 변수 내용 전달
					((classUSER *)pPARAM->m_pOWNER)->Send_gsv_Quest_VAR();
				}
#ifdef __CLAN_WAR_EXT
				else if( CClanWar::GetInstance()->ApplyClanWar( (CObjNPC*)pPARAM->m_pNpcVAR, (classUSER *)pPARAM->m_pOWNER, (DWORD)iValue ) == TRUE )
#else
				else if(GF_Clan_War_Input((LPVOID)pPARAM->m_pNpcVAR,((classUSER *)pPARAM->m_pOWNER)->GetClanID(),((classUSER *)pPARAM->m_pOWNER)->Get_LEVEL(),iValue, ((classUSER *)pPARAM->m_pOWNER)))
#endif
				{
					//	클랜전 퀘스트 변수 처리
					pPARAM->m_pOWNER->Quest_SetClanWarVAR(pDATA->m_wVarNO,iValue);
					//	클랜전 신청 체널 넣기
					pPARAM->m_pOWNER->Quest_SetClanWarVAR(6,G_SV_Channel);
					////	수정된 유저 퀘스트 변수 내용 전달
					((classUSER *)pPARAM->m_pOWNER)->Send_gsv_Quest_VAR();
				}
				else
				{
					...
					////	수정된 유저 퀘스트 변수 내용 전달
					((classUSER *)pPARAM->m_pOWNER)->Send_gsv_Quest_VAR();
					//	LV 제한 및 기타 오류 메시지 전송
					//((classUSER *)pPARAM->m_pOWNER)->Send_gsv_CLANWAR_Err(3);
				}
			}
			else if(pDATA->m_wVarNO == 4)
			{
#ifdef __CLAN_WAR_EXT
				CClanWar::GetInstance()->WinningRewardJulyItem( (classUSER*)pPARAM->m_pOWNER );
#else
				//	해당 유저 보상 처리
				GF_Clan_Master_ClanWar_Gift((LPVOID)pPARAM->m_pOWNER);
#endif
				////	수정된 유저 퀘스트 변수 내용 전달
				((classUSER *)pPARAM->m_pOWNER)->Send_gsv_Quest_VAR();
			}
...
/// 퀘스트 등록/삭제 요청
	switch( pREWD->m_Rewd000.btOp ) 
	{
		...
		case 1 :	// 등록.
			{
#ifdef __QUEST_BLOCK_LIST
				// 정당한 범위의 퀘스트 ID인가?
				if( 0 < pREWD->m_Rewd000.iQuestSN && pREWD->m_Rewd000.iQuestSN < 5501 )
				{

					// 블록 리스트에 포함된 퀘스트 ID 인가? (등록된 퀘스트면 -> 퀘스트 요청 거부)
					ZString zQuestID = pREWD->m_Rewd000.iQuestSN;
					ZString zCompare;
					char szConfirm[100];

					memset( szConfirm, 0, 100 );
					::GetPrivateProfileString( "QUEST_BLOCK_LIST", zQuestID.c_str(), "", szConfirm, 100, "./QUEST_BLOCK_LIST.ini" );
					zCompare = szConfirm;
					if( !zCompare.CompareIC( ZString("BLOCK") ) )
					{
						// 블록된 퀘스트 요청
						g_LOG.CS_ODS( LOG_NORMAL, "[ERROR] BLOCK QUEST ID : %d", pREWD->m_Rewd000.iQuestSN );
						g_LOG.CS_ODS( LOG_NORMAL, " [INFO] [%s] %s (%s)", pPARAM->m_pOWNER->Get_ACCOUNT(),
																		  pPARAM->m_pOWNER->Get_NAME(),
                                                                          pPARAM->m_pOWNER->Get_IP() );

						if( pPARAM && pPARAM->m_pOWNER )
							return IS_HACKING( pPARAM->m_pOWNER, "NOT USE QUEST" );
				...
				{
					// 정당한 범위의 퀘스트 아님
					g_LOG.CS_ODS( LOG_NORMAL, "[ERROR] OUT OF QUEST ID : %d", pREWD->m_Rewd000.iQuestSN );
					g_LOG.CS_ODS( LOG_NORMAL, " [INFO] [%s] %s (%s)", pPARAM->m_pOWNER->Get_ACCOUNT(),
																	  pPARAM->m_pOWNER->Get_NAME(),
																	  pPARAM->m_pOWNER->Get_IP() );				
...
#ifdef	__CLAN_WAR_SET
				//	보스 몹 죽인 퀘스트 등록 처리.
				//if(!GF_ClanWar_Boss_MOB_Kill((LPVOID)pPARAM->m_pOWNER,pREWD->m_Rewd000.iQuestSN))
				//	break;	// 클랜전 퀘스트 등록됨.
				//	트리거로 처리 않함.

				// (20070309:남병철)
				// 클랜전 승리 퀘스트 --> 서버에서 직접 부여
//				if((pREWD->m_Rewd000.iQuestSN >= 2814)&&(pREWD->m_Rewd000.iQuestSN <= 2816))
//					break;
				// 클랜전 승리(2814~2816) 무승부(2817~2819) 퀘스트 --> 서버에서 직접 부여
				if( (pREWD->m_Rewd000.iQuestSN >= 2814) && (pREWD->m_Rewd000.iQuestSN <= 2819) )
					break;
...
*/
	...
	{
		//	돈 복사 방지 초기화
		((classUSER *)pPARAM->m_pOWNER)->mf_Quest_CheckInit(0);
	...
	if ( 1 == pREWD->m_Rewd005.btTarget ) 
	{
		...
		// 보상 금액이 동일한 경우 막는다. pREWD->m_Rewd005.iValue
		if(((classUSER *)pPARAM->m_pOWNER)->mf_Quest_CheckUP(pREWD->m_Rewd005.iValue))
		{
			//	동일 보상금 처리
...
bool F_QSTREWD035 ( uniQstENTITY *pREWD, tQST_PARAM *pPARAM  )
{
	if ( !pPARAM->m_pNpcVAR )
		return false;

	//	팀 타입 번호 지정.
	// 0: 개인 번호 부여, 1: 파티 번호 부여, 2:클랜 번호 부여
	switch( pREWD->m_Rewd035.specType ) {
		case 0 :
			return pPARAM->m_pOWNER->Set_TeamNoFromUNIQUE(pREWD->m_Rewd035.specTeam);
		case 1 :
			return pPARAM->m_pOWNER->Set_TeamNoFromPartyIDX(pREWD->m_Rewd035.specTeam);
		case 2 :
			return pPARAM->m_pOWNER->Set_TeamNoFromClanIDX(pREWD->m_Rewd035.specTeam);
	}
	return true;	
}
#endif
...
//	김영환 추가.
#ifdef __CLAN_WAR_SET
	F_QSTREWD035,	// 팀 번호 부여.
#else
	F_QST_FALSE,
#endif
	F_QST_FALSE,	F_QST_FALSE,
	F_QST_FALSE,	F_QST_FALSE,	F_QST_FALSE,	F_QST_FALSE,	F_QST_FALSE,
	F_QST_FALSE,	F_QST_FALSE,	F_QST_FALSE,	F_QST_FALSE,	F_QST_FALSE,
	F_QST_FALSE,	F_QST_FALSE,	F_QST_FALSE,	F_QST_FALSE,	F_QST_FALSE,
	F_QST_FALSE,	F_QST_FALSE,	F_QST_FALSE,	F_QST_FALSE,	F_QST_FALSE,
...
bool CQuestDATA::LoadQuestTable ()
{
	...
	// (20061121:남병철) : 퀘스트 리스트 확보
	int iRow = m_STB.m_nDataCnt;
	for( int i = 1; i < iRow; i++ )
	{
		if( m_STB.m_ppNAME[i] != NULL )
		{
			g_zQuestList[QLZ_QUEST_NO	].Add( i );
			g_zQuestList[QLZ_QUEST_NAME	].Add( m_STB.m_ppNAME[i] );
		}
	}
