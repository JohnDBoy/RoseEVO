...
////	오브젝에 대한 스킬 명령 설정 AI 함수
////	문제점이 있서 수정 한다. 담당 : 김영환,곽홍근
bool CObjAI::SetCMD_Skill2OBJ (int iServerTarget, short nSkillIDX)
{
...
#endif
	if ( (nSkillIDX == m_nToDoSkillIDX) && 
			(CMD_SKILL2OBJ == m_wCommand) &&
			(iServerTarget == this->Get_TargetIDX()))		return false;
	...
	if ( pDestCHAR ) {
		...
		if ( SKILL_TARGET_FILTER_DEAD_USER == iTarget_Filter)
		{
			// 사용자이고 죽은넘.
		...
		//	김영환 수정 
		//	m_wCommand 명령을 m_wBeforeCMD에 넣기 전에
		if( SKILL_TARGET_FILTER_ENEMY_ALL == iTarget_Filter 
            ...
            m_wBeforeCMD = m_wCommand;	// 타겟이 적인 경우에 커멘트 처리.
		else
