...
//	short nMyTeamNO = pEVENT->m_pSourCHAR->Get_TeamNO();
	...
	while( pFindCHAR ) {
		if ( ( pEVENT->m_pSourCHAR!=pFindCHAR )		&&
			 ...
			 // (20060920:남병철) : 클랜전 몬스터 공격명령 수정
			 pEVENT->m_pSourCHAR->Get_ObjTYPE() == pFindCHAR->Get_ObjTYPE() &&
...
//				 //--------------------------------------------------
//#ifdef WATCH_DATA
//				 g_WatchData.Log( "용?Source %f ==??== 사람?Find %f", pEVENT->m_pSourCHAR->Get_ObjTYPE(), pFindCHAR->Get_ObjTYPE() );
//#endif
//				 //--------------------------------------------------
...
*/
	...
	// pEVENT->m_pSourCHAR는 CObjMOB 이다.
	CObjCHAR* pCHAR = (CObjCHAR*)pEVENT->m_pSourCHAR;
...
struct AIACT24
{
	...
	BYTE		btTarget;		// 0=조건에 체크된 캐릭터, 1=현재 공격타겟, 2=자기자신(셀프스킬), 3=주인 캐릭터(백성주 추가)
...
*/
	...
	switch( pAct->btTarget )
	{
		...
		case 2 :
			...
			break;
//-------------------------------------
// 2006.06.02/김대성/추가 - 3번 동작 주인 캐릭터 타겟팅 추가 
		case 3:
			{
				...
				CAI_OBJ *pOwner2 = pOwner->Get_CALLER();
				pEVENT->m_pSourCHAR->SetCMD_Skill2OBJ( pOwner->Get_TAG(), pAct->nSkill, pAct->nMotion );
			}
			break;
//-------------------------------------
	}
