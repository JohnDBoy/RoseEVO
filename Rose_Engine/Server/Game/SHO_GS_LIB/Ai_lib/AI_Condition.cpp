...
//-------------------------------------
// 2006.05.08/김대성/추가 - 객체가 생성된후 시간체크

bool F_AICOND_30 (stCondHead *pConDATA, t_AIPARAM *pAIParam)	
{
	AICOND30 *pCond = (AICOND30*)pConDATA;

	if((::GetTickCount () - pAIParam->m_pSourCHAR->dwCreatedTime) > (pCond->dwTimeLength * 1000))
	{
//		g_LOG.CS_ODS (0xffff, "** F_AICOND_30() \n");
		return true;
	}
...
//-------------------------------------
// 2006.06.08/김대성/추가 - 주인캐릭터

bool F_AICOND_31 (stCondHead *pConDATA, t_AIPARAM *pAIParam)	
{
	AICOND31 *pCond = (AICOND31*)pConDATA;

	pAIParam->m_pFindCHAR = pAIParam->m_pSourCHAR->Get_CALLER();

	if(pAIParam->m_pFindCHAR != NULL)
		return true;
	else
		return false;
}
...
//-------------------------------------
// 2006.05.08/김대성/수정 - 객체가 생성된후 시간체크
/*
	{	F_AICOND_NULL	},
*/
	{	F_AICOND_30		},
