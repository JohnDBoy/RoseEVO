...
*/
// 2006.03.14/김대성 - 최대 HP (레벨 * HP)를 크게 늘렸더니 short형으로는 계산이 안되는 부분이 있음
	for (int nI=0; nI<this->m_nSavedDamageCNT; nI++) {
//-------------------------------------
...
#ifdef __BUFF_CALC_
int CObjMOB::Get_AbilityValue2 (WORD wType)
{
	switch( wType ) {
		//		case AT_STR		:		return Get_STR ();
		//		case AT_DEX		:		return Get_DEX ();
		...
		if ( this->m_SavedDAMAGED[ nI ].m_dwInsertTIME ) {
			...
			if ( pHitterUSER && pHitterUSER->GetZONE() == this->GetZONE() ) {
...
#ifndef __EXPERIENCE_ITEM // 경험치 2배 아이템이 추가되면서 수식이 바뀜
				iEXP = (iEXP*(this->GetZONE())->Get_Exp_LV()/100);
#endif
#endif
				...
				if ( iEXP > 0 ) {
					...
					// 존 경험치 및 이벤트(GM치트) 경험치 적용 + 마일리지 경험치 2배 아이템 적용
					iEXP = pHitterUSER->ExecuteExpBuff( iEXP );
#endif
					// 파티 없으면 바로 경험치 올림.
...
//							ZTRACE( "같은 파티원의 경험치를 몰아서 얻는다 OBJ_ID : %d, Damage : %d", m_SavedDAMAGED[ nI ].m_iObjectIDX, m_SavedDAMAGED[ nI ].m_iDamage );
				...
				// 누적된 경험치를 파티를 통해 전송 <-- -_-+++???
				// (20061016:남병철) : 파티 상태에서 파티원과 떨어져서 홀로 사냥하게되면 파티 경험치 적용이 해제되어
				// 그 동안 사냥한 경험치가 누적된 개인 경험치를 먹게된다.
...
#ifdef __EXPERIENCE_ITEM
					// 존 경험치 및 이벤트(GM치트) 경험치 적용 + 마일리지 경험치 2배 아이템 적용
					int iEXPResult = this->m_SavedDAMAGED[ nI ].m_iDamage;
					iEXPResult = pHitterUSER->ExecuteExpBuff( iEXPResult );
					// 파티 경험치 적용안됐다.
					pHitterUSER->Add_EXP( iEXPResult, true, this->Get_INDEX() );
#else
					// 파티 경험치 적용안됐다.
...
void CObjSUMMON::SetCallerOBJ( int iOwnerOBJ, t_HASHKEY HashOwnerOBJ, int iOwnerLEVEL, short nSkillIDX, CObjCHAR * pOwnerAVT)
{
	...
	if( IsTAIWAN() )
	{
		...
		//////////////////////////////////////////////////////////////////////////
		// 2006.06.20:이성활:소환시 Owner의 능력치 중 버프에 의한 능력치는 제외
		...
		m_iOriATK	= (int)( CObjMOB::GetOri_ATK()	+ pOwnerAVT->GetOri_ATK() * (nSkillLEV * 5 + 75 ) * 0.01f );
		m_iOriHIT	= (int)( CObjMOB::GetOri_HIT()	+ pOwnerAVT->GetOri_HIT() );
		m_iOriDEF	= (int)( CObjMOB::GetOri_DEF()	+ pOwnerAVT->GetOri_DEF() );
		m_iOriRES	= (int)( CObjMOB::GetOri_RES()	+ pOwnerAVT->GetOri_RES() );
		m_iOriAVOID = (int)( CObjMOB::GetOri_AVOID() + pOwnerAVT->GetOri_AVOID() );
		//////////////////////////////////////////////////////////////////////////
