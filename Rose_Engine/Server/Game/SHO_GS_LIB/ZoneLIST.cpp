...
////	해당 존의 PK 상태를 얻는다. : 김영환 작성
BOOL CZoneLIST::Get_PK_FLAG(short nZoneNO)
{
	CZoneTHREAD *pZONE = this->GetZONE( nZoneNO );
	if ( pZONE )
		return pZONE->Get_PK_FLAG();
	return false;
}

////	해당 존에 모든 유저에게 지정 Packet 전송 : 김영환 작성
/*
	해당 존에 모든 유저에게 전송할 데이터 전송.
	char *p_Data 버퍼는 t_PACKET 구조체에 설정된 정보를 구성해서 넘겨 주어야한다.
*/
BOOL CZoneLIST::Send_gsv_All_User_Data(short nZoneNO,char *p_Data)
{
	//	존 스레드 확인.
	if (NULL == m_ppThreadZONE[ nZoneNO ] )		return false;
	if(p_Data == NULL) return false;

	//	전송할 데이터 버퍼 할당.
	m_ppThreadZONE[ nZoneNO ]->SendPacketToZONE( (t_PACKET *)p_Data );
	return true;
}
...
////	해당 존의 모든 몹을 죽인다. : 작성자 김영환
void CZoneLIST::Zone_MOB_Kill(short nZoneNO,classUSER *pUSER,DWORD	p_Team_N)
{
	//	존 스레드 확인
	CZoneTHREAD *pZONE = this->GetZONE( nZoneNO );
	if ( pZONE )
	{
		pZONE->Kill_AllMOB_Team(pUSER,p_Team_N);
	}
}

////	해당 존의 몬스터의 채력을 회복 시킨다.
BOOL	CZoneLIST::Zone_MOB_HP_Reset(short nZoneNO,DWORD p_Team_N)
{
	//	존 스레드 확인
	CZoneTHREAD *pZONE = this->GetZONE( nZoneNO );
	if ( pZONE )
	{
		pZONE->Set_MAX_HP_All_MOB(p_Team_N);
	}
	return true;
}

////	해당 존의 팀 유저 갯수 얻기
DWORD	CZoneLIST::Get_Zone_Team_User(short nZoneNO,DWORD p_Team_N)
{
	//	존 스레드 확인
	CZoneTHREAD *pZONE = this->GetZONE( nZoneNO );
	if ( pZONE )
	{
		return pZONE->Get_Zone_Team_User_No(p_Team_N);
	}
	return 0;
}
