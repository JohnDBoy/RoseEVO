...
{
#ifdef	_DEBUG
	if ( nActionIdx >= g_TblAniTYPE.m_nDataCnt ||nActionIdx<0)
	{
		//assert( 0 && "nActionIdx >= g_TblAniTYPE.m_nDataCnt" );
		g_LOG.CS_ODS( 0xffff, "%s  nActionIdx >= g_TblAniTYPE.m_nDataCnt", Get_NAME());
		return NULL;
	}
#endif
...
short CObjAVT::SetPartITEM (short nEquipInvIDX)
{
...
//--------------------------------------
// 2006.11.20/추가/김대성 - 계산식 수정
#ifdef __BUFF_CALC_
int CObjAVT::Get_AbilityValue2 (WORD wType)
{
	switch( wType ) {
		// case AT_BIRTHSTONE :	break;
		case AT_SEX		:				return ( GetCur_RACE() & 0x01 );
		...
		case AT_HP		:				return GetCur_HP (); 
		case AT_MP		:				return GetCur_MP ();
		case AT_ATK		:				return GetCur_ATK2 ();				// 공격력 수정
		case AT_DEF		:				return GetCur_DEF2 ();				// 방어력 수정
		case AT_HIT		:				return GetCur_HIT2 ();				// 명중력 수저
		case AT_RES		:				return GetCur_RES2 ();				// 항마력 수정
		case AT_AVOID	:				return GetCur_AVOID2 ();			// 회피력 수정
		case AT_SPEED	:				return (int)GetCur_MOVE_SPEED2 ();	// 이동력 수정
		case AT_ATK_SPD :				return GetCur_ATK_SPD2 ();			// 공격속도 수정
		case AT_WEIGHT	:				return GetCur_WEIGHT ();
		case AT_CRITICAL   :			return Get_CRITICAL2 ();			// 크리티컬 수정
...
*/
// 2006.06.26/김대성/수정 - 무기에 의한 연료 감소 추가, 카트 정탄화
void CObjAVT::Dec_EngineLife(BOOL bWeapon)
//-------------------------------------
{
	tagITEM *pEngine = &m_Inventory.m_ItemRIDE[ RIDE_PART_ENGINE ];
	if ( ITEM_TYPE_RIDE_PART != pEngine->GetTYPE() )
...
//-------------------------------------
// 2006.06.26/김대성/추가 - 무기에 의한 연료 감소 추가
	if(bWeapon == TRUE)
	{
		tagITEM *pWeapon = &m_Inventory.m_ItemLIST[ INVENTORY_RIDE_ITEM0 + RIDE_PART_ARMS ];
		iUseValue = iUseValue + PAT_ITEM_USE_FUEL_RATE( pWeapon->GetItemNO() );
	}
//-------------------------------------
//-------------------------------------
// 2006.09.12/김대성/추가 - 본체에 의한 연료 감소 추가
	if(bWeapon == TRUE)
	{
		tagITEM *pBody = &m_Inventory.m_ItemLIST[ INVENTORY_RIDE_ITEM0 + RIDE_PART_BODY ];
		iUseValue = iUseValue + PAT_ITEM_USE_FUEL_RATE( pBody->GetItemNO() );
	}
//-------------------------------------
//-------------------------------------
// 2006.09.12/김대성/추가 - 어빌리티에 의한 연료 감소 추가
#ifdef __KCHS_BATTLECART__
	if(bWeapon == TRUE)
	{
		tagITEM *pAbility = &m_Inventory.m_ItemLIST[ INVENTORY_RIDE_ITEM0 + RIDE_PART_ABIL ];
		iUseValue = iUseValue + PAT_ITEM_USE_FUEL_RATE( pAbility->GetItemNO() );
	}
#endif
//-------------------------------------
...
#else
// 2006.06.26/김대성/수정 - 무기에 의한 연료 감소 추가, 카트 정탄화
		this->Dec_EngineLife (TRUE);		// 공격시 마다 엔진 연료 감소... 무기에 의한 연료 감소 추가 - 나중에 기획확정되면 적용
#endif
//-------------------------------------
...
{ 
	if ( cValue >= 0 && cValue < MAX_UNION_COUNT )	{
		// (20070507:남병철) : 조합 포인트 변경을 통해 서버 부하를 주는 공격을 들어오고 있어서 로그 막음
		// 디버그 뷰에 로그가 가득차서 서버 떨어짐 : 필리핀에서 첫 발생
