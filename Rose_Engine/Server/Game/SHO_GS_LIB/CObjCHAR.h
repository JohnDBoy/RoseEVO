...
public :
	bool Send_gsv_DAMAGE2Sector( int iAttackObject, __int64 wDamage, CObjITEM *pDropITEM );
//-------------------------------------
...
*/
// 2006.03.08/김대성- 최대 데미지 수정 => NET_Prototype.h 파일의 uniDAMAGE 구조체 수정 (WORD -> __int64)
	bool Send_gsv_DAMAGE2Target( CObjCHAR *pAtkOBJ, __int64 wDamage );
//-------------------------------------
...
*/
// 2006.03.08/김대성- 최대 데미지 수정 => NET_Prototype.h 파일의 uniDAMAGE 구조체 수정 (WORD -> __int64)
	bool Send_gsv_DAMAGE_OF_SKILL (int iSpellOBJ, short nSkillIDX, __int64 wDamage, BYTE btResult, short nSpellerINT, CObjITEM *pDropITEM=NULL);
//-------------------------------------
...
public :
	...
	bool  Send_gsv_CHAT ( char *szMsg,WORD wPHeader = GSV_CHAT );
...
#ifdef __BUFF_CALC_
virtual int		Get_AbilityValue2 (WORD wType)=0;
#endif
//-------------------------------------
...
// 2006.11.20/김대성/수정 - 계산공식 수정
	...
	int		Get_HIT ()				{	int iR = GetOri_HIT()+m_IngSTATUS.Adj_HIT();			return (iR>10) ? iR : 10;	}	// 명중력
	int		Get_AVOID ()			{	int iR = GetOri_AVOID()+m_IngSTATUS.Adj_AVOID();		return (iR>10) ? iR : 10;	}	// 회피력
	int		Get_CRITICAL()			{	int iR = GetOri_CRITICAL()+m_IngSTATUS.Adj_CRITICAL();	return (iR>10) ? iR : 10;	}	// 크리티컬
#else	//-----------------------------
	float	Get_MoveSPEED ()		
	{	
		if ( !m_bRunMODE ) return GetOri_WalkSPEED();
		...
		return ( nR > 100 ) ? nR : 100.f;
	}
	int		Get_ATK ()				{	int iR = GetOri_ATK()+m_IngSTATUS.Adj_APOWER();			return (iR>30) ? iR : 30;	}	// 공격력
	int		Get_DEF ()				{	int iR = GetOri_DEF()+m_IngSTATUS.Adj_DPOWER();			return (iR>30) ? iR : 30;	}	// 방어력
	int		Get_RES ()				{	int iR = GetOri_RES()+m_IngSTATUS.Adj_RES();			return (iR>10) ? iR : 10;	}	// 항마력
	int		Get_HIT ()				{	int iR = GetOri_HIT()+m_IngSTATUS.Adj_HIT();			return (iR>30) ? iR : 30;	}	// 명중력
	int		Get_AVOID ()			{	int iR = GetOri_AVOID()+m_IngSTATUS.Adj_AVOID();		return (iR>30) ? iR : 30;	}	// 회피력
	int		Get_CRITICAL()			{	int iR = GetOri_CRITICAL()+m_IngSTATUS.Adj_CRITICAL();	return (iR>10) ? iR : 10;	}	// 크리티컬
#endif
//-------------------------------------
//-------------------------------------
// 2006.11.20/김대성/추가 - 계산공식 수정
#ifdef __BUFF_CALC_
	short	Get_nAttackSPEED2 ();

	float	Get_MoveSPEED2 ()		
	{	
		if ( !m_bRunMODE ) return GetOri_WalkSPEED();
	...
	int		Get_ATK2 ()				{	int iR = GetOri_ATK();			return (iR>30) ? iR : 30;	}	// 공격력
	int		Get_DEF2 ()				{	int iR = GetOri_DEF();			return (iR>30) ? iR : 30;	}	// 방어력
	int		Get_RES2 ()				{	int iR = GetOri_RES();			return (iR>10) ? iR : 10;	}	// 항마력
	int		Get_HIT2 ()				{	int iR = GetOri_HIT();			return (iR>30) ? iR : 30;	}	// 명중력
	int		Get_AVOID2 ()			{	int iR = GetOri_AVOID();		return (iR>30) ? iR : 30;	}	// 회피력
	int		Get_CRITICAL2()			{	int iR = GetOri_CRITICAL();	return (iR>10) ? iR : 10;	}		// 크리티컬
#endif
//-------------------------------------
	...
	void	Set_IngSTATUS( DWORD dwFlags );
...
*/
// 2006.03.13/김대성 - 최대 데미지 수정 => NET_Prototype.h 파일의 uniDAMAGE 구조체 수정 (WORD -> __int64)
	void	Add_DAMAGE ( __int64 wDamage )
//-------------------------------------
	{
		// 자살 명령
