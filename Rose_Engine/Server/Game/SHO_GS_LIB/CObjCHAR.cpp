...
//-------------------------------------------------------------------------------------------------
bool CObjCHAR::Send_gsv_CHAT( char *szMsg,WORD wPHeader )
{
    classPACKET *pCPacket = Packet_AllocNLock ();
	if ( !pCPacket )
		return false;

    pCPacket->m_HEADER.m_wType = wPHeader;
...
*/
// 2006.03.08/김대성 - 최대 데미지 수정 => NET_Prototype.h 파일의 uniDAMAGE 구조체 수정 (WORD -> __int64)
bool CObjCHAR::Send_gsv_DAMAGE2Sector( int iAttackObject, __int64 wDamage, CObjITEM *pDropITEM)
//-------------------------------------
{
	...
	//	죽은 놈이 유저인 경우 직접 죽은 패킷 전송한다.
	//	섹터 패킷으로 전송 하면 모든 유저가 받지만.. 본인이 빠질수 있다.
	//	영환이의 생각... 
	//	게임 유저 인지 확인하고..
	if(this->IsUSER())
	{
		...
		{
			//	죽었다는 확인 메시지 전송한다.
			((classUSER *)this)->Send_gsv_User_MSG_1LV(1,0);
		}
	}
#endif
...
*/
// 2006.03.08/김대성- 최대 데미지 수정 => NET_Prototype.h 파일의 uniDAMAGE 구조체 수정 (WORD -> __int64)
bool CObjCHAR::Send_gsv_DAMAGE2Target( CObjCHAR *pAtkOBJ, __int64 wDamage )
//-------------------------------------
{
    classPACKET *pCPacket = Packet_AllocNLock ();
	if ( !pCPacket ) 
...
*/
// 2006.03.08/김대성- 최대 데미지 수정 => NET_Prototype.h 파일의 uniDAMAGE 구조체 수정 (WORD -> __int64)
bool CObjCHAR::Send_gsv_DAMAGE_OF_SKILL (int iSpellOBJ, short nSkillIDX, __int64 wDamage, BYTE btResult, short nSpellerINT, CObjITEM *pDropITEM)
//-------------------------------------
{
	classPACKET *pCPacket = Packet_AllocNLock ();
	if ( !pCPacket )
...
BYTE CObjCHAR::Apply_DAMAGE (CObjCHAR *pTarget, uniDAMAGE &sDamage, CObjITEM **ppOutITEM, bool bDropItem)
{
	...
	//	클랜전 PK 모드 정지 이면 공격 데미지 없음 [AI도 중지해야한다.]
#ifdef __CLAN_WAR_EXT

	// 클랜전 캐릭터이고 PK 모드가 해제된 클랜전 맵(클랜전 종료)이면 데미지 불가
	if( CClanWar::GetInstance()->IsPKMode( this ) == FALSE )
		return SEND_DAMAGE_TO_NULL;

#else
	if(!GF_ClanWar_PK_Is((LPVOID)this))
		return SEND_DAMAGE_TO_NULL;
#endif
	...
	if ( iDamage )
	{
		...
		{
			//20060630 홍근 : 무적 상태일 경우 정확히 hp가 0이 되는경우 몬스터가 아바타 인식을 못하게 되는 경우 수정.
			pTarget->Set_HP( pTarget->Get_MaxHP() );
		...
		if ( pTarget->Sub_HP( iDamage ) <= 0 )
		{
			...
			{
				if ( pTarget->IsUSER() || pTarget->GetCallerUsrIDX() ) 
				{
					// USER 또는 소환된몹
					// TODO:: PK !!!!
				} 
				else
				{
					...
					{		// MOB or NPC
						...
						/*
							클랜전 보스 몹이 죽었는지 확인
							보스 몹이 죽었다면 트리거 발동 사용안하고 서버에서 검출하여 분기 처리한다.
							작성자 : 김영환
						*/
#ifdef	__CLAN_WAR_SET
...
#ifdef __CLAN_WAR_EXT
						// 클랜전 보스가 죽었는가?
						CClanWar::GetInstance()->IsBossKilled( (CObjMOB*)pTarget );

#else
						//	해당 몬스터가 클랜전 보스인지 확인
						//	this는 유저가 아니라 유저 소환 몹일수있다.
						GF_ClanWar_Boss_Kill_Check((LPVOID)pTarget,(LPVOID)this);
#endif

#endif

						/*	
							몹을 죽인 유저를 기준으로 몹이 소유한 이벤트 및 아이템 드랍 결정. 로직이다.
							주석 : 감영환
							재발좀... 주석점 달아라....띠블
						*/
						if ( pMobOWNER ) 
						...
						#ifdef __KRRE_ITEMDROP    //한국쪽 리뉴얼 드랍이면...
								if ( CCal::Get_ReNewal_DropITEM( pMobOWNER->Get_LEVEL()-pTarget->Get_LEVEL(), (CObjMOB*)pTarget, sITEM, this->GetZONE()->Get_ZoneNO(), pMobOWNER->GetCur_DropRATE(), pMobOWNER->Get_CHARM() ) ) 
						#else
	...
	//-------------------------------------
	// 2006.03.09/김대성 - 막타유저 로그남기기. 무지막지하게 체력이 높은 몬스터를 수십명의 유저가 일주일동안 사냥해서 잡는 이벤트 (스페셜 보스이벤트 - 필리핀 탈라)
	//					   몬스터를 죽일때 마지막으로 공격한 유저의 정보를 로그로 남긴다.
	//					   DB에 로그를 남기는 것이 귀찮으므로(새로운 테이블도 만들어야 하고..) 텍스트 파일로 로그를 남긴다.
						{
							int iNpcIndex = pTarget->Get_CharNO();	// 이벤트용 몹일경우에만 로그를 남긴다.
																	// stb파일에서 읽어와서 처리해야 하나 귀찮아서 그냥 몬스터 번호를 
																	// 그대로 하드코딩해 넣어서 처리한다.
							if(996 <= iNpcIndex && iNpcIndex <= 998)	// 996(모스골렘), 997(네펜더스), 998(투라크)
							{
								struct tm *l;
								time_t long_time;

								time( &long_time );				/* Get time as long integer. */
								l = localtime( &long_time );	/* Convert to local time. */

								FILE *fp;
								fp = fopen("SHO_GS_Event_Log.txt", "a+");
								
								if(this->IsUSER())	// 사용자
								{
									// 케릭 이름으로 계정 얻기
									classUSER *pUSER=NULL;
									pUSER = g_pUserLIST->Find_CHAR(this->Get_NAME());
									if ( pUSER )
									{
										fprintf(fp, "%04d/%02d/%02d %02d:%02d:%02d - ( %s )( %s ) kill ( %s  %d )\n", 
											l->tm_year+1900, l->tm_mon+1, l->tm_mday, l->tm_hour, l->tm_min, l->tm_sec,
											pUSER->Get_ACCOUNT(), this->Get_NAME(), pTarget->Get_NAME(), pTarget->Get_CharNO());
									} 
									else
									{
										fprintf(fp, "%04d/%02d/%02d %02d:%02d:%02d - ( ??? )( %s ) kill ( %s  %d )\n", 
											l->tm_year+1900, l->tm_mon+1, l->tm_mday, l->tm_hour, l->tm_min, l->tm_sec,
											this->Get_NAME(), pTarget->Get_NAME(), pTarget->Get_CharNO());
									}
								}
								else			// 사용자가 소환한 소환수
								{
									...
									// 케릭 이름으로 계정 얻기
									classUSER *pUSER=NULL;
									pUSER = g_pUserLIST->Find_CHAR(pSummonOWNER->Get_NAME());
									if ( pUSER )
									{
										fprintf(fp, "%04d/%02d/%02d %02d:%02d:%02d - ( %s )( %s - %s ) kill ( %s  %d )\n", 
											l->tm_year+1900, l->tm_mon+1, l->tm_mday, l->tm_hour, l->tm_min, l->tm_sec,
											pUSER->Get_ACCOUNT(), pSummonOWNER->Get_NAME(), this->Get_NAME(), 
											pTarget->Get_NAME(), pTarget->Get_CharNO());
									} 
									else
									{
										fprintf(fp, "%04d/%02d/%02d %02d:%02d:%02d - ( ??? )( %s - %s ) kill ( %s  %d )\n", 
											l->tm_year+1900, l->tm_mon+1, l->tm_mday, l->tm_hour, l->tm_min, l->tm_sec,
											pSummonOWNER->Get_NAME(), this->Get_NAME(), 
											pTarget->Get_NAME(), pTarget->Get_CharNO());
									}
								}
								fclose(fp);
	...
	//-------------------------------------
		...
		else 
		{
			...
			{	
				// 파티일 경우 파티원에게 피 전송...
				if ( pTarget->GetPARTY() ) {
					pTarget->GetPARTY()->Change_ObjectIDX( (classUSER*)pTarget );
				}
			}

			/*
				해당 몬스터가 클랜전 보스인지 확인한다.
				클랜전 보스의 HP 변동에 따라서 HP 변동 메시지 전송한다.
				작성자 : 김영환
			*/
#ifdef	__CLAN_WAR_SET
			...
			if (pTarget->IsNPC()) 
			{
				...
				if( (pZoneThread = pTarget->GetZONE()) != NULL )
				{
					CClanWar::GetInstance()->NotifyBossHP( pZoneThread->Get_ZoneNO() );
				}
			}
#else
			//	대상이 NPC 또는 몹인지 확인.
			...
			{
				//	클랜전 보스 몹인지 확인.
				DWORD	temp_List = GF_ClanWar_Boss_MOB_Is((LPVOID)pTarget);
				//	존이 플레이 중인지 확인.
				if(GF_Clan_War_Zone_Play_Is(temp_List))
				{
					//	보스 몹의 HP 전송 처리.
					GF_ClanWar_Boss_MOB_HP(temp_List);
				}
			}
#endif
...
*/
// 2006.03.07/김대성 - 최대 데미지 수정 => NET_Prototype.h 파일의 uniDAMAGE 구조체 수정 (WORD -> __int64)
						sDamage.m_wDamage = (__int64) ( (double)sDamage.m_wVALUE * SKILL_POWER( nShieldSKILL ) / 100.f );
//-------------------------------------
...
//#endif
		...
		nAdjValue = CCal::Get_SkillAdjustVALUE( this, nSkillIDX, nI, pSpeller->Get_INT() ,nIngSTB);
		// 적용 가능 한가 ???
		...
		//20060630 홍근 : 공속 버프 스킬 수정.
		DWORD	dwFlags = c_dwIngFLAG[ STATE_TYPE(nIngSTB) ];
		Set_IngSTATUS( dwFlags );
...
//-------------------------------------
// 2006.06.20/김대성/추가 - 범위공격이 제대로 적용안돼는 버그 수정
	if(SKILL_SCOPE( Get_ActiveSKILL() ) > 0)
	{
		BYTE btResult;
		uniDAMAGE sDamage;
		CObjCHAR *pFindCHAR;
		short nResultCNT=0;
		tPOINTF &PosCENTER = pTarget->Get_CurPOSITION();
		...
		while( pFindCHAR ) 
		{
			if(pFindCHAR == pTarget)	// 타겟팅된거 빼고 나머지
			{
				pFindCHAR = (CObjCHAR*)this->AI_FindNextOBJ ();
				continue;
			}
		...
		this->Send_gsv_RESULT_OF_SKILL( Get_ActiveSKILL() );
	}
//-------------------------------------
...
void CObjCHAR::Set_IngSTATUS( DWORD dwFlags )
{
	if ( dwFlags ) 
	{
		if ( dwFlags & ( ING_INC_MOV_SPD | ING_DEC_MOV_SPD ) ) 
		{
		...
		if ( dwFlags & ( c_dwIngFLAG[ ING_INC_ATK_SPD ] | c_dwIngFLAG[ ING_DEC_ATK_SPD ] ) ) 
		{
...
#ifdef __BUFF_CALC_
short CObjCHAR::Get_nAttackSPEED2 ()
{
	int iR = GetOri_ATKSPEED();
