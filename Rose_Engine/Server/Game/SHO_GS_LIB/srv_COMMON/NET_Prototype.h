/**
 * \file net_prototype.h
 * \date 2005-08-25
 *
 * \brief
 *		server <-> client 통신 프로토콜및 데이타타입 설정
 *
 */
/*
...
//-------------------------------------------------------------------------------------------------
// 9-05-2008 : Lynxluna : prevent unpatched client, change packet values
...
#define GSV_CHAR_HPMP_INFO      0x07ec							//1초마다 서버에서 클라이언트에게 MP/HP 보낸다...      

#define CLI_TRADE_CHAT				0x07ed							//거래채팅
#define GSV_TRADE_CHAT				0x07ed							//거래채팅


#define GSV_MILEAGE_INV_DATA  0x801             //마일리지 인벤 데이터..
#define GSV_DESTROY_MILEAGE_ITEM 0x802	 //파괴된 마일리지아이템..
#define GSV_NOTIFY_MIEAGE_ITEM 0x803			 //변경된 마일리지 아이템 정보
#define 	GSV_SET_MILEAGE_INV_ONLY			0x0804       //마일리지 인벤셋팅..


#define CLI_USE_MILEAGE_ITEM 0x805
#define GSV_USE_MILEAGE_ITEM 0x805              //마일리지 아이템사용

#define CLI_EQUIP_MILEAGE_ITEM 0x806          //마일리지 아이템 착용
#define GSV_EQUIP_MILEAGE_ITEM 0x806          //마일리지 아이템 착용

#define CLI_EQUIP_RIDE_MILEAGE_ITEM 0x807   //카트 마일리지아이템 착용
#define GSV_EQUIP_RIDE_MILEAGE_ITEM 0x807  //카트 마일리지 아이템 착용

//-------------------------------------
// 2006.03.29/김대성/추가 - Arcfour 암호화
#define CLI_DUMMY_1					0x808	// 암호화 관련 더미패킷1 - 패킷의 크기는 일정범위 안에서 임의의 크기로 - 서버응답 필요없음.
//-------------------------------------

//-------------------------------------
//	2006.07.24 김영환 클랜전 관련 메시지 헤더 추가
#define GSV_CLANWAR_INTO_OK		0x0810	// 클랜전 참가 확인. [서버=>클라]
#define CLI_CLANWAR_IN_OK		0x0811	// 클랜전 참가 OK 메시지 [클라=>서버]
#define GSV_CLANWAR_TIME		0x0812	// 클랜전 타임 메시지 [서버=>클라]
#define GSV_CLANWAR_ERR			0x0813	// 클랜전 오류 처리 메시지 [서버=>클라]
#define GSV_CLANWAR_PROGRESS	0x0814	// 클랜전 진행 메시지 타입 [서버=>클라]
//	2006.11.9 김영환 클랜전 2차 기능 추가 내용
#define GSV_CLANWAR_BOSS_HP		0x0816	// 클랜전 보스 HP 전송	[서버=>클라] 주기적으로 전송


//-------------------------------------
// (20060912:남병철) : 파티 경험치 획득 제한 관련 메시지 헤더
#define GSV_WARNING_MSGCODE		0x0815	// [서버->클라]


//	2006.09.18 김영환 접속 유저의 중요 메시지 헤더
//	__MESSAGE_LV_1 정의에 의한 1등급 메시지 처리.
#define GSV_USER_MSG_1LV		0x0820	// 1등급 메시지 처리... 해더.

// (20061204:남병철) : 경험치 2배 아이템
#ifdef __EXPERIENCE_ITEM
#define CLI_MILEAGE_ITEM_TIME	0x0821	// [클라->서버]
#define GSV_MILEAGE_ITEM_TIME	0x0822	// [서버->클라]
#endif
...
#define LSV_DBG_CMD					0x0DEB
#define WSV_DBG_CMD					0x0DEB
#define GSV_DBG_CMD					0x0DEB
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------
// 2006.03.29/김대성/추가 - Arcfour 암호화
// 암호화 관련 더미패킷1 - 패킷의 크기는 일정범위 안에서 임의의 크기로
struct cli_DUMMY_1 : public t_PACKETHEADER {
	BYTE m_Dummy[128];
};
//-------------------------------------

// n-protect 모듈과 통신시 사용하는 패킷구조
...
struct srv_DBG_CMD : public t_PACKETHEADER
{
	// szCommand [] // 데바그 명령
};
...
struct gsv_SELECT_CHAR_old : public t_PACKETHEADER {
	BYTE				m_btCharRACE;
	short				m_nZoneNO;
	tPOINTF				m_PosSTART;
	short				m_nReviveZoneNO;

	tagPartITEM			m_PartITEM[ MAX_BODY_PART - 1 ];
...
//클라이언트에서 스샷찍을때 서버시간을 요청한다..
//아래 구조체에 서버시간을 저장해서 보내준다..
...
struct gsv_CHAR_HPMP_INFO:public t_PACKETHEADER
{
	short m_CurHP;
	short m_CurMP;
};
...
struct cli_TRADE_CHAT : public cli_CHAT {
} ;
struct gsv_TRADE_CHAT : public t_PACKETHEADER {
} ;
...
*/
// 2006.03.16/김대성 - 최대 데미지 2047(0x07ff) 제한을 없애기 위해 자료형 수정 ( #define	MAX_DAMAGE			0x07ff )
union uniDAMAGE {
	__int64	m_wDamage;				// WORD => __int64 수정  
	struct {
		DWORD	m_wVALUE;			// 비트단위로 쪼개지 않고 그냥 씀
		DWORD	m_wACTION;			// 비트단위로 쪼개지 않고 그냥 씀
	} ;
} ;
//-------------------------------------
...
*/
// 2006.03.16/김대성 - 최대 데미지 수정 - 2147483647
...
*/
// 2006.03.16/김대성 - 최대 데미지 수정 - uniDAMAGE 의 데이타형이 WORD 에서 DWORD로 바뀜
// gsv_DAMAGE::m_wDamage와 연산되는 값.
#define	DMG_BIT_DUMMY		0x0100000000	// 추가 데미지 붙은 거다 :: 자신이 때린경우 추가 데미지 계산을 해.
#define	DMG_BIT_IMMEDIATE	0x0200000000	// 패키 받고 즉각 처리 하면 된다:예)방패 뎀쥐 스킬에 의해 반사된 뎀쥐
#define	DMG_BIT_HITTED		0x0400000000	// 맞는 동작을 한다.
#define	DMG_BIT_CRITICAL	0x0800000000	// 크리티컬 데미지다
#define	DMG_BIT_DEAD		0x1000000000	// 죽었다.
//-------------------------------------
...
// (20070322:남병철) : 클랜 보상 포인트 부족
#define STORE_TRADE_RESULT_OUT_OF_REWARD_POINT	0x07	// 클랜 보상 포인트 부족
...
typedef gsv_DESTROY_MILEAGE_ITEM gsv_NOTIFY_MILEAGE_ITEM;   //변경된 마일리지 아이템 통보
typedef gsv_DESTROY_MILEAGE_ITEM gsv_SET_MILEAGE_INV;						  //마일리지 인벤정보 통보..


typedef cli_USE_ITEM cli_USE_MILEAGE_ITEM;											//마일리지 아이템 사용
typedef gsv_USE_ITEM gsv_USE_MILEAGE_ITEM;

typedef cli_EQUIP_ITEM cli_EQUIP_MILEAGE_ITEM;								//마일리지 아이템 착용				
...
struct gsv_TELEPORT_REPLY : public t_PACKETHEADER {
	...
	BYTE	m_btRideMODE;
//-------------------------------------
// 2006.05.10/김대성/추가 - 로딩이미지때문에 워프할때마다 특정npc의 상태값을 같이 전송해 준다. 
//                          클라이언트쪽에만 추가. 서버쪽은 예전 그대로...
//	BYTE	m_pDATA[52];	// sizeof( tagObjVAR ) 
//-------------------------------------
} ;
...
struct gsv_EFFECT_OF_SKILL : public t_PACKETHEADER {
	...
	//	unsigned short	m_btSuccessBITS : 4;	// 성공여부
	//} ;
	union {
		struct {
			unsigned short	m_nSkillIDX		: 12;
...
#define CRAFT_DRILL_SOCKET_FROM_ITEM		0x06			//드릴아이템 사용으로 소켓생성..
#define CRAFT_DRILL_SOCKET_FROM_NPC		0x07			//드릴아이템 사용으로 소켓생성..
...
struct cli_CRAFT_DRILL_SOCKET_REQ : public cli_CRAFT_ITEM_REQ {
	short	m_nUseInvSLOTorNpcIDX;			//사용할 드릴아이템 인벤번호 또는 npc번호
	BYTE	m_btTargetInvIDX;						//대상 아이템 인벤 번호
} ;
...
#define CRAFE_INVALID_ITEM				0x08 //유효하지 않은 아이템
#define CRAFE_NOT_ENOUGH_MONEY		0x09  //돈불충분
#define CRAFE_NOT_ENOUGH_MP             0x0A  //MP 불충분
...
#define CRAFE_GIFTBOX_SUCCESS		0x13	// 선물상자 분해 성공

//소켓드릴 시스템..                   허재영 (2005.10.27)
#define CRAFT_DRILL_SOCKET_SUCCESS	0x20    //소캣 생성 성공
#define CRAFT_DRILL_CLEAR_USEITME      0x21    // 드릴 아이템만 사라졌다..
#define CRAFE_DRILL_INVALID_USEITEM   0x22    //드릴 아이템이 아니다.
#define	CRAFT_DRILL_EXIST_SOCKET	        0x23	  // 소켓이 이미 있다.
#define	CRAFT_DRILL_INVALID_TITEM	    0x24	  //소켓을뚫을 수 없는 아이템이다...
...
struct cli_QUEST_REQ_EX : public t_PACKETHEADER {
	BYTE	m_btTYPE;
	BYTE	m_btQuestSLOT;
	union {
		int			m_iQuestID;
	...
	t_HASHKEY	m_SucTriggerHash;
} ;
#define	TYPE_QUEST_REQ_ADD						0x01	// 퀘스트 추가 요청
#define	TYPE_QUEST_REQ_DEL						0x02	// 퀘스트 삭제 요청
#define	TYPE_QUEST_REQ_DO_TRIGGER				0x03	// 퀘스트 완료 검증 요청
...
struct cli_MESSENGER_CHAT : public t_PACKETHEADER {
	DWORD	m_dwUserTAG;
	char	m_ToName[31];	// (20061102:남병철) : 1:1 채팅 받는 사람 이름
	char	m_szMSG[0];
} ;
struct wsv_MESSENGER_CHAT : public t_PACKETHEADER {
	DWORD	m_dwUserTAG;
	char	m_FromName[31];	// (20061102:남병철) : 1:1 채팅 보내는 사람 이름
	char	m_szMSG[0];
} ;
#else
struct cli_MESSENGER_CHAT : public t_PACKETHEADER {
...
#ifdef __COMMUNITY_1_1_CHAT
// (20061102:남병철) : 아래 두가지 경우에 모두 사용하는 구조체
// CHAT_REPLY_USER_ADD
// CHAT_REPLY_ROOM_JOINED
struct wsv_CHAT_ROOM_USER : public tag_CHAT_HEADER {
	WORD	m_wUserID;
	DWORD	m_dwDBID;	// (20061101:남병철) : 채팅룸에서 개별 채팅 기능 추가
	// szName[]
} ;
#else
struct wsv_CHAT_ROOM_USER : public tag_CHAT_HEADER {
...
#ifdef __COMMUNITY_1_1_CHAT
#define CHAT_REPLY_MAKE_TITLE_DUP	0x018	// (20061027:남병철) : 방제 중복으로 방만들기 실패
#endif
...
#define REPLY_MALL_ITEM_INVAILD_MILEAGE 0x0c            //마일리지 아이템이 아니다.
...
struct gsv_MALL_ITEM_REPLY : public t_PACKETHEADER {
	...
	*/
	...
	union {
#ifdef KRRE_MILEAGE_ITEM
		Set_MileageInv m_BringITEM[0];
#else
...
#define PLAY_FLAG_EUROPE_DEFAULT  PLAY_FLAG_KOREA_DEFAULT
...
//-------------------------------------------------------------------------------------------------
#ifdef	__KCHS_BATTLECART__
struct gsv_PATSTATE_CHANGE : public t_PACKETHEADER { // GSV_PATSTATE_CHAGE
...
struct gsv_cli_CLAN_WAR_OK : public t_PACKETHEADER {
	DWORD	m_Sub_Type;	// 부가 옵션 (0:오류,1:클랜마스터 진행확인, 2:클랜원 참여. 100 : 포키.... 오류코드 HIWORD 값 사용.. 미정)
	DWORD	m_Clan_N;	// 클랜 번호
	WORD	m_Zone_N;	// 존에 대한 배열 번호 (1부터 차래로..)
	WORD	m_Team_N;	// 팀 번호 내용.	(A팀 = 1000,B팀 = 2000)
};

//	클랜전 타임 패킷...
struct gsv_cli_CLAN_WAR_TIME : public t_PACKETHEADER {
	DWORD	m_Play_Type;	// 1: 게임 진행 시간, 2:게임 종료 대기 시간
	DWORD	m_Play_Count;	// 남은 카운트.. 1이면 10초, 120 이면 20분이다...
};

//	클랜전 오류 메시지 패킷
struct gsv_cli_CLAN_WAR_ERR : public t_PACKETHEADER {
	WORD	m_CW_Err_w;		// 클랜전 오류 번호
};

//	클랜전 진행 메시지 패킷
struct gsv_cli_CLAN_WAR_Progress : public t_PACKETHEADER {
	WORD	m_CW_Progress;		// 클랜전 진행번호
	//	1 = 클랜전 시작, 2 = A팀 승리, 3 = B팀 승리, 4 = 시간초과 비김, ....
};

//	클랜전 보스 몹 체력 전송 패킷
struct gsv_cli_CLAN_WAR_BOSS_HP : public t_PACKETHEADER {
	short	m_A_nCharIdx;	// A팀 보스 몹 인덱스
	short	m_B_nCharIdx;	// B팀 보스 몹 인덱스
	int	m_A_Boss_Max_HP;	// A 팀 보스 몹의 최대 체력
	int	m_A_Boss_HP;		// A 팀 보스 몹의 현재 체력
	int	m_B_Boss_Max_HP;	// B 팀 보스 몹의 최대 체력
	int	m_B_Boss_HP;		// B 팀 보스 몹의 현재 체력
	//	m_B_Boss_HP = 0 또는 m_B_Boss_Max_HP = 0 이면 해당 몹은 죽은 것으로 표기 한다.
};

// (20060912:남병철) : 시스템 워닝 코드
enum EGSV_WARNING_MSGCODE		// m_dwWarningCode에 적용
{
	EGSV_PARTY_EXP_LIMIT = 0,						// 파티 경험치 획득 제한 경고 메시지 (ex. "파티 구성원의 레벨차이가 큽니다.")
	EGSV_DONT_DUPLICATE_EXPERIENCE_MILEAGE_ITEM_USE,// 마일리지 아이템 중복사용 경고 (ex. "아이템을 중복 사용할 수 없습니다.")
	EGSV_EXPERIENCE_MILEAGE_ITEM_EXPIRED			// 경험치 마일리지 아이템 만료 (ex. "경험치 아이템이 만료 되었습니다.")
};
...
// (20061204:남병철) : 마일리지 아이템 시간 요청 특정 캐릭터의 특정 마일리지 아이템 적용 시간 요청
#ifdef __EXPERIENCE_ITEM
struct cli_MILEAGE_ITEM_TIME : public t_PACKETHEADER
{
	DWORD	dwIndex;			// 마일리지 아이템 인덱스(종류) (ex. 0~2:창고무료,확장,모양 / 3:카트게이지 / 4:경험치2배)
	DWORD	dwCharIndex;		// 캐릭터 인덱스	
};
...
struct gsv_cli_USER_MSG_1LV : public t_PACKETHEADER {
	WORD	m_Sub_Type; // 1: 유저 죽었다.	100: 클랜전 존 GATE 처리.
	WORD	m_Data;
	/*
		유저에게 m_Sub_Type = 100 값이 전달되면 맵에 충돌 NPC on/off 처리함.
		m_Data = 0 값이면 충돌 NPC On(충돌 처리)
		m_Data = 1 값이면 충돌 NPC Off(충돌 처리 않함)
	*/
};
...
struct t_PACKET
{
	union
	{
		...
		//	클랜전 관련 추가. 김영환 --------------------------------------
		gsv_cli_CLAN_WAR_OK			m_gsv_cli_ClanWar_OK;	// 클랜전 참가 확인. 메시지.
		gsv_cli_CLAN_WAR_TIME		m_gsv_cli_ClanWar_Time;	// 타임 카운트
		gsv_cli_CLAN_WAR_ERR		m_gsv_cli_ClanWar_Err;	// 클랜전 오류
		gsv_cli_CLAN_WAR_Progress	m_gsv_cli_ClanWar_Progress;	//	클랜전 진행. 메시지 타입
		gsv_cli_CLAN_WAR_BOSS_HP	m_gsv_cli_ClanWar_BOSS_HP;	//	클랜전 보스 몹 HP 전송 구조체
		...
		// 명령
		srv_DBG_CMD m_sv_Debug_Command;

		// (20060912:남병철) : 워닝 메시지 코드 전달 헤더 (EX. 파티 경험치 획득 제한)
		// GS서버에 보내서 관련된 클라이언트에게 메세지 브로드캐스팅
		// (EX. 파티 경험치 획득 제한 : 파티 내부의 경험치 공유 가능한 유저에게 브로드캐스팅)
...
//-------------------------------------
		...
		*/
		...
		cli_TRADE_CHAT     m_cli_TRADE_CHAT;
		gsv_TRADE_CHAT  m_gsv_TRADE_CHAT;
		...
		cli_QUEST_REQ_EX			m_cli_QUEST_REQ_EX;
...
//		gsv_SUB_EVENTOBJ			m_gsv_SUB_EVENTOBJ;
		...
		cli_CRAFT_DRILL_SOCKET_REQ  m_cli_CRAFT_DRILL_SOKCET_REQ;   //소켓 생성 요청...
		...
		gsv_CHAR_HPMP_INFO  m_gsv_CHAR_HPMP_INFO;

#ifdef __EXPERIENCE_ITEM
		cli_MILEAGE_ITEM_TIME		m_cli_MILEAGE_ITEM_TIME;
		gsv_MILEAGE_ITEM_TIME		m_gsv_MILEAGE_ITEM_TIME;
#endif
		...
		//마일리지 아이템 관련... 새로추가된..  패킷타입......
		gsv_MILEAGE_INV_DATA				m_gsv_MILEAGE_INV_DATA;
		gsv_DESTROY_MILEAGE_ITEM	m_gsv_DESTROY_MILEAGE_ITEM;  //기간 만료로 파괴될 아이템
		gsv_NOTIFY_MILEAGE_ITEM		m_gsv_NOTIFY_MILEAGE_ITEM;        //아이템 정보 변동된 아이템 ..
		gsv_SET_MILEAGE_INV					m_gsv_SET_MILEAGE_INV;							  //변경된 마일리지 인벤정보.

		cli_EQUIP_MILEAGE_ITEM			m_cli_EQUIP_MILEAGE_ITEM;
		gsv_EQUIP_MILEAGE_ITEM			m_gsv_EQUIP_MILEAGE_ITEM;

		cli_USE_MILEAGE_ITEM					m_cli_USE_MILEAGE_ITEM;							//마일리지 아이템 사용
...
// 2007.02.13/김대성/추가 - 클라이언트와 서버시간 동기화
#ifdef __ITEM_TIME_LIMIT
		cli_SERVER_TIME				m_cli_SERVER_TIME;
