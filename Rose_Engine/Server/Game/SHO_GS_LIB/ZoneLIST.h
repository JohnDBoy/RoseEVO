...
//	tagEVENTPOS *Get_NearstEventPOS( short nZoneNO, char *szPosName, tPOINTF PosCUR );
    ...
    BOOL Get_PK_FLAG(short nZoneNO);
	////	해당 존에 모든 유저에게 지정 Packet 전송 : 김영환 작성
	BOOL Send_gsv_All_User_Data(short nZoneNO,char *p_Data);

	bool Do_QuestTrigger( short nZoneNO, short nTeamNO, t_HASHKEY HashKEY );

	////	해당 존의 모든 몹을 죽인다. : 작성자 김영환
	void Zone_MOB_Kill(short nZoneNO,classUSER *pUSER,DWORD	p_Team_N);
	////	해당 존의 몬스터의 채력을 회복 시킨다.  : 작성자 김영환
	BOOL Zone_MOB_HP_Reset(short nZoneNO,DWORD p_Team_N);

	////	해당 존의 팀 유저 갯수 얻기 : 김영환
	DWORD	Get_Zone_Team_User(short nZoneNO,DWORD p_Team_N);
