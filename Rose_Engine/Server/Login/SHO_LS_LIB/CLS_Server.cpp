...
bool CLS_Server::Recv_zws_SERVER_INFO (t_PACKET *pPacket)
{
	...
	//////////////////////////////////////////////////////////////////////////
	// 2006.06.09:이성활: szServerIP에 대한 Null 체크 추가
	//if ( *szServerIP )
...
*/
// 2006.11.28/김대성/수정 - 패킷이 잘못되었을경우 szAccount 에 쓰레기값...
		char buf[35];
		strncpy(buf, szAccount, sizeof(buf));
		buf[sizeof(buf)-1] = '\0';
		LogString( 0xffff, "Recv_gsv_SUB_ACCOUNT:: Account[ %s ] not found in JOIN list\n", buf );
//-------------------------------------
...
*/
	...
	TCHAR szABC[ MAX_PATH ] = {0};
    switch ( pPacket->m_wType ) {
		case SRV_ACTIVE_MODE :
			_stprintf( szABC, "Handling Packet : SRV_ACTIVE_MODE, size %d", pPacket->m_nSize );
