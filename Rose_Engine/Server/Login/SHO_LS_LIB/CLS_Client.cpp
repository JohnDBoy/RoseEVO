...
CLS_Client::~CLS_Client ()
{
	//g_LOG.CS_ODS (0xffff, "~CLS_Client()\n");
...
bool CLS_Client::Send_lsv_LOGIN_REPLY(BYTE btResult, int iPayType)
{
	...
	pCPacket->m_srv_LOGIN_REPLY.m_wRight   = this->m_dwRIGHT>=0x0ffff ? 0x0ffff : (WORD)this->m_dwRIGHT;
...
//-------------------------------------
// 2007.03.12/김대성/추가 - 일본 웹 ActiveX 연동
bool CLS_Client::Send_lsv_LOGIN_REPLY2(BYTE *pszAccount, BYTE *pMD5PW)
{
	classPACKET *pCPacket = Packet_AllocNLock ();
	if ( !pCPacket )
		return false;

	pCPacket->m_HEADER.m_wType = LSV_LOGIN_REPLY2;
	pCPacket->m_HEADER.m_nSize = sizeof( srv_LOGIN_REPLY2 );

	memcpy(pCPacket->m_srv_LOGIN_REPLY2.sztxtAccount, pszAccount, sizeof(pCPacket->m_srv_LOGIN_REPLY2.sztxtAccount));
	memcpy(pCPacket->m_srv_LOGIN_REPLY2.szMD5PW, pMD5PW, sizeof(pCPacket->m_srv_LOGIN_REPLY2.szMD5PW));
...
//-------------------------------------
// 2007.03.12/김대성/추가 - 일본 웹 ActiveX 연동
bool CLS_Client::Recv_cli_LOGIN_REQ2 ( t_PACKET *pPacket )
{
	if(g_pThreadSQL_ActiveX != NULL)
		return g_pThreadSQL_ActiveX->Add_SqlPACKET( this->m_iSocketIDX, NULL, pPacket );
	else
		return false;
}
//-------------------------------------
...
_PASS :
			...
			if ( pCAccount )
			{
				...
				//영국일 경우에만 값이 존재한다.. 그러나 걍 복사해준다..
				::CopyMemory(&pCAccount->m_timeStartTime,&m_timeStartTime,sizeof(sqlTIMESTAMP));   // 과금 시작시간
				::CopyMemory(&pCAccount->m_timeStartTime,&m_timeStartTime,sizeof(sqlTIMESTAMP));  //  과금 만료시간..
...
// 2006.03.24/김대성/추가 - Arcfour 암호화 모듈
	char *p = (char *)pPacket;
//	m_cipher.decrypt(p+sizeof(pPacket->m_nSize), pPacket->m_nSize - sizeof(pPacket->m_nSize));
	for(int i=2; i < (pPacket->m_nSize - 2); i++)		p[i] = p[i] ^ 'a';

	// 정상적인 패킷인지 체크
	//if(pPacket->m_wReserved != 603)		// 곽홍근씨 결혼식날 6월 3일
	if(pPacket->m_wReserved != __PACKET_HEADER_VERSION)		// 해킹때문에 수정
	{
		g_LOG.CS_ODS (0xffff, "** m_cipher.decrypt() - Invalid packet header: Server(%d), Client(%d), type(0x%x), Size(%d) ", __PACKET_HEADER_VERSION, pPacket->m_wReserved, pPacket->m_wType, pPacket->m_nSize);
		return false;
	}
//-------------------------------------
...
// 2006.03.29/김대서/추가 - 암호화를 위해 클라이언트가 임의의 크기의 더미패킷을 보낸다.
	if ( CLI_DUMMY_1 == pPacket->m_wType ) 
	{
		return true; 
	}
//-------------------------------------
...
// 2007.03.12/김대성/추가 - 일본 웹 ActiveX 연동
				case CLI_LOGIN_REQ2 :
					if ( Recv_cli_LOGIN_REQ2( (t_PACKET*)pPacket ) ) {
						m_nProcSTEP = CLIENT_STEP_LOGIN_REQ;
						return true;
					}
					return false;
...
//-------------------------------------
// 2007.03.12/김대성/추가 - 일본 웹 ActiveX 연동
bool CLS_ListCLIENT::Send_lsv_LOGIN_REPLY2 (int iSocketIDX, BYTE *pszAccount, BYTE *pMD5PW)
{
	CLS_Client *pClient = (CLS_Client *)this->GetSOCKET( iSocketIDX );
	if ( NULL == pClient )
		return false;

	return pClient->Send_lsv_LOGIN_REPLY2( pszAccount, pMD5PW );
}
//-------------------------------------
