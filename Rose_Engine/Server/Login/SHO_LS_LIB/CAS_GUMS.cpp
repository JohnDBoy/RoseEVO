...
#ifdef __NORTHUSA
				// 미국 iPayType은 PLAY_FLAG와 같다 !!!
				// (20070719:남병철) : 미국 설정값을 일본과 동일하게 수정
				//dwPlayFlag |= PLAY_FLAG_EXTRA_CHAR; // 수정하기전 미국 설정
				dwPlayFlag = iPayType|PAY_FLAG_JP_BATTLE|PAY_FLAG_JP_DUNGEON_ADV;   // 2.Entry, 8.Dungeon Adventure 무료화
				break;					
#endif
...
#ifdef	__PHILIPPINE				// 필리핀은 과거 버그로인해 요금체계와는 상관없이 엑스트라 캐릭터를 사용할수 있다.||
									// 한가지 요금체계이며 플래티넘 창고는 사용하지 못한다. 2006.06.13
									// 검서버는 한국과 동일하다.
				default:
					dwPlayFlag |= PLAY_FLAG_EXTRA_CHAR;
#endif

// (20070719:남병철) : 미국을 일본과 동일하게 맞추기전 백업
//#ifdef __NORTHUSA
//				default:
//					dwPlayFlag |= PLAY_FLAG_EXTRA_CHAR; // 수정하기전 미국 설정
//					
...
//-------------------------------------
/*
			dwPlayFlag = iPayType|PLAY_FLAG_DUNGEON_ADV;   //일본에서 임시로 던젼 어드밴스를 준다..(허재영 2005.11.25)
*/
// 2006.03.17/김대성
			dwPlayFlag = iPayType|PAY_FLAG_JP_BATTLE|PAY_FLAG_JP_DUNGEON_ADV;   // 2.Entry, 8.Dungeon Adventure 무료화
//-------------------------------------
...
//-------------------------------------------------------------------------------------------------
	...
	{
		int iPayType_Old = iPayType;
		int iSocketIDX = atoi( szUserTAG );
	    CLS_Client *pClient = (CLS_Client *)g_pListCLIENT->GetSOCKET( iSocketIDX );
	    if ( pClient && !_strcmpi( szAccount, pClient->m_Account.Get() ) ) {
			BYTE btType;
			switch( eNation ) {
				case GUMS_KOREA :	// 한국은...
					// (20070719:남병철) : 미국일 경우 일본과 동일하게 세팅
#ifdef __NORTHUSA
...
//-------------------------------------
// 2006.05.11/김대성/추가 - iPayType 을 Gums 안붙이고 처리하는 루틴인 Proc_KOR_TEST_LOGIN_REQ() 에서 사용하는 방식으로 세팅
					if(iPayType == BILLING_MSG_PAY_FAP)	// FAP	로그인 된 사용자는 개인정액 사용자(플레티넘)임을 나타낸다.
						iPayType = PLAY_FLAG_KOREA_DEFAULT | PLAY_FLAG_EXTRA_STOCK | PLAY_FLAG_EXTRA_CHAR;
					else
						iPayType = PLAY_FLAG_KOREA_DEFAULT;
//-------------------------------------
					break;
				case GUMS_JAPAN :	// 일본은...
					btType = RESULT_LOGIN_REPLY_JAPAN_OK;
...
*/
// 2006.05.16/김대성/수정 - 
			pClient->m_dwPayFLAG = this->GetPlayingFLAG( eNation, iPayType_Old );
//-------------------------------------
...
//	m_csHEAD.Unlock ();
	...
	if(iMsgType== GUMS110_REQ_PRELOGIN_TYPE||iMsgType ==GUMS010_REQ_LOGIN||
	 iMsgType == GUMS020_REQ_LOGOUT||iMsgType ==GUMS040_REQ_REMAIN_TIME)
	{
	#ifdef	__SHO_LS_LIB
		g_LOG.CS_ODS( LOG_DEBUG, "LS SEND:: %s\n", (char*)&pCPacket->m_pDATA[ sizeof(t_PACKETHEADER) ] );
	#else
		g_LOG.CS_ODS( LOG_DEBUG, "AS SEND:: %s\n",  (char*)&pCPacket->m_pDATA[ sizeof(t_PACKETHEADER) ] );
	#endif
...
void AS_gumSOCKET::Recv_GumsPACKET (char *szString, int iStrLen)
{
	...
	if ( GUMS100_REPLY_INIT_SUCCESS == iMsgTYPE ) {
		...
		g_LOG.CS_ODS( LOG_DEBUG, "Recved GUM Intialized MSG[ Type:%d,  %s ]\n", iMsgTYPE, szString );
#ifndef	__SHO_LS_LIB
	...
	switch( iMsgTYPE ) {
		...
		{
			if ( pLast == NULL ) {
				g_LOG.CS_ODS( LOG_DEBUG, "ERROR in GUMS  :: Type:%d, PayType == NULL[ %s ]\n", iMsgTYPE, szString );
				return;
			}
			eGumsNATION eNation = Gums_NATION( iMsgTYPE );
			switch( eNation ) {
				case GUMS_JAPAN :		// JAPAN !!! 
				{
					...
					g_LOG.CS_ODS( LOG_DEBUG, " Recv GUMS %d LoginSUC :: ID:%s  PayFlag:%d\n", iMsgTYPE,pUserID,dwPayFlag );
		...
		case	GUMS1112_REPLY_FREE_SRV_ONLY		:
			...
			g_LOG.CS_ODS( LOG_DEBUG, " Recv GUMS %d LoginOnlyFREE SUC :: ID:%s \n", iMsgTYPE, pUserID );
			return;

		// Response Message type 2	:: pLast == NULL
		case 	GUMS012_REPLY_LOGIN_FAILED_INVALID_ID_OR_PW		:	// 사용자의 ID나 PW가 틀린경우
			g_LOG.CS_ODS( LOG_DEBUG, "%d invalid id or password :: %s \n", iMsgTYPE, szString );
		...
		case 	GUMS042_REPLY_EXPIRE_DAY2			:	// 종량 사용자의 남은 시간을 돌려준다.
		{
			...
			g_LOG.CS_ODS( 0xffff, " Recv GUMS %d _REPLY_EXPIRE_DAY:: %s, %s\n", iMsgTYPE, pUserID, pLast );
		...
		case	GUMS1064_REPLY_DUP_USERID_DIS		:
			g_LOG.CS_ODS( LOG_DEBUG, "%d time expired or duplicated ID :: %s \n", iMsgTYPE, szString );
		...
		case	GUMS1065_REPLY_USER_NOT_FOUND		:
			...
			if ( this->RecvGums_TimeOUT( iMsgTYPE, pUserID ) /* g_pListACCOUNT->RecvGums_TimeOUT( pUserID ) */ ) {
				...
				g_LOG.CS_ODS( LOG_DEBUG, "TimeOut:: %d user not found:: %s \n", iMsgTYPE, szString );
			return;
#endif
	}
...
/*
#ifdef	__SHO_LS_LIB
					g_LOG.CS_ODS( LOG_DEBUG, "LS RECV:: %s\n", szString );
#else
					g_LOG.CS_ODS( LOG_DEBUG, "AS RECV:: %s\n", szString );
#endif
*/
