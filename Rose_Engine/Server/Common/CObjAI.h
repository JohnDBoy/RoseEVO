/*
	$Header: /7HeartsOnline/Server/SHO_GS/Sho_gs_lib/Common/CObjAI.h 31    05-10-31 6:31p Young $
*/
#ifndef	__CCOMMAND_H
#define	__CCOMMAND_H
#include "CGameOBJ.h"
...
//	virtual bool		SetCMD_TOGGLE (BYTE btTYPE);
	...
	bool				Is_Attack() {  if(m_wState == CS_ATTACK)  return true; return false;} //현재상태가 공격중이다.
	bool                Is_Stop()    {  if(m_wState == CS_STOP)      return true; return false;} //현재상태가 아무것도 안하고 있는상태이다.
...
*/
// 2006.03.14/김대성 - 이벤트몹의 최대 HP를 13억 정도로 세팅했더니 AI의 조건식(자신의 HP 체크)이 제대로 체크가 안됨
		return ( (int)((double)this->Get_HP() * 100 / (double)this->Get_MaxHP()) );
//-------------------------------------
	}
