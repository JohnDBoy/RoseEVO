...
bool BCBUtil::LoadControl (char *szIniFileName, TWinControl *pParentCTRL)
{
    ...
    pProperty = (tagCtrlDATA*) new BYTE[ 2048 ];
    ...
    for (int iL=0; iL<pList->Count; iL++) {
        ...
        /*
        if ( stClassName == "TVirtualStringTree" ) {
            TVirtualStringTree *pTVST = (TVirtualStringTree*)pCTRL;
        ...
        */
        ...
        if ( stClassName == "TCheckBox" ){
            TCheckBox *pTCheckBox = (TCheckBox*)pCTRL;
            if ( ::GetPrivateProfileString (
                            pParentCTRL->Name.c_str(),
                            pCTRL->Name.c_str(),
                            "0",
                            ...
                            MAX_PATH,
                            szIniFileName ) ) {
                pTCheckBox->Checked = atoi( pProperty->szText );
            }
        } else
        if ( stClassName == "TComboBox" ) {
            TComboBox *pTComboBox = (TComboBox*)pCTRL;
            if ( ::GetPrivateProfileString (
                            pParentCTRL->Name.c_str(),
                            pCTRL->Name.c_str(),
                            "0",
                ...
                for (int iIdx=0; iIdx<iItemCnt; iIdx++) {
                    ...
                    if ( ::GetPrivateProfileString (
                                ...
                                MAX_PATH,
                                szIniFileName ) ) {
                        pTComboBox->Items->Add( pProperty->szText );
                    }
                }
                stKeyName = "selIdx_" + pCTRL->Name;
                ::GetPrivateProfileString (
                                ...
                                MAX_PATH,
                                szIniFileName );
                pTComboBox->ItemIndex = StrToInt( pProperty->szText );
            }
        } else
    ...
    for (int iL=0; iL<pList->Count; iL++) {
        ...
        */
        ...
        if ( stClassName == "TCheckBox" ) {
            ...
            String stChecked = IntToStr( pTCheckBox->Checked );
            ::WritePrivateProfileString (
                            pParentCTRL->Name.c_str(),
                            pCTRL->Name.c_str(),
                            stChecked.c_str(),
        ...
        if ( stClassName == "TComboBox" ) {
            ...
            String stCNT = IntToStr( pTComboBox->Items->Count );
            // write item count
            ::WritePrivateProfileString (
                            pParentCTRL->Name.c_str(),
                            pCTRL->Name.c_str(),
            ...
            String stKeyName;
            for (int iIdx=0; iIdx<pTComboBox->Items->Count; iIdx++) {
                stKeyName = "item_" + IntToStr(iIdx) + pCTRL->Name;
                ::WritePrivateProfileString (
                                pParentCTRL->Name.c_str(),
                                stKeyName.c_str(),
                                pTComboBox->Items->Strings[ iIdx ].c_str(),
                                szIniFileName );
            }
            stCNT = IntToStr( pTComboBox->ItemIndex );
            stKeyName = "selIdx_" + pCTRL->Name;
            ::WritePrivateProfileString (
                            pParentCTRL->Name.c_str(),
                            stKeyName.c_str(),
...
//---------------------------------------------------------------------------
bool BCBUtil::IsRegValueExist (char *KeyName, char *StrName )
{
    bool bExist = false;
    ...
    try
    {
        ...
        if ( Registry->OpenKey (KeyName, false) ) {
            bExist = Registry->ValueExists ( StrName );
            Registry->CloseKey ();
        }
    }
...
//---------------------------------------------------------------------------
int BCBUtil::GetRegistryINT (char *KeyName, char *StrName, bool bCreate, int iDefaultValue)
{
...
//---------------------------------------------------------------------------
/*
DWORD BCBUtil::FindProcess ( char *szExeName )
{
    HANDLE hSnapshot;
...
*/
/*
BOOL BCBUtil::KillProcess ( DWORD dwPID )
{
    HANDLE hProcess;
