...
/*
					if ( pBE->m_btCharSlotNO )
						nPlatinumCharCNT++;
*/
// 2006.06.19/김대성/수정 - 에볼루션 버전에서 Extra 캐릭터구분하기 위해
...
*/
// 2006.08.09/김대성/수정 - 한국에보 서버통합과정에서 플래티넘 캐릭터정보의 구별을 ASP에서 처리하기가 어려워 별도의 컬럼을 추가
...
/*
		this->m_pSQL->MakeQuery("INSERT tblGS_AVATAR (txtACCOUNT, txtNAME, binBasicE, binBasicI, binBasicA, binGrowA,  blobINV ) VALUES(",
			...
			MQ_PARAM_ADDSTR, ");"	,	MQ_PARAM_END);
*/
// 2006.06.012/김대성/수정 - DATA_VER_2 때문에 엑스트라캐릭터 프래그값이 잘 맞지 않아서 
//							( 게임서버에서 Proc_KRRE_cli_SELECT_CHAR() 에서 캐릭터정보 읽어들일때 )
#ifdef __KCHS_BATTLECART__ // __OLD_DATA_COMPATIBLE__
...
#pragma message( "한국 일본 버젼은 캐릭터 삭제시간 확인하고 아래 부분 확인 할 것.." )
#pragma message( "한국 일본 버젼은 캐릭터 삭제시간 확인하고 아래 부분 확인 할 것.." )
		if( IsKOREAN() || IsJAPAN() )
