...
#define	SKILL_RELOAD_TYPE(S)				g_SkillList.m_SkillDATA.GetData( S , 27 )		/// 재충전 시간 그룹 번호
#define	SET_SKILL_RELOAD_TYPE(S,N)				g_SkillList.m_SkillDATA.SetData( S , 27,N )		/// 재충전 시간 그룹 번호
...
#define	SKILL_AVAILBLE_CLASS(S,T)			g_SkillList.m_SkillDATA.GetData( S , 1+34+(T) )
*/
...
#define	SET_SKILL_ANI_CASTING_REPEAT_CNT(S,N)		g_SkillList.m_SkillDATA.SetData( S , 55 ,N)
...
#define	SKILL_LEVELUP_NEED_ZULY( I )		g_SkillList.m_SkillDATA.GetData( I , 85 )								//	스킬 레렙업시 소모 비용 / 100

#define	SKILL_AVAILBLE_STATUS( I )			g_SkillList.m_SkillDATA.GetData( I , 86 )								// 스킬 가능 상태
#define	SKILL_ATTRIBUTE( I )				g_SkillList.m_SkillDATA.GetData( I , 87 )								// 스킬 속성
...
#ifdef  __KRRE_NEW_AVATAR_DB                //새로운 디비구조에 따라서 스킬도 변경되었기때문에...
#define	SKILL_INCREASE_ABILITY_CNT			3

#define SKILL_STATE_STB( I,T )				g_SkillList.m_SkillDATA.GetData( I , 88+(T)*4 )	///	상태 번호
#define	SKILL_INCREASE_ABILITY(S,T)			g_SkillList.m_SkillDATA.GetData( S , 89+(T)*4 )
#define	SKILL_INCREASE_ABILITY_VALUE(S,T)	g_SkillList.m_SkillDATA.GetData( S , 90+(T)*4 )
#define	SKILL_CHANGE_ABILITY_RATE(S,T)		g_SkillList.m_SkillDATA.GetData( S , 91+(T)*4 )

#else
#define	SKILL_INCREASE_ABILITY_CNT			2

#define SKILL_STATE_STB( I,C )				g_SkillList.m_SkillDATA.GetData( I , 11+C )	///	상태 번호
#define SKILL_STATE_STB1( I )				g_SkillList.m_SkillDATA.GetData( I , 11 )		///	상태 번호
#define SKILL_STATE_STB2( I )				g_SkillList.m_SkillDATA.GetData( I , 12 )		///	상태 번호
...
#define SKILL_CHILD_SKILL_INDEX1( I,C )		g_SkillList.m_SkillDATA.GetData( I , 101+(C)*2 )		// 하위 스킬
#define SKILL_CHILD_SKILL_LEVEL1( I,C )		g_SkillList.m_SkillDATA.GetData( I , 102+(C)*2 )		// 하위 스킬 레벨
