...

		{
			...
			for (int iC=0; iC<iFriendCNT; iC++) {
				...
				if ( this->m_pSQL->GetNextRECORD() ) {
					...
					if ( _strcmpi(pName, FR.m_Name.Get() ) ) {
	...
	switch( pMsgCMD->m_btCMD ) {
		...
		case 0x0ff :
		{
			...
			if ( pMSGR && FRIEND_STATUS_REFUSE_MSG != pMSGR->m_btMsgrSTATUS )
				pMSGR->MSGR_Msg( pMsgCMD->m_pPacket, this );
			return true;
		}
	}
...
void CMessenger::MSGR_Msg( t_PACKET *pPacket, CThreadMSGR *pMSGR )
{
	CDLList< tagMyFriend >::tagNODE *pNODE;
		...
		if ( pNODE->m_VALUE.m_dwDBID == pPacket->m_cli_MESSENGER_CHAT.m_dwUserTAG ) {
			if ( pNODE->m_VALUE.m_pMSGR ) {
				...
				if ( pCPacket ) {
					...
					// (20061102:남병철) : 1:1 친구 채팅일때는 NULL로 채워서 보냄 (여기서는 보내는 사람 이름 채움)
					memset( pCPacket->m_wsv_MESSENGER_CHAT.m_FromName, 0, 31 );
					strcpy( pCPacket->m_wsv_MESSENGER_CHAT.m_FromName, m_Name.Get() );
#endif
...
//					ZString zLine;
//
//					zLine.Format( "dwDBID : %d\nFromName : %s\nm_Name : %s\nszMSG : %s\n", pCPacket->m_wsv_MESSENGER_CHAT.m_dwUserTAG, 
//						pCPacket->m_wsv_MESSENGER_CHAT.m_FromName,
//						m_Name.Get(),
//						pCPacket->m_wsv_MESSENGER_CHAT.m_szMSG );
//					//::MessageBox( NULL, zLine.c_str(), "MSG", MB_OK );
//#endif
			...
			{
				// (20061110:남병철) : 친구이며 오프라인 상태 일때는 1:1 메시지 전달
				break;
			}
#endif
			return;
...
#ifdef __COMMUNITY_1_1_CHAT
	// (20061101:남병철) : 친구가 아니어도 1:1 채팅
	if( pMSGR != NULL )
	{
		CMessenger* pMsgr = NULL;

		pMsgr = pMSGR->SearchMSGR( pPacket->m_cli_MESSENGER_CHAT.m_ToName );
		...
		if( pMsgr != NULL )
		{
			...
			if ( pCPacket )
			{
				...
				this->SendPacket( pMsgr->m_iOwnerSocetIDX, pMsgr->m_dwDBID, pCPacket );
