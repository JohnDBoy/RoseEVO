...
bool CWS_Server::Send_str_PACKET (WORD wType, char *szString)
{
	...
	if ( !pCPacket ) {
        g_LOG.CS_ODS (0xffff, "CWS_Server(%s:%s)::Send_str_PACKET(), Paclet allock falied... \n", this->m_ServerNAME.Get(), this->m_ServerIP.Get() );
		return false;
    }
...
bool CWS_Server::Send_wls_CONFIRM_ACCOUNT_REPLY (BYTE btResult, DWORD dwWSID, DWORD dwGSID, CWS_Client *pCLIENT)// DWORD dwLoginTIME, DWORD dwRight, char *szAccount, char *szChar, bool bFirstZONE, DWORD dwLSID )
{
	classPACKET *pCPacket = Packet_AllocNLock ();
	if ( !pCPacket ) {
        g_LOG.CS_ODS (0xffff, "CWS_Server(%s:%s)::Send_lsv_ADD_ACCOUNT_REPLY(), Paclet allock falied... \n", this->m_ServerNAME.Get(), this->m_ServerIP.Get() );
		return false;
    }
	...
	this->LockLIST ();
	{
		for (short nI=1; nI<MAX_CHANNEL_SERVER;nI++) {
			if ( m_ppChannelSERVER[nI] && m_ppChannelSERVER[nI]->m_bActive ) {
				...
				//////////////////////////////////////////////////////////////////////////
				// 2006.06.12:이성활:AppendString() 실패시 for loop 빠져나감, 위 AppendData는 실패해도 
				// 일단 넘어가고 마지막 AppendString에서 체크
				// pCPacket->AppendString( m_ppChannelSERVER[nI]->m_ServerNAME.Get() );
				if (!pCPacket->AppendString( m_ppChannelSERVER[nI]->m_ServerNAME.Get() ))
					break;
				//////////////////////////////////////////////////////////////////////////
	...
	this->LockLIST ();
	{
		...
		for (short nI=1; nI<MAX_CHANNEL_SERVER;nI++) {
			if ( m_ppChannelSERVER[nI] ) {
				...
				pCPacket->AppendString( m_ppChannelSERVER[nI]->m_ServerNAME.Get() );
				//////////////////////////////////////////////////////////////////////////
				// 2006.06.12:이성활:AppendString() 실패시 for loop 빠져나감, 위 AppendData, AppendString는 
				// 실패해도 일단 넘어가고 마지막 AppendString에서 체크
				// pCPacket->AppendString( m_ppChannelSERVER[nI]->m_ServerIP.Get() );
				if (!pCPacket->AppendString( m_ppChannelSERVER[nI]->m_ServerIP.Get() ))
					break;
				//////////////////////////////////////////////////////////////////////////
