...
CClientStorage::CClientStorage(void)
{
	...
	m_iChatDlgType			= CChatDLG::CHATSIZE_MODE_MIDDLE;
}
...
void CClientStorage::Load()
{
	...
	m_iChatDlgType =  GetPrivateProfileInt("DIALOG","CHATDLGTYPE", 1, g_szIniFileName );
	...
	m_VideoOption.iAntiAliasing = GetPrivateProfileInt("VIDEO","ANTIALIASING",g_iDefaultAntiAliasing,g_szIniFileName );
	...
	m_SoundOption.iSoundOff = GetPrivateProfileInt("SOUND", "SOUNDOFF", 0, g_szIniFileName );
	...
	//홍근 NPC이름 안나오는 부분 수정.
	//컨트롤에 체크박스를 지워서 옵션 세팅 과정에서 잘못 세팅 되는듯.
	//여기서 강제로 Default로 세팅한다.
	m_PlayOption.iShowPcName	= c_iDefaultShowName;
	m_PlayOption.iShowNpcName	= c_iDefaultShowName;
	m_PlayOption.iShowMobName	= c_iDefaultShowName;
	...
	m_CommunityOption.iAutoPickup= GetPrivateProfileInt("COMMUNITY","AUTOPICKUP",1,g_szIniFileName );
	...
	m_ShortCutOption.iShortCutView = GetPrivateProfileInt( "PLAY", "SHORTCUTVIEW", 1, g_szIniFileName );
}
...
void CClientStorage::Save()
{
	...
	_itoa( m_VideoOption.iAntiAliasing, szTemp, 10 );
	WritePrivateProfileString("VIDEO","ANTIALIASING", szTemp,g_szIniFileName );
	...
	_itoa( m_SoundOption.iSoundOff, szTemp, 10 );
	WritePrivateProfileString("SOUND", "SOUNDOFF", szTemp,g_szIniFileName );
	...
	_itoa( m_CommunityOption.iAutoPickup, szTemp, 10 );
	WritePrivateProfileString("COMMUNITY","AUTOPICKUP", szTemp, g_szIniFileName );
...
//	WritePrivateProfileStruct(m_strAvatarName.c_str(),"INVENTORY", m_VirtualInventory, sizeof( m_VirtualInventory ),g_szIniFileName );
	...
	_itoa( m_ShortCutOption.iShortCutView, szTemp, 10 );
	WritePrivateProfileString("PLAY","SHORTCUTVIEW", szTemp, g_szIniFileName );
}
...
void CClientStorage::ApplyCameraOption( short i )
{
	...
	{		
		...
		setCameraPerspective( camera, (float)(CAMERA_FOV(i)), 0.0f, (float)(CAMERA_NEAR_PLANE(i) * 100), (float)(CAMERA_FAR_PLANE(i) * 100));
		g_GameDATA.m_nSightRange = CAMERA_MAX_RANGE(i) / 10;
	}
}

void CClientStorage::ApplyAntiAliasingOption( short i)
{
	...
	if(i == 0)
		...
		useMultiPass ( 1 );
	setFullSceneAntiAliasing( AntiAliasingValues[i]);
}
...
bool CClientStorage::IsAutoPickup()
{
	return (m_CommunityOption.iAutoPickup)?true:false;
}
...
void CClientStorage::SetShortCutOption( t_OptionShortCut & option )
{
	m_ShortCutOption = option;
}

void CClientStorage::GetShortCutOption( t_OptionShortCut & option )
{
	option = m_ShortCutOption;
}