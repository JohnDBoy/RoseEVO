/*
	$Header: /Client/Network/SendPACKET.cpp 12    07-03-20 3:01p Raning $
*/
#include "stdAFX.h"
#include "CNetwork.h"
...
void CSendPACKET::Send_cli_LOGIN_REQ2( DWORD dwSeq, char * szSessionKey )
{	
	m_pSendPacket->m_HEADER.m_wType = CLI_LOGIN_REQ2;
	m_pSendPacket->m_HEADER.m_nSize = sizeof( cli_LOGIN_REQ2 );

	m_pSendPacket->m_cli_LOGIN_REQ2.m_dwSeq = dwSeq;
	::CopyMemory( m_pSendPacket->m_cli_LOGIN_REQ2.szSessionKey, szSessionKey, 33);

	ZeroMemory(m_pMD5Buff, sizeof(m_pMD5Buff));	
...
bool CSendPACKET::Send_cli_GET_FIELDITEM_REQ( CGameOBJ *pUSER, int iServerObject )
{
	...
	if(pUSER->Get_TYPE() == OBJ_CART||pUSER->Get_TYPE() ==OBJ_CGEAR||pUSER->Get_TYPE() ==OBJ_NRIDE) //카트이거나 캐슬기어일경우
	{
...
bool CSendPACKET::Send_cli_GET_FIELDITEM_REQ( WORD wServerObject )
{
	m_pSendPacket->m_HEADER.m_wType = CLI_GET_FIELDITEM_REQ;
	m_pSendPacket->m_HEADER.m_nSize = sizeof( cli_GET_FIELDITEM_REQ );

	m_pSendPacket->m_cli_GET_FIELDITEM_REQ.m_wServerItemIDX  = wServerObject;
	this->Send_PACKET(m_pSendPacket);
	//g_CommandFilter.SetPrevCommand( NULL );
	return true;
}
...
//-------------------------------------------------------------------------------------------------
void CSendPACKET::Send_cli_SKILL_LEVELUP_REQ (WORD wSkillSLOT, short nNextLevelSkillIDX)
{
	short nCurLevelSkillIDX = g_pAVATAR->m_Skills.m_nSkillINDEX[ wSkillSLOT ];
	...
	m_pSendPacket->m_cli_SKILL_LEVELUP_REQ.m_wSkillSLOT = wSkillSLOT;					/// 현재 레벨업을 할 스킬이 있는 슬롯
...
//-------------------------------------------------------------------------------------------------
void CSendPACKET::Send_cli_SELF_SKILL ( WORD wSkillSLOT )
{
	/*
	...
	*/
	...
	m_pSendPacket->m_gsv_SELF_SKILL.m_nSkillIDX   = g_pAVATAR->m_Skills.m_nSkillINDEX[ wSkillSLOT ];
#else
	if ( g_pNet->m_bWarping )
		return;
	...
	/// 이전명령 필터링...
	if( !g_CommandFilter.CanSendSelfSkillCommand( wSkillSLOT ) )
		return;
	...
	/// 스킬 연타 방지..
	//--------------------------------------------------------------------------------------
	if( CPreventDuplicatedCommand::GetSingleton().CanSendSelfSkillCommand( wSkillSLOT ) )
	{
		CPreventDuplicatedCommand::GetSingleton().PushSelfSkillCommand( wSkillSLOT );
	}else
		return;
	...
	m_pSendPacket->m_cli_SELF_SKILL.m_wSkillSLOT = wSkillSLOT;
#endif
...
//-------------------------------------------------------------------------------------------------
void CSendPACKET::Send_cli_TARGET_SKILL ( int iClientTarget, WORD wSkillSLOT )
{
	...
	if( !(CGame::GetInstance().GetPayType() & PLAY_FLAG_BATTLE) )
	{
		...
		m_pSendPacket->m_gsv_TARGET_SKILL.m_nSkillIDX   = g_pAVATAR->m_Skills.m_nSkillINDEX[ wSkillSLOT ];
#else
		
		g_CommandFilter.SetPrevCommand( NULL );
		/// 이전명령 필터링...
		if( !g_CommandFilter.CanSendTargetSkillCommand( iClientTarget, wSkillSLOT ) )
			return;
		...
		/// 스킬 연타 방지..
		//--------------------------------------------------------------------------------------
		if( CPreventDuplicatedCommand::GetSingleton().CanSendTargetSkillCommand( iClientTarget, wSkillSLOT ) )
		{
			CPreventDuplicatedCommand::GetSingleton().PushTargetSkillCommand( iClientTarget, wSkillSLOT );
		}else		
			return;
		...
		m_pSendPacket->m_cli_TARGET_SKILL.m_wSkillSLOT	= wSkillSLOT;
#endif
		Log_String( LOG_NORMAL, "Send Target Skill[%d]\n", g_pAVATAR->m_Skills.m_nSkillINDEX[ wSkillSLOT ] );
		this->Send_PACKET( m_pSendPacket );
}

//-------------------------------------------------------------------------------------------------
void CSendPACKET::Send_cli_POSITION_SKILL (D3DVECTOR &PosTO, WORD wSkillSLOT)
{
//홍근 : 일본 빌링 수정
#ifdef __JAPAN_BILL
	if( !(CGame::GetInstance().GetPayType() & PLAY_FLAG_BATTLE) )
	{
		...
		g_CommandFilter.SetPrevCommand( NULL );
		/// 이전명령 필터링...
		if( !g_CommandFilter.CanSendPositionSkillCommand( PosTO, wSkillSLOT ) )
			return;
		...
		/// 스킬 연타 방지..
		//--------------------------------------------------------------------------------------
		if( CPreventDuplicatedCommand::GetSingleton().CanSendPositionSkillCommand( PosTO, wSkillSLOT ) )
		{
			CPreventDuplicatedCommand::GetSingleton().PushPositionSkillCommand( PosTO, wSkillSLOT );
		}else
			return;
		...
		m_pSendPacket->m_cli_POSITION_SKILL.m_wSkillSLOT = wSkillSLOT;
#endif
		this->Send_PACKET( m_pSendPacket );
}
...
void CSendPACKET::Send_cli_CREATE_ITEM_REQ( WORD wSkillSLOT, char cTargetItemTYPE, short nTargetItemNO, short* pnUseItemINV )
{
	...
	m_pSendPacket->m_cli_CREATE_ITEM_REQ.m_wSkillSLOT = wSkillSLOT;
...
void CSendPACKET::Send_cli_MESSENGER_CHAT( DWORD dwUserTag, const char * toName, char* pszMsg )
{
	assert( pszMsg );
	if( pszMsg == NULL ) return;
	...
	strcpy( m_pSendPacket->m_cli_MESSENGER_CHAT.m_ToName, toName );	
	Packet_AppendString( m_pSendPacket, pszMsg );
	this->Send_PACKET( m_pSendPacket, true );
}
#else
...
void CSendPACKET::Send_cli_CRAFT_DRILL_SOCKET_REQ( BYTE btType, short nUseInvSLOTorNpcIDX, BYTE btTargetInvIDX )
{
	m_pSendPacket->m_HEADER.m_wType = CLI_CRAFT_ITEM_REQ;
	m_pSendPacket->m_HEADER.m_nSize = sizeof( cli_CRAFT_DRILL_SOCKET_REQ );

	m_pSendPacket->m_cli_CRAFT_DRILL_SOKCET_REQ.m_btTYPE = btType;
	m_pSendPacket->m_cli_CRAFT_DRILL_SOKCET_REQ.m_nUseInvSLOTorNpcIDX = nUseInvSLOTorNpcIDX;
	m_pSendPacket->m_cli_CRAFT_DRILL_SOKCET_REQ.m_btTargetInvIDX = btTargetInvIDX;
...
//---------------------------------------------------------------------------------------
//  Name  : Send_cli_CLANWAR_JOIN_ACK
//  Desc  : 클랜전에 입장에 대한 응답
//  Author: Antonio - 2006-07-24  오후 12:07:11
//---------------------------------------------------------------------------------------
void CSendPACKET::Send_cli_CLANWAR_JOIN_ACK( gsv_cli_CLAN_WAR_OK *p )
{
	m_pSendPacket->m_HEADER.m_wType = CLI_CLANWAR_IN_OK;
	m_pSendPacket->m_HEADER.m_nSize = sizeof( gsv_cli_CLAN_WAR_OK );
	m_pSendPacket->m_gsv_cli_ClanWar_OK.m_Sub_Type	= p->m_Sub_Type;
	m_pSendPacket->m_gsv_cli_ClanWar_OK.m_Clen_N	= p->m_Clen_N;
	m_pSendPacket->m_gsv_cli_ClanWar_OK.m_Zone_N	= p->m_Zone_N;
	m_pSendPacket->m_gsv_cli_ClanWar_OK.m_Team_N	= p->m_Team_N;
	Send_PACKET( m_pSendPacket );
}

//---------------------------------------------------------------------------------------
//  Name  : Send_cli_CLANWAR_INFO()
//  Desc  : 
//  Author: Antonio - 2006-08-24  오후 8:22:35
//---------------------------------------------------------------------------------------
void CSendPACKET::Send_cli_CLANWAR_INFO( int value )
{
	m_pSendPacket->m_HEADER.m_wType = GSV_CLANWAR_ERR;
	m_pSendPacket->m_HEADER.m_nSize = sizeof( gsv_cli_CLAN_WAR_ERR );
	m_pSendPacket->m_gsv_cli_ClanWar_Err.m_CW_Err_w = value;
	Send_PACKET( m_pSendPacket );
}
...
void  CSendPACKET::Send_cli_DUMMY_1()
{
	int iRandValue		= rand()%100 + 28;	

	int iPacketSize		= iRandValue + sizeof( t_PACKETHEADER );
	
	m_pSendPacket->m_HEADER.m_wType = CLI_DUMMY_1;
	m_pSendPacket->m_HEADER.m_nSize = iPacketSize;
	
	char * szDummy = (char *)g_pCApp;		
	CopyMemory( m_pSendPacket->m_cli_DUMMY_1.m_Dummy, szDummy+rand()%100, iRandValue );
...
// 마일리지 아이템의 남은 시간 요청.
// 마일리지 아이템 인덱스(종류) (ex. 0~2:창고무료,확장,모양 / 3:카트게이지 / 4:경험치2배)
void CSendPACKET::Send_cli_MILEAGE_ITEM_TIME(DWORD dwIndex)
{
	m_pSendPacket->m_HEADER.m_wType = CLI_MILEAGE_ITEM_TIME;
	m_pSendPacket->m_HEADER.m_nSize = sizeof( cli_MILEAGE_ITEM_TIME );

	// 06. 12. 05 - 김주현 : 인덱스는 4부터는 + 43을 해서 계산한다. 서버에서 처리. t_AbilityINDEX 94번부터 참고.
	m_pSendPacket->m_cli_MIELAGE_ITEM_TIME.dwIndex = dwIndex;
	m_pSendPacket->m_cli_MIELAGE_ITEM_TIME.dwCharIndex = g_pObjMGR->Get_ServerObjectIndex(g_pAVATAR->Get_INDEX());
