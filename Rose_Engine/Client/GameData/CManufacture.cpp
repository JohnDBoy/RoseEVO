...
bool CManufacture::LoadMakableClasses( int iSkillNo, int iSkillLv )
{
	...
	case HIGH_BATTLEARMOR_TYPE:
	case HIGH_MAGICARMOR_TYPE:
	case HIGH_NORMALARMOR_TYPE:
		...
		break;
	case POTION_TYPE:
	case HIGH_POTION_TYPE:
		LoadClassesFromSTB( ITEM_TYPE_USE, iMakeNo,iSkillLv );
		break;
	case CART_TYPE:
...
bool CManufacture::LoadMakableItem( int iClass, int iSkillNo, int iSkillLv )
{
	...
	case POTION_TYPE:
	case HIGH_POTION_TYPE:
		LoadItemsFromSTB( ITEM_TYPE_USE, iClass, iMakeNo,iSkillLv );
		break;
...
int CManufacture::LoadGemClassesFromSTB( t_eITEM itemType, int iMakeNo, int iSkillLv )
{
	...
	///일본 이벤트용 보석 추가 -_-;
#if defined(__JAPAN_NEW) || defined(__JAPAN)
...
void CManufacture::SetMakeItem( tagITEM& Item )
{
	...
	{
		if( getItemType( iMaterialItemNo ) )
		{
			m_RequireMaterials[0].SetItemClass( 0 );
			m_RequireMaterials[0].SetItemNo( getItemNo( iMaterialItemNo ) );
			m_RequireMaterials[0].SetItemType( getItemType( iMaterialItemNo ) );
	...
	for( int i = 1; i < g_iMaxCountMaterial; ++i )
	{
		...
		if( iMaterialItemNo > 0 && iMaterialItemCount > 0 )
		{
			if( getItemType( iMaterialItemNo ) )
			{
				...
				m_RequireMaterials[i].SetItemNo( getItemNo( iMaterialItemNo ) );
				m_RequireMaterials[i].SetItemType( getItemType( iMaterialItemNo ) );
...
int CManufacture::IsValidSendMakeItemReq()
{
	...
	//06. 02. 10 - 김주현 : 개인상점일때는 제조할수 없다. stl이 없네;;
