...
public:
	...
	void	SetRankPoint(int iRankPoint);		// 클랜 랭크 포인트.
	int		GetRankPoint();

	void	SetRewardPoint(int m_iRewardPoint );	// 클랜 보상 포인트.
	int		GetRewardPoint();
	...
	bool		IsValidJoinMember(char *pszName);

	int			GetMyClanPoint();
	CClanSkillSlot* GetClanSkillSlot();//					{ return &m_ClanSkillSlot; }

	S_ClanMember* FindClanChar(const char* pszName );  //내 클랜유저 찾기...(허재영 2005.11.14)
...
private:
	...
	int				m_iRankPoint;		// 클랜 랭크 포인트.
	int				m_iRewardPoint;		// 클랜 보상 포인트.
