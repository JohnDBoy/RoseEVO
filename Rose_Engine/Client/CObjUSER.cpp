/*
	$Header: /Client/CObjUSER.cpp 190   06-11-27 12:14p Raning $
*/
#include "stdAFX.h"
#include "Game.h"
#include "CObjUSER.h"
...
{	
	//Cal_RunSPEED() 속도 계산 해서 넣는다.
	//이후에 속도가 변하면 m_nRunSPEED 값을 서버에서 준다.
	//그런데 여기서 또 계산한다. ;;;;
...
#ifdef _DEBUG
	g_itMGR.AppendChatMsg( "총알 소모", IT_MGR::CHAT_TYPE_ALL );
#endif
...
void	CObjUSER::UpdateInventory()
{
	...
	for( int iIndex = 0; iIndex < INVENTORY_TOTAL_SIZE; ++iIndex )
	{
		...
		assert( pItem );
		if( !pItem->IsEmpty()&&pItem->GetTYPE() )
		{
...
void CObjUSER::Set_LEVEL (int iValue)
{
	SetCur_LEVEL (iValue);		
...
//---------------------------------------------------------------------------
/// 조성현 2005 12 - 27 카메라 방향 얻기
//---------------------------------------------------------------------------
float CObjUSER::GetCameraDirection ()
{
	HNODE hCamera;
    float fDirection[3];
    D3DXVECTOR3 vDir, vOrigin, vOut;
	float fAngle;
	
	hCamera = ::getCameraDefault();
	::getCameraDir(hCamera, fDirection);

	vDir.x = fDirection[0]; vDir.y = fDirection[1]; vDir.z = 0.0f;
    vOrigin.x = 0.0f; vOrigin.y = -1.0f; vOrigin.z = 0.0f;
   
    vDir /= D3DXVec3Length(&vDir);
	fAngle = D3DXVec3Dot(&vDir, &vOrigin);
	D3DXVec3Cross(&vOut, &vOrigin, &vDir);

	if( fabsf(vOut.z) < 0.00001f)
	{
		if(vDir.x < 0.0f)
			return 3.141592f;
		else
	...
	else
	{
		if(vOut.z > 0.0f)
			...
			return 2.0f * 3.141592f - acosf(fAngle);
	}
...
float CObjUSER::GetCur_MOVE_SPEED()
{
	return Get_DefaultSPEED();	
}

//기본스텟+아이템+패시브
int	CObjUSER::Get_AbilityValue_StatItemPas( WORD nType )
{
	switch( nType ) 
	{
		...
		case AT_STR		:		return GetDef_STR ();//
		case AT_DEX		:		return GetDef_DEX ();//
		case AT_INT		:		return GetDef_INT ();//
		case AT_CON		:		return GetDef_CON ();//
		case AT_CHARM	:		return GetDef_CHARM ();//
		case AT_SENSE	:		return GetDef_SENSE ();//
		
		case AT_HP		:		return GetCur_HP ();//x
		case AT_MP		:		return GetCur_MP ();//x

		case AT_ATK		:		return GetDef_ATK ();//o
		case AT_DEF		:		return GetDef_DEF ();//o
		case AT_HIT		:		return GetDef_HIT ();//o
		case AT_RES		:		return GetDef_RES ();//o
		case AT_AVOID	:		return GetDef_AVOID ();//o

		case AT_SPEED	:		return (int)GetOri_RunSPEED();//o
		case AT_ATK_SPD :		return GetOri_ATKSPEED();//o

		case AT_WEIGHT	:		return GetCur_WEIGHT();//x
		case AT_CRITICAL   :	return GetDef_CRITICAL();//o

			// case AT_RECOVER_HP :	
			// case AT_RECOVER_MP :	break;

		case AT_EXP		:		return GetCur_EXP ();//x
		case AT_LEVEL	:		return GetCur_LEVEL ();//x
		case AT_BONUSPOINT :	return GetCur_BonusPOINT ();//x
		case AT_SKILLPOINT :	return GetCur_SkillPOINT ();//x
			// case AT_CHAOS	:
			// case AT_PK_LEV	:	break;

		case AT_HEAD_SIZE :		return GetCur_HeadSIZE ();//x
		case AT_BODY_SIZE :		return GetCur_BodySIZE ();//x
		...
		case AT_MAX_HP :		return GetDef_MaxHP ();//o
		case AT_MAX_MP :		return GetDef_MaxMP ();//o
		case AT_STAMINA :		return m_GrowAbility.m_nSTAMINA;//x
		case AT_PATHP :			return GetCur_PatHP();//x
		default:
			{
				if( nType >= AT_UNION_POINT1 && nType <= AT_UNION_POINT10 )
					return GetCur_UnionPOINT( nType );
				break;
			}
	}
