...
void CUserDATA::Cal_BattleAbility ()
{
	...
	// 아이템에 붙은 추가 능력치 계산	
	InitPassiveSkill(); // 패시브 중복 수정.
	
	Cal_AddAbility ();		// 항상 앞서도록 !!!

	int iiii = this->m_iAddValue[29];
	...
	// 2차 전직할 경우 한국, 일본을 제외한 국가는 HP, mp, def 등을 + 해준다.
	// 05.11.16 - 김주현 : 홍근형이 선 작업.
...
#ifdef _NEWSTATUS
		// 05.12.06 : 김주현 - 신버젼용 HP 계산식 변경. 모든 직업 통일. [(LV+5)*SQRT(LV+20)*3.5+(STR*2)]+아이템 증가치
		float fC;
		iA=5,	iM1=20, fC=3.5f;
...
int CUserDATA::Cal_RESIST ()
{
	...
	int iTotNewKrRes = 0;
	int iRes = 0, iGradeRES = 0, iGradeRateRES = 0;
	...
	for (short nE=EQUIP_IDX_NULL+1; nE<MAX_EQUIP_IDX; nE++) {
		...
		if ( ITEM_RESISTENCE( m_Inventory.m_ItemEQUIP[ nE ].GetTYPE(), m_Inventory.m_ItemEQUIP[ nE ].GetItemNO() ) ) {
			...
			// 05.11.08 - 김주현 : 항마력 계산식 변경.
				iRes		 = ITEM_RESISTENCE( m_Inventory.m_ItemEQUIP[ nE ].GetTYPE(), m_Inventory.m_ItemEQUIP[ nE ].GetItemNO() );
				iGradeRES = ITEMGRADE_RES_KRNEW( m_Inventory.m_ItemEQUIP[ nE ].GetGrade() );
				iGradeRateRES = ITEMGRADE_RESRATE_KRNEW(m_Inventory.m_ItemEQUIP[ nE ].GetGrade());
				iTotNewKrRes += ( iRes + iGradeRES ) + ( iRes * iGradeRateRES );
			#else
	...
	#if defined ( _NEWINTERFACEKJH )
		this->m_Battle.m_nRES = (int) ( iTotNewKrRes + ( this->GetCur_INT()+5 ) * 0.6f + (this->GetCur_LEVEL()+15) * 0.8f );
	#else
	...
	#if defined ( _NEWINTERFACEKJH )
	iTotNewKrRes = this->GetPassiveSkillValue( AT_PSV_RES ) + (short)( this->m_Battle.m_nRES * this->GetPassiveSkillRate( AT_PSV_RES ) / 100.f );
	this->m_Battle.m_nRES += iTotNewKrRes;
	#else
...
int CUserDATA::Cal_HIT ()
{
	...
	if ( this->GetCur_MOVE_MODE() <= MOVE_MODE_RUN ) 
	{
		...
		if ( pRightWPN->GetItemNO() && pRightWPN->GetLife() > 0 ) {
			#if defined ( _NEWINTERFACEKJH )
			// 05.11.08 - 김주현 : 명중력 계산식 변경.
				iHitRate = (int) ( ( GetCur_CON() + 10 )*0.8f ) + 
				(int) ((( ITEM_QUALITY(ITEM_TYPE_WEAPON,pRightWPN->GetItemNO()) ) * 0.6f + ITEMGRADE_HIT_KRNEW(pRightWPN->GetGrade())) + 
				(( ITEM_QUALITY(ITEM_TYPE_WEAPON,pRightWPN->GetItemNO()) ) * 0.6f * ITEMGRADE_HITRATE_KRNEW(pRightWPN->GetGrade()))  + 
				pRightWPN->GetDurability()*0.8f );
			#else
				iHitRate = (int) ( ( GetCur_CON() + 10 )*0.8f ) + 
				(int) ( ( ITEM_QUALITY(ITEM_TYPE_WEAPON,pRightWPN->GetItemNO()) ) * 0.6f + ITEMGRADE_HIT(pRightWPN->GetGrade()) +
				pRightWPN->GetDurability()*0.8f );
			#endif
		} 
		else 
	...
	else 
	{
		...
		if ( pRightWPN->GetLife() > 0 )
			...
			(int) (( ITEM_QUALITY(ITEM_TYPE_RIDE_PART,pRightWPN->GetItemNO()) ) * 1.2f + ITEMGRADE_HIT_KRNEW(pRightWPN->GetGrade())) + 
			(( ITEM_QUALITY(ITEM_TYPE_RIDE_PART,pRightWPN->GetItemNO()) ) * 1.2f * ITEMGRADE_HITRATE_KRNEW(pRightWPN->GetGrade())) ;
			#else			
			iHitRate = (int) ( ( GetCur_CON()+10 ) * 0.8f ) + ( GetCur_LEVEL() * 0.5f ) +
...
float CUserDATA::Cal_NEWKRAttack(int AttckPWR, unsigned short igrade)
{

// 05.11.08 - 김주현 : 공격력 계산식 변경.
	#if defined ( _NEWINTERFACEKJH )
		return (( AttckPWR + ITEMGRADE_ATK_KRNEW( igrade )) + ( AttckPWR * ITEMGRADE_ATKRATE_KRNEW( igrade )));
	#else
		return (AttckPWR + ITEMGRADE_ATK( igrade ));
	#endif
}
	...
	if ( this->GetCur_MOVE_MODE() <= MOVE_MODE_RUN ) 
	{
		...
		else 
		{
			switch ( WEAPON_TYPE( pRightWPN->m_nItemNo ) / 10 ) {
				case 21 :	// 한손
				case 22 :	// 양손		// 근접 무기
					iAP = (int)( ( GetCur_STR()*0.75f + GetCur_LEVEL()*0.2f ) + ( Cal_NEWKRAttack(iWeaponAP ,pRightWPN->GetGrade()) * ( GetCur_STR()*0.05f+29) / 30.f ) );
					break;
				case 24 :				// 마법 무기
					if ( 241 == WEAPON_TYPE( pRightWPN->m_nItemNo ) ) {	
						// 마법 지팡이.
						iAP = (int)( ( GetCur_STR()*0.4f + GetCur_INT()*0.4f + GetCur_LEVEL()*0.2f ) + (Cal_NEWKRAttack(iWeaponAP ,pRightWPN->GetGrade()) * ( GetCur_INT()*0.05f + 29 ) / 30.f ) );
					} else {
						iAP = (int)( ( GetCur_INT()*0.6f + GetCur_LEVEL()*0.2f ) + ( Cal_NEWKRAttack(iWeaponAP ,pRightWPN->GetGrade()) * ( GetCur_SENSE()*0.1f + 26 ) / 27.f ) );
					}
					break;
				case 25 :				// 카타르
					if ( 252 == WEAPON_TYPE( pRightWPN->m_nItemNo ) ) {	
						// 이도류
						iAP = (int)( ( GetCur_STR()*0.63f + GetCur_DEX()*0.45f + GetCur_LEVEL()*0.2f ) + ( Cal_NEWKRAttack(iWeaponAP ,pRightWPN->GetGrade()) * ( GetCur_DEX()*0.05f + 25 ) / 26.f ) );
					} else {
						iAP = (int)( ( GetCur_STR()*0.42f + GetCur_DEX()*0.55f + GetCur_LEVEL()*0.2f ) + ( Cal_NEWKRAttack(iWeaponAP ,pRightWPN->GetGrade()) * ( GetCur_DEX()*0.05f + 20 ) / 21.f ) );
					}
					break;
			...
			case SHOT_TYPE_ARROW:///활무기
				{
					iAP = (int)( ( GetCur_DEX()*0.52f + GetCur_STR()*0.1f + GetCur_LEVEL()*0.1f + 3 ) +
						( ( Cal_NEWKRAttack(iWeaponAP ,pRightWPN->GetGrade()) + PatArmsAttackPower ) * 
...
int CUserDATA::Cal_DEFENCE ()
{
	...
	for (nE=EQUIP_IDX_NULL+1; nE<MAX_EQUIP_IDX; nE++) 
	{
	...
	#if defined ( _NEWINTERFACEKJH )
		// 05.11.08 - 김주현 : 제련 방식 변경 관련 방어력 계산식 변경.
		if ( ITEM_DEFENCE( m_Inventory.m_ItemEQUIP[ nE ].GetTYPE(), m_Inventory.m_ItemEQUIP[ nE ].GetItemNO() ) ) {
			...
			iDefRATE	 = ITEMGRADE_DEFRATE_KRNEW( m_Inventory.m_ItemEQUIP[ nE ].GetGrade() );
			iTotDEFNEWKR += (iTotDEF + iTotGradeDEF) + ( iTotDEF * iDefRATE );
	...
	else 
	{
		for (nE=0; nE<MAX_RIDING_PART; nE++) 
		{
			if ( 0 == m_Inventory.m_ItemRIDE[ nE ].GetLife() || 0 == m_Inventory.m_ItemRIDE[ nE ].GetTYPE() )
				continue;
			#if defined ( _NEWINTERFACEKJH )
			iTotDEFNEWKR	+= ITEMGRADE_DEF_KRNEW( m_Inventory.m_ItemRIDE[ nE ].GetGrade() );
	...
	{
	#ifdef _NEWSTATUS
			// 05.12.06 : 김주현 - 신버젼용 방어력 계산식 변경
			this->m_Battle.m_nDEF = (int) ((iTotDEFNEWKR + ( this->GetCur_STR()+5 ) * 0.35f )) + this->m_iAddValue[ AT_DEF ];
	#else
			#if defined( _NEWINTERFACEKJH )
				this->m_Battle.m_nDEF = (int) ((iTotDEFNEWKR + ( this->GetCur_STR()+5 ) * 0.35f + (this->GetCur_LEVEL()+15) * 0.7f )) + this->m_iAddValue[ AT_DEF ];
			#else
	...
	#ifdef _NEWSTATUS
			// 05.12.06 : 김주현 - 신버젼용 방어력 계산식 변경
			this->m_Battle.m_nDEF = (int)(( iTotDEFNEWKR + ( this->GetCur_STR()+5 ) * 0.35f)) * 0.8f + this->m_iAddValue[ AT_DEF ];
	#else
		#if defined( _NEWINTERFACEKJH )
			this->m_Battle.m_nDEF = (int)(( iTotDEFNEWKR + ( this->GetCur_STR()+5 ) * 0.35f + (this->GetCur_LEVEL()+15) * 0.7f )) * 0.8f + this->m_iAddValue[ AT_DEF ];
		#else
...
short CUserDATA::Skill_FindLearnedSlot ( short nSkillIDX )
{
...
#ifdef _DEBUG
	int iTab_Type = SKILL_TAB_TYPE(nSkillIDX);
#endif
	

	switch( SKILL_TAB_TYPE(nSkillIDX) )
	{
	...
	case SKILL_TAB_JOB2:
		nI = 0;
		nMaxCnt = MAX_JOB_SKILL;
		break;
	
	case SKILL_TAB_PARTS_CART:	
	case SKILL_TAB_PARTS_CASTLEGEAR:
		nI = MAX_JOB_SKILL;
		nMaxCnt = nI + MAX_PATS_SKILL;
		break;

	case SKILL_TAB_UNIQUE:	
		nI = MAX_JOB_SKILL + MAX_PATS_SKILL;
		nMaxCnt = nI + MAX_UNIQUE_SKILL;
		break;
	}

	for( nI=0 ; nI<nMaxCnt ; nI++ ) 
	{
		if ( 0 == m_Skills.m_nSkillINDEX[ nI ] )
		{
			continue;
		}	

		if ( n1LevSkillIDX == SKILL_1LEV_INDEX( m_Skills.m_nSkillINDEX[ nI ] ) )
		{
			return ( nI );
		}
	}
...
bool CUserDATA::Skill_CheckExclusive( short nSkillIDX )
{
	short nI, n1LevSkillIDX = SKILL_1LEV_INDEX( nSkillIDX );
	short *pSkill = NULL;
	short nMaxCnt = 0;
	...
	for( nI=0 ; nI<nMaxCnt ; nI++ ) 
	{
		//한가지 스킬을 배우면 배타적인 스킬을 배울 수 없다.
		if ( n1LevSkillIDX == SKILL_CANT_USE_OTHER_SKILL( m_Skills.m_nSkillINDEX[ nI ] ) )
		{
			return false;
		}
	}	
...
BYTE CUserDATA::Skill_LearnCondition( short nSkillIDX )
{
	...
	// 5. 배타적인 스킬 판단.
	if ( !this->Skill_CheckExclusive( nSkillIDX ) ) 
	{
		return RESULT_SKILL_LEARN_FAILED;	
	}
...
short CUserDATA::GetPassiveSkillAttackSpeed( short nRightWeaponItemNo )
{
	...
	eIndex = WEAPON_TYPE(nRightWeaponItemNo)  -93;
		...
		for (short nI=0; nI<SKILL_INCREASE_ABILITY_CNT; nI++) 
		{
			...
			if ( SKILL_INCREASE_ABILITY( nSkillIDX, nI ) >= AT_STR  && SKILL_INCREASE_ABILITY( nSkillIDX, nI ) <= AT_SENSE ) 
			{
				...
				else 
				{
					...
					////06. 02. 23 - 김주현 : 패시브 스킬 관련 버그 수정 이거 누가 + 를 하게 한고지?? 스탯을 막 더해버린다??
...
// Begin - 2006-08-31  오후 5:20:09 - 김영환: 일반 퀘스트 슬롯 10개 -> 9개 사용으로 수정
short CUserDATA::Quest_Append_9 ( int iQuestIDX )
{
	if ( iQuestIDX ) {
		short nI;
		for (nI=0; nI<QUEST_PER_PLAYER_CW; nI++) {
			if ( iQuestIDX == m_Quests.m_QUEST[ nI ].GetID() ) {
				m_Quests.m_QUEST[ nI ].Init ();
				m_Quests.m_QUEST[ nI ].SetID( iQuestIDX, true );
				return nI;
...
int CUserDATA::Set_InitSKILLType ( int iBeginSkillIndex, int iSize )
{
	...
	CSkillSlot*	p = NULL;
	assert( g_pAVATAR );
	if( g_pAVATAR )	{	p = g_pAVATAR->GetSkillSlot();	}	
...
bool CUserDATA::Reward_InitSKILLType ( int iInitType, int iSkillSlot )
{
	...
	*/		
	...
	case SKILL_TYPE_ALL:
		...
		break;	
	default:
		iRecoverySP += Set_InitSKILLType( iSkillSlot, 1 );
	}
...
void  CUserDATA::SetCur_MP (short nValue)	
{
	this->m_GrowAbility.m_nMP=nValue;				
}
...
int CUserDATA::GetCur_SaveMP ()
{
#ifdef _DEBUG
	int i = GetPassiveSkillValue( AT_PSV_SAVE_MP );
	int ii = m_iAddValue[ AT_SAVE_MP ];
	int iii = m_iAddValue[ AT_SAVE_MP ]*GetPassiveSkillRate( AT_PSV_SAVE_MP );
	int iiii = GetPassiveSkillRate( AT_PSV_SAVE_MP ) ;
	int iiiii = (short)(m_iAddValue[ AT_SAVE_MP ]*GetPassiveSkillRate( AT_PSV_SAVE_MP )/100.f) ;
	int iiiiii = ( GetPassiveSkillValue( AT_PSV_SAVE_MP ) + m_iAddValue[ AT_SAVE_MP ] + (short)(m_iAddValue[ AT_SAVE_MP ]*GetPassiveSkillRate( AT_PSV_SAVE_MP )/100.f) );	
