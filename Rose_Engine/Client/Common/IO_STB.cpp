...
STBDATA::STBDATA(char *pszName) 
{
	m_bLoad2=false, m_ppDATA=NULL,m_nColCnt=m_nDataCnt=0; SetSTBName(pszName); 
}
...
int	STBDATA::GetData(DWORD nRow,DWORD  nCol)
{
...
int & STBDATA::GetData(DWORD nRow,DWORD  nCol)
{
	if(nRow>=(DWORD)m_nDataCnt)
	{	
		MessageBox( NULL, CStr::Printf("%s GetData Count  Error  %d, %d ",m_strSTBName,m_nDataCnt,nRow), "Error", MB_OK  );
		nRow = 0;
		nCol = 0;
	}
...
bool STBDATA::Load2 (char *szFileName, bool bHasTYPE, bool bMakeKEY)
{
	...
	if ( cFILE.Open (szFileName) ) {
		...
		this->m_nDataCnt = cFILE.GetRowCount ();
		this->m_nColCnt  = cFILE.GetColumnCount ();
...
void STBDATA::Free (void)
{
	if ( m_bLoad2 ) 
	{
		if ( m_ppVALUE ) 
		{
			for ( int nY=0; nY<this->m_nDataCnt; nY++) 
			{
