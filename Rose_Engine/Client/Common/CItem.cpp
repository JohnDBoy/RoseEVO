...
//20060706 홍근 아이템 갯수 확장. __ITEM_MAX
...
bool tagBaseITEM::IsEnableSeparate ()
{
	...
	//06.08.08 - 김주현 : 에볼루션은 보석도 분해가 가능하다.
	#ifdef __USEJEMBREAK
	#else
	if( GetTYPE() == ITEM_TYPE_GEM )///보석은 분해가 불가능하다
		return false;
	#endif
	
	if( GetTYPE() == ITEM_TYPE_QUEST) /// 퀘스트 아이템은 분해가 불가능.
		return false;

	if( GetTYPE() == ITEM_TYPE_JEWEL) /// 장신구도 분해가 불가능.
		return false;
	...
	if( GetTYPE() == ITEM_TYPE_USE )///소모 아이템은 분해가 불가능하다
		return false;
