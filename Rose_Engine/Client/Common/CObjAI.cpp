/*
	** 주의 사항 !!!
	...
	$Header: /Client/Common/CObjAI.cpp 245   06-12-29 2:14a Choo0219 $
...
void CObjAI::SetCMD_MOVE ( WORD wSrvDIST, const D3DVECTOR& PosTO, int iServerTarget )
{
	...
	// 05.12.08 - 김주현 : 현재의 한국과 일본은 카트를 탑승했을시 아이템을 주을수 없도록 설정되어 있음.
	if(pDestOBJ 
	...
	else
	{
		...
		if ( pDestOBJ ) 
		{
			...
			{
				//06. 05. 19 - 김주현 : 일본 에보에서도 카트 탑승시에는 아이템을 주을수 없다.
				#ifdef __JAPAN_NEW
...
#ifdef __TEST // 홍근 : 타겟 바꿔서 스킬 쓰는 경우 유져 공격 가능 버그
	m_wState  = CS_STOP;
#endif	
...
#ifdef __TEST // 홍근 : 타겟 바꿔서 스킬 쓰는 경우 유져 공격 버그
			{
				int iTarget_Filter = SKILL_CLASS_FILTER( nSkillIDX );

				if( SKILL_TARGET_FILTER_ENEMY_ALL == iTarget_Filter 
					|| SKILL_TARGET_FILTER_ENEMY_PC == iTarget_Filter
					|| SKILL_TARGET_FILTER_ENEMY_MOB == iTarget_Filter )
				{
                    m_wBeforeCMD = m_wCommand;
				}
				else
...
{
#ifdef _DEBUG
//	::InputSceneSphere(m_PosGOTO.x, m_PosGOTO.y, 0.0f, 50.0f); 
#endif
...
//-------------------------------------------------------------------------------
#if defined(_GBC)
	//박지호:: 카트 탑승시 아바타가 공격 하지 못하도록 리턴을 한다. 
	if(GetPetMode() >= 0)
		 return 1;
#endif 
//-------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------	
#if defined(_GBC)
	//박지호::펫모드일때 펫을 이동 시키고,아바타 스킬을 캐스팅한다.
	if(GetPetMode() >= 0)
...
void CObjAI::Set_AdjustSPEED( float fSpeed )
{
	m_fAdjustSPEED = fSpeed;
}


int CObjAI::Get_PercentHP()
{
...
//--------------------------------------------------------------------------------
/// NPC  충돌에 대한 이펙트 처리 Frame
//--------------------------------------------------------------------------------

bool CObjAI::ProcSpecialMotionFrame (void)
{
	int iFrame = this->GetCurrentFrame();
	if ( iFrame < 0 ) 
	{
...
bool CObjAI::ProcOneSpecialActionFrame( int iIndex )
{
	if ( m_pCurMOTION && m_pCurMOTION->m_nActionPointCNT ) 
	{
		if ( m_pCurMOTION->m_pFrameEvent[ iIndex ] ) 
		{
			ActionSpecialEVENT(m_pCurMOTION->m_pFrameEvent[ iIndex ]);
			return true;
		}
	}
