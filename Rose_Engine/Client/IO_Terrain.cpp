...
{
	if( /*g_GameDATA.m_bJustObjectLoadMode*/ true )     //조성현 2005 11 - 7 Culling과정
		return 0;
...
bool CMAP_PATCH::UnregisterFromNZIN (void)
{
	...
	while( pEftNode ) 
	{
		...
		{
/*			assert( 0 && "pEftNode->DATA.m_pEffect = NULL" );   //조성현 2005 11 - 8  Test
...
void CMAP::AddEffect(D3DVECTOR &Position, D3DXQUATERNION &Rotation, char *szEffectFile)
{
	...
	tagMAPEFT sMapEFT;   //조성현 2005 11 - 7 Rotation추가..
...
void CTERRAIN::ReadEventObjINFO ( CFileSystem* pFileSystem, long lOffset)
{
	...
	//조성현 2005 12 - 5  Test Event Object의 위치 파악..
    //-----------------------------------------------------	
...
{
#ifdef _DEBUG	
	m_gNullEffectNumber = 0; //조성현 2005 11 - 8 Test
#endif	
...
//조성현 2005 12 - 5 Test EventObject 보이게 하기..
//-------------------------------------------------------------------------------------------------------
...
//   float vMin[3], vMax[3];
 ...
 /*  for(int i = 0; i < m_gNullEffectNumber; i += 1)  //조성현 2005 11 - 8 Test
   {
...
void CTERRAIN::UpdatePatchManager(short nCenterMapXIDX, short nCenterMapYIDX)
{
	...
	for (nI=0; s_AddSecIdx[ MOVE_UPDATE_ALL ][ nI ]>=0; nI++) {
		    ...
		    //조성현 2005 12 - 29 캐릭터선택 화면 전체 Loading 문제..
...
void CTERRAIN::AddMAPALL ()
{
	...
	for (int nX=0; nX < MAP_COUNT_PER_ZONE_AXIS; nX++) {
		...
		{
			LogString ( LOG_DEBUG, "AddMAP: ALL (%d, %d) \n", nX,nY);
