...
void CItem::AddItemGrade( tagITEM& sItem, CInfo& Info )
{
// 05.11.07 - 김주현 : 아이템 능력치 설명하는 부분에 아이템 등급 추가.
	Info.AddString( CStr::Printf( "%s:%d", STR_ITEMGRADE, ITEM_DESC_GRADE(sItem.GetTYPE(), sItem.GetItemNO()) ) );
}
#endif
...
void CItem::AddItemTimeLimite( tagITEM& sItem, CInfo& ToolTip )
{
#ifdef __ITEM_TIME_LIMMIT
	//보통 아이템 정보 표시 : 적용시간과, 남은시간이 둘다 들어 있다.
	if( sItem.wPeriod && sItem.dwPickOutTime )
	{
		struct tm Client_tm;
		time_t ss = 0;
		...
		//서버 시간.
		//시간 서버 시간으로 통일해서 사용.
		time_t Server_time = Client_time + g_GameDATA.m_dwServerTimeGap;				
		
		time_t long_time = (sItem.dwPickOutTime) + sItem.get_basicTime() + (sItem.wPeriod) * 3600 ;
		...
		time_t End_time	= mktime( &l );
		
		struct tm l2 = *localtime( &End_time );
		struct tm l3 = *localtime( &Server_time );

		time_t Using_time = End_time - Server_time;

		if( Using_time >= 0 )
		{

			//"[적용시간 %d/%d/%d %d시]"
			ToolTip.AddString( CStr::Printf( LIST_STRING(966),
				sItem.get_startTime().tm_year,
				sItem.get_startTime().tm_mon,
				sItem.get_startTime().tm_mday,
				sItem.get_startTime().tm_hour ), 0XFF8080FF );

#ifdef __TEST
			ToolTip.AddString( CStr::Printf( "Period:%d, ClientTime:%d, ServerTimeGap:%d, UsingTime:%d",
				sItem.wPeriod,
				Client_time,
				g_GameDATA.m_dwServerTimeGap,
				Using_time), 0XFF8080FF );
#endif

			//사용시간
			struct tm Remain_tm = *localtime( &Using_time );
			struct tm Basic_tm = *localtime( &ss );
			Remain_tm = *gmtime( &Using_time );
			Basic_tm = *gmtime( &ss );

			std::string strBuf;
			strBuf = LIST_STRING(967);//"[사용시간 "
			if( Remain_tm.tm_year-Basic_tm.tm_year )
			{
				strBuf += CStr::Printf(LIST_STRING(968), Remain_tm.tm_year-Basic_tm.tm_year );//"%d년 "
			}
			if( Remain_tm.tm_mon-Basic_tm.tm_mon )
			{
				strBuf += CStr::Printf(LIST_STRING(969), Remain_tm.tm_mon-Basic_tm.tm_mon );//"%d월 "
			}
			if( Remain_tm.tm_mday-Basic_tm.tm_mday )
			{
				strBuf += CStr::Printf(LIST_STRING(970), Remain_tm.tm_mday-Basic_tm.tm_mday );//"%d일 "
			}
			if( Remain_tm.tm_hour-Basic_tm.tm_hour )
			{
				strBuf += CStr::Printf(LIST_STRING(971), Remain_tm.tm_hour-Basic_tm.tm_hour );//"%d시간 "
			}
			if( Remain_tm.tm_min-Basic_tm.tm_min )
			{
				strBuf += CStr::Printf(LIST_STRING(972), Remain_tm.tm_min-Basic_tm.tm_min );//"%d분 "
			}
			strBuf += "]";
			
			ToolTip.AddString( strBuf.c_str(), 0XFF8080FF );			
			
		}
	}
	// DeliveryStoreDlg에서 사용 : 적용전 이므로 적용시간은 없고, 사용 할 수 있는 기간만 있다.
	//sItem.wPeriod = 1990; //test
	else if( sItem.wPeriod )	
	{	
		int iMonth	= sItem.wPeriod / 720;
		int iDay	= (sItem.wPeriod % 720) / 24;
		int iHour	= sItem.wPeriod % 24;

		//973 : %d월 %d일 %d시간 동안 사용할 수 있는 아이템입니다.
		ToolTip.AddString( CStr::Printf(LIST_STRING(973), iMonth, iDay, iHour ), 0XFF8080FF );
		...
		if( iMonth )
		{
			strBuf += CStr::Printf(LIST_STRING(969), iMonth );//"%d월 "
		}
		if( iDay )
		{
			strBuf += CStr::Printf(LIST_STRING(970), iDay );//"%d일 "
		}
		if( iHour )
		{
			strBuf += CStr::Printf(LIST_STRING(971), iHour );//"%d시간 "
		}

		strBuf += "동안 사용할 수 있는 아이템입니다.";
...
void CItem::GetToolTipRidePart_BattleCart( CInfo& ToolTip,  DWORD dwDialogType, DWORD dwType )
{
	...
	//21 기간제 아이템
	AddItemTimeLimite( sItem, ToolTip );
...
int CItem::GetGrade(tagITEM& sItem)
{
	return 0;
}
...
void CItem::GetToolTip( CInfo& ToolTip,  DWORD dwDialogType, DWORD dwType )
{
	...
	case ITEM_TYPE_WEAPON:
		{
			...
			#if defined( _NEWINTERFACEKJH )
			{
				int a = sItem.GetHitRate();
				int b = ITEMGRADE_HIT_KRNEW( sItem.GetGrade() );
				int c = (int)ITEMGRADE_HITRATE_KRNEW( sItem.GetGrade());
				float d = sItem.GetHitRate() * ITEMGRADE_HITRATE_KRNEW( sItem.GetGrade() );
				int e = (sItem.GetHitRate() + ITEMGRADE_HIT_KRNEW( sItem.GetGrade() ));
				int f = e + (int)d;
				strTemp.append( CStr::Printf("%d", sItem.GetHitRate() + ITEMGRADE_HIT_KRNEW( sItem.GetGrade() ) + (int)(sItem.GetHitRate() * ITEMGRADE_HITRATE_KRNEW( sItem.GetGrade() ))) );
			}
			#else
			...
			ToolTip.AddString( CStr::Printf( "%s:%d  %s:%s", 
									CStringManager::GetSingleton().GetAbility( AT_ATK ), 
									(int)((GetAttackPower( sItem ) + ITEMGRADE_ATK_KRNEW( sItem.GetGrade() )) + (GetAttackPower( sItem ) * ITEMGRADE_ATKRATE_KRNEW(sItem.GetGrade() ))) ,
									CStringManager::GetSingleton().GetAbility( AT_ATK_SPD ),
									strAttackSpeed.c_str()) );
			#else
			ToolTip.AddString( CStr::Printf( "%s:%d  %s:%s", 
		...
		#if defined( _NEWINTERFACEKJH )
			AddItemGrade(sItem, ToolTip); // 아이템 등급 추가.
		#endif
			if(bIsDetail) 
			{
	...
	case ITEM_TYPE_USE:
		{
//품질 대신에 해당 아이템의 능력치로 변경.
#ifdef __EXPERIENCE_ITEM
	
			switch ( ITEM_NEED_DATA_TYPE( ITEM_TYPE_USE, nItemIdx, 0 ) )
			{
			case AT_PSV_EXPERIENCE_ITEM:
				{
					AddItemDefaultTopExpItemText( sItem, ToolTip);
				}
				break;
			...
			case USE_ITEM_FUEL:
				{
					//06.11.13 - 김주현 : 엔진연료 아이템의 툴팁에 엔진연료란에 % 기호가 추가.
					ToolTip.AddString( CStr::Printf("%s : %d%%", 
			...
			case USE_ITEM_MAINTAIN_ITEM:///시간제 쿠폰 아이템
				//06. 12. 15 - 김주현 : 사용 가능한 시간을 일단위에서 분단위까지로 확장.
#ifdef __EXPERIENCE_ITEM
				if( USEITEM_ADD_DATA_TYPE( nItemIdx ) )
				{
					if(USEITEM_ADD_DATA_VALUE( nItemIdx ) / 144)
					{
						int iHour = USEITEM_ADD_DATA_VALUE( nItemIdx ) % 144 / 6;
						
						int iMinute = USEITEM_ADD_DATA_VALUE( nItemIdx ) % 144 % 6 * 10;

						if( iHour && iMinute)
						{
							ToolTip.AddString( CStr::Printf( "[%s %d %s %d %s %d %s]", CStringManager::GetSingleton().GetAbility( USEITEM_ADD_DATA_TYPE( nItemIdx ) ), USEITEM_ADD_DATA_VALUE( nItemIdx ) / 144, STR_DAY, iHour, STR_HOUR, iMinute, STR_MINUTE) );						
						}
						else if( iHour && !iMinute)
						{
							ToolTip.AddString( CStr::Printf( "[%s %d %s %d %s]", CStringManager::GetSingleton().GetAbility( USEITEM_ADD_DATA_TYPE( nItemIdx ) ), USEITEM_ADD_DATA_VALUE( nItemIdx ) / 144, STR_DAY, iHour, STR_HOUR) );
						}
						else if( !iHour && iMinute)
						{
							ToolTip.AddString( CStr::Printf( "[%s %d %s %d %s]", CStringManager::GetSingleton().GetAbility( USEITEM_ADD_DATA_TYPE( nItemIdx ) ), USEITEM_ADD_DATA_VALUE( nItemIdx ) / 144, STR_DAY, iMinute, STR_MINUTE) );
						}
						else
						{
					...
					else
					{
						...
						if( iHour && iMinute)
						{
							ToolTip.AddString( CStr::Printf( "[%s %d %s %d %s]", CStringManager::GetSingleton().GetAbility( USEITEM_ADD_DATA_TYPE( nItemIdx ) ), iHour, STR_HOUR, iMinute, STR_MINUTE) );						
						}
						else if( iHour && !iMinute)
						{
							ToolTip.AddString( CStr::Printf( "[%s %d %s]", CStringManager::GetSingleton().GetAbility( USEITEM_ADD_DATA_TYPE( nItemIdx ) ), iHour, STR_HOUR) );
						}
						else	// !iHour && iMinute 
						{
							ToolTip.AddString( CStr::Printf( "[%s %d %s]", CStringManager::GetSingleton().GetAbility( USEITEM_ADD_DATA_TYPE( nItemIdx ) ), iMinute, STR_MINUTE) );
						}
					}
...
#ifdef __TEST  // 옵션이 붙을 경우 옵션 번호 출력.	
	if( sItem.GetOption() )
		ToolTip.AddString( CStr::Printf("[option: %d]", sItem.GetOption() ), g_dwYELLOW );
#endif
}
...
//		dwTextColor = g_dwYELLOW;
	...
	if( sItem.GetTYPE() != ITEM_TYPE_RIDE_PART )
	{
		...
		#if defined	( _NEWINTERFACEKJH )
		{
			stdTemp.append( CStr::Printf("%d", (sItem.GetAvoidRate() + ITEMGRADE_AVOID_KRNEW( sItem.GetGrade() )) +(int)(sItem.GetAvoidRate() * ITEMGRADE_AVOIDRATE_KRNEW( sItem.GetGrade() )) ));
		}
		#else
...
void CItem::AddItemDefaultTopExpItemText( tagITEM& sItem, CInfo& Info)
{
	//이름
	Info.AddString( (char*)(ITEM_NAME( sItem.GetTYPE(), sItem.GetItemNO() ) ), GetItemNameColor( sItem.GetTYPE(), sItem.GetItemNO()) , g_GameDATA.m_hFONT[ FONT_NORMAL_BOLD ]);
	...
	int iQuality = ITEM_QUALITY(ITEM_TYPE_USE,sItem.GetItemNO() ) % 100;
	if( iQuality > 0)
	{
		float fTemp = float(ITEM_QUALITY(ITEM_TYPE_USE,sItem.GetItemNO() )) / 100.0f;
		sprintf(Temp, "x%.1f", fTemp);
	}
	else
	{
		int iTemp = ITEM_QUALITY(ITEM_TYPE_USE,sItem.GetItemNO() ) / 100;
		sprintf(Temp, "x%d", iTemp);
	}

		
	//분류, 경험치
	char* pszBuf = CStr::Printf( "%s:%s %s:%s",
		STR_ITEM_TYPE,
		CStringManager::GetSingleton().GetItemType( ITEM_TYPE( sItem.GetTYPE(), sItem.GetItemNO() ) ),
		LIST_STRING(814),
		Temp);
...
void CItem::AddItemDefence( tagITEM& sItem, CInfo& Info )
{
	...
	if( sItem.GetGrade() )
	#if defined ( _NEWINTERFACEKJH )
		stdTemp.append(CStr::Printf(":%d ", (GetDefence( sItem ) + ITEMGRADE_DEF_KRNEW(sItem.GetGrade())) + (int)(GetDefence( sItem ) * ITEMGRADE_DEFRATE_KRNEW(sItem.GetGrade()))) );
	#else
	...
	if( sItem.GetGrade() )
	#if defined ( _NEWINTERFACEKJH )
		stdTemp.append( CStr::Printf(":%d", (int)(ITEM_RESISTENCE(nItemType, nItemIdx) + ITEMGRADE_RES_KRNEW( sItem.GetGrade() ) ) + (int)(ITEM_RESISTENCE(nItemType, nItemIdx) * ITEMGRADE_RESRATE_KRNEW( sItem.GetGrade() ) )) );
	#else
...
void CItem::AddItemPrice(tagITEM& sItem, DWORD dwDlgType, DWORD dwType, CInfo& Info )
{
	...
	{
#ifdef __CLAN_STORE
		if (CStore::GetInstance().IsClanStore() )
		{
			iPrice = ITEM_TRADE_UNIONPOINT( sItem.GetTYPE(), sItem.GetItemNO() ); //15 Column.
			pszBuf = CStr::Printf( "%s:%d", LIST_STRING( 955 ), iPrice ); //보상포인트.
		}
		else
#endif		
...
int CItem::GetItemRareType( int type, int no )
{
	...
	case ITEM_TYPE_BOOTS:
	case ITEM_TYPE_FACE_ITEM: // 07. 02. 26 - 김주현 : 페이스아이템 / 백아이템 레어타입에 추가.
	case ITEM_TYPE_KNAPSACK:
...
DWORD CItem::GetItemNameColor( int type, int no )
{
	...
	else if( iRareType == 31) // 07.02. 23 - 김주현 : 마일리지 아이템 색상 추가.
		color = D3DCOLOR_ARGB( 0xFF, 0xFD, 0xCD, 0);///황금색
