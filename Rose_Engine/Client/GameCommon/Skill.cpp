/*
	$Header: /Client/GameCommon/Skill.cpp 91    06-07-04 10:34a Kjhpower $
*/
#include "stdAFX.h"
#include "Skill.h"
...
/// @brief draw icon..
//----------------------------------------------------------------------------------------------------
void CSkill::DrawIcon( int iX, int iY, DWORD dwColor )
{		
...
/////////2005. 5. 30.  박 지호 
	if(CCountry::GetSingleton().IsUseItemDelayNewVersion())
	{
		...
		{
			if( IsEnable() )		
				pDrawObj->Draw( iX,	iY,	IMAGE_RES_SKILL_ICON, nIconNo, dwColor ); 
			else
...
int CCreateWindowActionSkill::SendSkillActionReq( int iSkillSlot, int iTarget, D3DXVECTOR3 &PosTO )
{
	...
	//06. 02. 24 - 김주현 : 상급 제조 / 특수포션 추가 51부터  60 까지
	int iWindowType = SKILL_POWER(GetSkillIndex() );
	if( iWindowType >= 11 && iWindowType <= 39 || iWindowType >= 51 && iWindowType <= 60)
...
bool CSkillManager::FireSkill( int iSkillSlotIDX, int iTargetObj, D3DXVECTOR3 &PosTO )
{
	...
	if(CSkillManager::GetSkillTargetType( pSkill->GetSkillIndex() ) == SKILL_TARGET_NONE)
	{
		int iSkillIndex = SKILL_SCOPE( pSkill->GetSkillIndex() );

		if(g_pTerrain->IsPVPZone() && CSkillManager::CheckDistanceNPC(iSkillIndex) == false)
		{
			g_itMGR.AppendChatMsg( STR_CANT_CAST_NEARNPC,  IT_MGR::CHAT_TYPE_SYSTEM);
            return false;
		}
	}
	else
	{
		...
		{			
			...
			if(g_pObjMGR->m_pOBJECTS[ iTargetObj ]->Get_TYPE() == OBJ_NPC)
			{
				g_itMGR.AppendChatMsg( STR_CANT_CAST_NPC,  IT_MGR::CHAT_TYPE_SYSTEM);
				return false;
			}
		}
	}
...
{	
	...
	for ( short nI=0; nI<SKILL_USE_PROPERTY_CNT; nI++) 
	{
		...
		//06. 02. 09 - 김주현 : HP, MP 스킬 사용할때 요기서 MP랑 HP를 즉각적으로 깍는것 같은데...-_-a 서버에서 보내주므로.. 일단.. X
...
/*static*/ bool CSkillManager::CheckNeedWeapon( int iSkillIDX, CObjCHAR* pCaster )
{
	...
	int iCurrentLWeaponNO = pCaster->Get_LWeapon();	
	...
	for( int i = 0; i < SKILL_NEED_WEAPON_CNT; i++ )
	{
		...
		if( SKILL_NEED_WEAPON( iSkillIDX, i ) )
		{
			...
			if( SKILL_NEED_WEAPON( iSkillIDX, i ) == WEAPON_TYPE( iCurrentRWeaponNO ) 
				|| SKILL_NEED_WEAPON( iSkillIDX, i ) == SUBWPN_TYPE( iCurrentLWeaponNO ) )
				return true;
		}
	}
...
bool CSkillManager::CheckDistanceNPC(int iskill_distance)
{
	iskill_distance += 150;
	...
	for( ; begin != npcInfoInMapList.end() ; ++begin )
	{
		...
		if( abs( iDistance ) < iskill_distance )
		{
			return false;
		}
	}					
...
bool CSkillManager::CheckHealType(int skillindx)
{
	if(SKILL_TYPE(skillindx) == SKILL_ACTION_SELF_BOUND || SKILL_TYPE(skillindx) == SKILL_ACTION_TARGET_BOUND)
		return true;
