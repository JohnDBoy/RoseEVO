		...
		case OBJ_MOB:
			{
				...
				if( !(g_pAVATAR->GetPetMode() < 0) )
				{
					if(g_pAVATAR->m_pObjCART->Get_COMMAND() == CMD_ATTACK && iTargetObj == g_pObjMGR->Get_ClientObjectIndex( g_pAVATAR->m_iServerTarget ) )
                    break;
				}
				else
				{				
...
void CUserInputSystem::SetTargetSelf()
{
	...
	//06. 02. 22 - 김주현 : 셀프 타게팅 스킬을 사용하게 될 경우 공격 중지.
	//공격 중지 관련 패킷이 없어서;; 이동패킷을 이용해서 꼼수를;; 다른 유져가 보게되면 약간 이동하는걸로 보일수도..

	g_pNet->Send_cli_MOUSECMD(0, g_pAVATAR->Get_CurPOS());
