...
void CBankWindowDlg::SetInterfacePos_After()
{
	CWinCtrl * pCtrl = NULL;
	...
	if( pCtrl = Find( IID_BTN_CANCEL ) )
	{
		//859 취소
		((CTButton*)pCtrl)->SetText( LIST_STRING(859) );		
	}
	...
	case MSG_SETYPE:
		{
			...
			{
			case TYPE_SAVE:
				m_Title.set_string( STR_QUERY_BANK_SAVE, rc );
				break;
			case TYPE_WITHDRAW:
				m_Title.set_string( STR_QUERY_BANK_WITHDRAW, rc );
				break;
			default:
				break;
			}
