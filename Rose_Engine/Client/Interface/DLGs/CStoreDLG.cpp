...
void CStoreDLG::SetTabButtonText( short iIndex, const char* szText )
{
	...
	if( pCtrl && pCtrl->GetControlType() == CTRL_RADIOBUTTON )
	{
		...
		pBtn->SetFont( FONT_NORMAL_OUTLINE );
	}
}
...
void CStoreDLG::Show()
{
	...
	GetCaption()->SetString( CStr::Printf("  %s", STR_STORE ) );
	...
	if( CTDialog* pDlg = CTDialog::GetProcessMouseOverDialog() )
	{
		if( pDlg == (CTDialog*)this )
		{
			...
			{		
				...
				if( pCtrl->is_ellipsis() && pBtn->IsInside( ptMouse.x, ptMouse.y ) )
					CToolTipMgr::GetInstance().RegToolTip
					( 
						pBtn->GetPosition().x - 10, 
						pBtn->GetPosition().y - 18,
						pCtrl->get_string() 
					);
			}
...
void CStoreDLG::SetInterfacePos_After()
{
	CWinCtrl * pCtrl = NULL;

	//문자열 추가.
	//상점 124
	GetCaption()->SetString( LIST_STRING(124) );
}