...
void CSystemMsgDlg::Draw()//helptext/anounce
{
	if( !IsVision() ) return;
	...
	{
		g_DrawImpl.Draw( m_sPosition.x, iPosY, m_iWidth, UI_IMAGE_RES_ID, m_iImageIndex ,D3DCOLOR_ARGB(50,255,255,255) );
...
bool CSystemMsgDlg::SetMessage( const char* szTitle, const char* szMsg, short iType, D3DCOLOR color, DWORD dwShowTime, HNODE hFont)
{
	...
	m_iImageIndex =  CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID,"ID_BLACK_PANEL");
	if( m_iImageIndex < 0 )
		return false;
