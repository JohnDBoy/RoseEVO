...
bool CDeliveryStoreDlg::Create( const char* IDD )
{
	...
	if( CWinCtrl* pCtrl = Find( IID_BTN_GIFT ) )
	{
		if( pCtrl->GetControlType() == CTRL_BUTTON )
		{
			...
			pBtn->SetAlign( DT_CENTER|DT_VCENTER );	
		}
	}
	return true;
}
...
void CDeliveryStoreDlg::Draw()
{
	...
	drawFont( g_GameDATA.m_hFONT[ FONT_NORMAL_BOLD ], true, 35,  7, g_dwWHITE, STR_DELIVERY_STORE );
...
void CDeliveryStoreDlg::SetInterfacePos_After()
{
	CWinCtrl * pCtrl = NULL;

	if( pCtrl = Find("TAB_NAME") )
	{
		((CTImage*)pCtrl)->SetText( LIST_STRING(795) );//"캐쉬아이탬"
