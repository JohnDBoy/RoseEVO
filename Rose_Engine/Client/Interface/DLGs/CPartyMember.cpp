...
CPartyMember::CPartyMember(DWORD dwObjectTag, WORD wObjSvrIdx ,const char* pszName )
{
	...
	m_iGuageIdx			 = CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI18_GUAGE_HP");
	m_iGuageBGIdx		 = CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI18_GUAGE_HP_BASE");
...
void	CPartyMember::Draw()
{
	...
	if( pObjAVT )
	{
		...
		g_DrawImpl.Draw( m_rcThis.right - nMaxWidth, m_rcThis.top + 25, UI_IMAGE_RES_ID, m_iGuageBGIdx );
		g_DrawImpl.Draw( m_rcThis.right - nMaxWidth, m_rcThis.top + 25, nWidth, UI_IMAGE_RES_ID,  m_iGuageIdx );
...
bool CPartyMember::AddStatusIcon( DWORD dwFlag, CEnduranceProperty* pEntity  )
{
	std::map< int , int >::iterator iter;
	CIconStatus StatusIcon;
	CInfo	ToolTip;
	...
	for( int i = 0; i < g_TblSTATE.m_nDataCnt; ++i )
	{
		if( STATE_TYPE( i ) == iter->second )
		{
			int iElapsedTime = g_GameDATA.GetGameTime() - pEntity->GetStartTimeDword();
			...
			StatusIcon.SetToolTip( ToolTip );
			StatusIcon.SetGraphicID( IMAGE_RES_STATE_ICON, STATE_SYMBOL( i ) );
			StatusIcon.SetVisibility( iVisibility );
			m_listStatusIcon.push_back( StatusIcon );
			return true;
		}
	}
...
bool CPartyMember::AddStatusIcon( DWORD dwFlag, DWORD dwStart, int iEndurance )
{
	std::map< int , int >::iterator iter;
	CIconStatus StatusIcon;
	CInfo	ToolTip;
	...
	for( int i = 0; i < g_TblSTATE.m_nDataCnt; ++i )
	{
		...
		{
			int ddd = iter->first;
			int ddggggg = iter->second;
			int iElapsedTime = g_GameDATA.GetGameTime() - dwStart;
			int iEnduranceTime = iEndurance * 1000;
			int iRemainedTime = abs( iEnduranceTime - iElapsedTime );
			int iVisibility = 255;
			if( iRemainedTime < 10000 || ( iElapsedTime > ( iEnduranceTime ) ) )
				iVisibility = ( ( iRemainedTime / 200 ) & 1 )? 255 : 0;
	...
	if( pObjAVT )
	{
		...
		for( int i = 1; i < sizeof( DWORD ) * 8; ++i )
		{
			if( dwStatusFlag & dwCompareFlag )
			{
				...
				std::map< int , int >::iterator iter;
				iter = CEndurancePack::m_StateFlagTable.find( dwCompareFlag );
				if( iter == CEndurancePack::m_StateFlagTable.end() )
					continue;

				CEnduranceProperty* pEntity = pObjAVT->m_EndurancePack.GetEntity(iter->second);

				//06. 11. 01 - 김주현 : 헐.. entity가 이상한게 들어온다;; 
				if( pEntity == NULL)
				{
					if( CGame::GetInstance().GetRight() >= 256 )
						g_itMGR.AppendChatMsg( "Test Message!! PartyBuff Error!!", IT_MGR::CHAT_TYPE_ALL);
					continue;
				}
				DWORD dwStartTime = pEntity->GetStartTimeDword();
				int iEnduranceTime = pEntity->GetEnduranceTime();

				AddStatusIcon( dwCompareFlag,  pEntity);
				//AddStatusIcon( dwCompareFlag, dwStartTime,  iEnduranceTime);
#else
