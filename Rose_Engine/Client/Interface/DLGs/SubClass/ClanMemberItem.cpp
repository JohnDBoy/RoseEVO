...
void CClanMemberItem::Draw()
{
	...
	if( m_strChannel.empty() )
		g_DrawImpl.Draw( m_sPosition.x, m_sPosition.y, UI_IMAGE_RES_ID, m_iOfflineMarkImageID );
	else
		g_DrawImpl.Draw( m_sPosition.x, m_sPosition.y, UI_IMAGE_RES_ID, m_iOnlineMarkImageID );
