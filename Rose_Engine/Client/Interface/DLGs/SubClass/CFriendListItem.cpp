...
CFriendListItem * CFriendListItem::Clone()
{
	CFriendListItem * pFLI = new CFriendListItem( m_dwUserTag, m_btStatus, m_strName.c_str() );

	pFLI->SetWidth(this->GetWidth());
	pFLI->SetHeight(this->GetHeight());
...
void CFriendListItem::Draw()
{
	...
	case FRIEND_STATUS_OFFLINE:// 접속끊김		: 친구목록의 m_dwUserTAG가 접속끊김
		{
			dwColor   = D3DCOLOR_ARGB( 255, 200, 200, 200);;
	...
	g_DrawImpl.Draw( (int)m_sPosition.x, (int)m_sPosition.y, UI_IMAGE_RES_ID, image_id );	

	RECT rc = { 20, 0, m_iWidth-10, m_iHeight };

	::drawFont( g_GameDATA.m_hFONT[ FONT_SMALL ], true, &rc, dwColor, DT_VCENTER, CStr::Printf("%-16s", m_strName.c_str() ) );
	::drawFont( g_GameDATA.m_hFONT[ FONT_SMALL ], true, &rc, g_dwYELLOW, DT_VCENTER|DT_RIGHT, CStr::Printf("(%s)", pszStatus ) ); //여기
}
