...
void CPrivateChatDlg::Show()
{
	...
	int iControlID = m_iControlID;
	POINT pt = { GetPosition().x, GetPosition().y };
	Clear();
	Create("DlgPrivateChat");		
	SetInterfacePos_After();
	MoveWindow(pt);
	SetControlID( iControlID );
...
void CPrivateChatDlg::Draw()
{
	...
	::drawFont( g_GameDATA.m_hFONT[ FONT_NORMAL_BOLD ], true, 30, 3, g_dwWHITE,
		CStr::Printf("%s", m_strName.c_str() ) );
}
...
//홍근 오전 11:32 2006-11-01 커뮤니티 기능 추가.
...
void CPrivateChatDlg::SendChatMsg()
{
	...
	if( pCtrl )
	{
		...
		if( pszMsg && strlen(pszMsg) )
		{
			g_pNet->Send_cli_MESSENGER_CHAT( m_dwUserTag, m_strName.c_str(),  pszMsg );
...
void CPrivateChatDlg::AddChatMsg( const char* pszMsg )
{
	...
	pListBox->AppendText( pszMsg, g_dwWHITE );
...
void CPrivateChatDlg::SetInterfacePos_After()
{
}