...
void CPrivateStoreDlg::SetInterfacePos_After()
{
	POINT ptOffset;
	CWinCtrl * pCtrl = NULL;
	...
	if( pCtrl = Find("ITEM_SLOT_POS") )
	{
		for( int iSlot = 0 ; iSlot < c_iMaxSlotCount; ++iSlot )
		{
			...
			ptOffset.x = pCtrl->GetOffset().x + ( iSlot % 5 ) * ( m_SellSlots[iSlot].GetWidth() + 1 );
			ptOffset.y = pCtrl->GetOffset().y + ( iSlot / 5 ) * ( m_SellSlots[iSlot].GetHeight() + 1 );

			m_SellSlots[iSlot].SetParent( GetDialogType() );
			m_SellSlots[iSlot].SetOffset( ptOffset );
			m_SellSlots[iSlot].SetDragAvailable();
			m_SellSlots[iSlot].SetDragItem( m_pSellDragItem );

			m_BuySlots[iSlot].SetParent( GetDialogType() );
	...
	if( pCtrl = Find(IID_BTN_SELL) )
	{
		//판매 889
		pCtrl->SetText( LIST_STRING(889) );
	}
	if( pCtrl = Find(IID_BTN_BUY) )
	{
		//구입 890
		pCtrl->SetText(LIST_STRING(890));
	}
	if( pCtrl = Find(IID_BTN_START) )
	{
		//개시 891
		pCtrl->SetText(LIST_STRING(891));
	}
	if( pCtrl = Find(IID_BTN_STOP) )
	{
		//중지 892
		pCtrl->SetText(LIST_STRING(892));
	}
