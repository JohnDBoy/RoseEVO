...
CNumberInputDlg::CNumberInputDlg(void)
{
	...
	m_iMaxNumber		= -1;
	m_pCmd				= NULL;
	SetNumberInputType(TYPE_USE_MAX_HIDE);
}
...
void CNumberInputDlg::SetNumberInputType(int iType)
{
	m_iNumberInput_Type = iType;
}
...
void CNumberInputDlg::OnLButtonUp( unsigned iProcID )
{
	...
	case IID_BTN_MAX:
		if( m_iMaxNumber >= 0 )
		{
			switch(m_iNumberInput_Type)
			{
			case TYPE_USE_MAX_HIDE:
...
void CNumberInputDlg::SetInterfacePos_After()
{
	CWinCtrl * pCtrl = NULL;
	if( pCtrl = Find("CALC_BG") )
	{
