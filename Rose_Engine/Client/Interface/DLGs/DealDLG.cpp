...
void CDealDLG::SetInterfacePos_After()
{
	POINT ptOffset;

	

	CWinCtrl * pCtrl00 = Find("ITEM_SLOT_00");
	CWinCtrl * pCtrl01 = Find("ITEM_SLOT_01");
	CWinCtrl * pCtrl10 = Find("ITEM_SLOT_10");
	CWinCtrl * pCtrl20 = Find("ITEM_SLOT_20");
	CWinCtrl * pCtrl30 = Find("ITEM_SLOT_30");

	if( !pCtrl00 || !pCtrl01 || !pCtrl10 || !pCtrl20 || !pCtrl30 )
	{
		return;
	}	
	...
	for( int iSlot = 0; iSlot < TOTAL_DEAL_INVENTORY; ++iSlot )
	{
		...
		ptOffset.x = pCtrl20->GetOffset().x  + ( iSlot % 5 ) * ( m_Slots[DEAL_SELL ][iSlot].GetWidth() + 1 );
		ptOffset.y = pCtrl20->GetOffset().y  + ( iSlot / 5 ) * ( m_Slots[DEAL_SELL ][iSlot].GetHeight() + 1 );

		m_Slots[DEAL_SELL ][iSlot].SetOffset( ptOffset );
		m_Slots[DEAL_SELL ][iSlot].SetParent( m_iType );
	...
	for( int iSlot = 0; iSlot < TOTAL_DEAL_INVENTORY; ++iSlot )
	{
		...
		ptOffset.x = pCtrl00->GetOffset().x + ( iSlot % 5 ) * ( m_Slots[DEAL_BUY][iSlot].GetWidth() + 1 ) ;
		ptOffset.y = pCtrl00->GetOffset().y + ( iSlot / 5 ) * ( m_Slots[DEAL_BUY][iSlot].GetHeight() + 1 );

		m_Slots[DEAL_BUY][iSlot].SetOffset( ptOffset );
		m_Slots[DEAL_BUY][iSlot].SetParent( m_iType );
	...
	//문자열 추가
	//거래 896
	GetCaption()->SetString( LIST_STRING(896) );
	
	if( pCtrl00 =Find(DEAL_BTN_OK) )
	{
		//확인 858
	...
	{
		//구입 890
		pCtrl00->SetText( LIST_STRING(890) );
	}
	if( pCtrl00 = Find("SELL") )
	{
		//판매 889
		pCtrl00->SetText( LIST_STRING(889) );
	}
...
void CDealDLG::Draw()
{
	...
	CGameUtil::ConvertMoney2String( Price, money_buffer, money_buffer_size );
	::drawFontf( g_GameDATA.m_hFONT[ FONT_NORMAL ], true, 39, 268, g_dwWHITE, "%s", money_buffer ); 
