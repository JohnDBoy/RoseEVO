...
void CDialogDlg::SetScript( char* pszScript )
{
	...
	else
	{
		...
		int iWidth		= 350;
		int iImageID    = CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI13_NPC_SCRIPT_IMAGE_TOP" );
		int iLineHeight = 16;
		...
		iImageID    = CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI13_NPC_SCRIPT_IMAGE_MIDDLE" );
		iLineHeight = 20;
		pItem = new CDialogNpcScriptItem( 1, pszScript , iImageID, iLineHeight, iWidth , 25 );
		pListBox->Add( pItem ); iListBoxHeight += pItem->GetHeight();

		iImageID    = CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI13_NPC_SCRIPT_IMAGE_BOTTOM" );
		iLineHeight = 16;
		...
		iImageID = CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI13_NPC_SCRIPT_ANSWER_TOP" );
		iLineHeight  = 10;
		iWidth   = 350;
		...
		iImageID = CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI13_NPC_SCRIPT_ANSWER_BOTTOM" );
		iLineHeight  = 17;
...
void CDialogDlg::AddAnswerExample( char* pszScript , int iEventID , void (*fpEventHandle)(int iEventID))
{
	...
	else
	{
		...
		int iImageID = CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI13_NPC_SCRIPT_ANSWER_MIDDLE" );
		int iHeight  = 25;
		int iWidth	 = 350;
...
#ifdef __KEYBOARD_DIALOG // 키보드로 대화 진행
	if( uiMsg == WM_KEYDOWN )
	{
		switch( wParam ) 
		{
		...
		default:
			{
				...
				if( pCtrl )
				{
					...
					int iSelectItem = wParam-'1';

					if( iSelectItem >= 0 && pListBox->GetSize() > iSelectItem )
					{
						pListBox->SetSelected( iSelectItem );
						CDialogNpcScriptAnswerItemNew* pAnswerItem = (CDialogNpcScriptAnswerItemNew*)pListBox->GetItem(wParam-'1');
						if(pAnswerItem)
						{
