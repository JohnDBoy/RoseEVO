...
bool CTCmdDropItem::Exec( CTObject* pObj )
{
	...
	// 06. 05. 19 - 김주현 : 카트 탑승 상태일때는 아이템 버리기도 안된다.
...
#else
	///일본이 아닌경우 플래티넘 탭은 플래티넘 사용자만 물건을 넣을수 있다.
	if( bPlatinum )
	{
...
bool CTCmdBuyItem::Exec(CTObject* pObj )
{
	...
	if (CStore::GetInstance().IsClanStore() )
	{
		...
		if( lPrice > CClan::GetInstance().GetRewardPoint() )
		{
...
bool CTCmdSaveMoneyFromBank::Exec( CTObject* pObj )
{
	g_pNet->Send_cli_MOVE_ZULY_INV2BANK( m_iNumber );	
	return true;
}

bool CTCmdWithdrawMoneyFromBank::Exec( CTObject* pObj )
{
