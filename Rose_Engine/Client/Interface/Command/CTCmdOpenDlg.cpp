...
bool CTCmdCreateMsgBox::Exec( CTObject* pObj )
{
	...
	pMsgBox->Create( (char*)m_Data.strXml.c_str() );
	...
	pMsgBox->SetswTimer(m_Data.bUseTimer); // 타이머 사용 여부 설정.
	pMsgBox->SetMsgTimer(m_Data.dwTimer);  // 타이머에 적용할 시간.	
	pMsgBox->SetCheckCommand(m_Data.iCheckUseOKButton); //커맨드를 어떤걸 적용할지 여부.
	...
	//확인 취소.
	pMsgBox->SetButtonText( LIST_STRING(858) , LIST_STRING(859) );
...
bool CTCmdOpenPrivateChatDlg::Exec( CTObject* pObj )
{
	...
	pPrivateChatDlg->SetInterfacePos_After();
