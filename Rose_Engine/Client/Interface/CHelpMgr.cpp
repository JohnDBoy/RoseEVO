...
void CHelpMgr::SetNextHelpMsg()
{
	...
	if( pDlg )
	{
		...
		int iNextShowHelpID = 0;
		//06. 07 . 31 - 김주현 : 게임 플레이한지 1시간이 경과하면 경고 문구 출력.
		// 1시간이 경과했습니다.		
		if(g_GameDATA.GetElapsedGameTime() > ONEHOUR && m_bFirstHour == false)
		{
			iNextShowHelpID = 310;
			m_bFirstHour = true;
		}
		//3시간이 경과했습니다. 잠시 쉬었다 하시기 바랍니다.
		else if(g_GameDATA.GetElapsedGameTime() > THREEHOUR && m_bSecondHour == false)
		{
			iNextShowHelpID = 311;
			m_bSecondHour = true;
		}
		...
		{
			iNextShowHelpID = GetNextIndex( m_iPrevShowHelpID );
		}
