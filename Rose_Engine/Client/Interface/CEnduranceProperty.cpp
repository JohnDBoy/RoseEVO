...
const int ENDU_ICON_X		= 2;
const int ENDU_ICON_Y		= 66;
...
void CEndurancePack::MakeStateSTBIDXTable()
{
	...
	//06. 06. 20 - 김주현 : 변신일 경우엔 36, 발키리서포트 54
	m_StateSTBIDXTable[ ING_DUMMY_DAMAGE ]			= 54;	// 유지 : 추가 데미지
...
{	
	...
	if( iStatusSTBNO != 0 )
	{
		...
		{		
...
#ifdef __CAL_BUF2 // 홍근 : 중복 적용된 상태 제거 할때 세팅값도 초기화 : 오전 11:36 2006-11-10
				m_CurrentStateValue[ STATE_TYPE( iStatusSTBNO ) ] = 0; 
#endif
			...
			if(STATE_TYPE(iStatusSTBNO) == ING_SLEEP)
			{
				m_bSleep = true;			
			}
		}
	...
	switch( iType )
	{
		...
		case ING_DEC_MOV_SPD:
			{				
				...
				/// 버프가 들어간 상태에서 다시 같은 버프를 쓰면
				/// 같은 버프이기 때문에 이전 버프 삭제로 Get_DefaultSPEED()값이 버프 세팅되지 않은 값으로 바뀌어 오차가 난다.
				/// 제거 하기로 한다.
				//float fAdjRate = m_pObjCHAR->Get_MoveSPEED() / m_pObjCHAR->Get_DefaultSPEED ();
#ifdef _DEBUG
				float fMS = m_pObjCHAR->Get_MoveSPEED();
				float fDS = m_pObjCHAR->Get_DefaultSPEED();
#endif
				m_CurrentStateValue[ iType ] = iValue; 				
				//m_pObjCHAR->Set_AdjustSPEED(m_pObjCHAR->Get_DefaultSPEED () * fAdjRate);	
				m_pObjCHAR->Set_AdjustSPEED(m_pObjCHAR->Get_DefaultSPEED () );	
#else
				/// 이동 속도에 관한것이라면 한번 갱신해준다.
				float fAdjRate = m_pObjCHAR->Get_MoveSPEED() / m_pObjCHAR->Get_DefaultSPEED ();
				m_CurrentStateValue[ iType ] = iValue; 				
		...
		case ING_DEC_ATK_SPD:									
			...
			::setAnimatableSpeed( m_pObjCHAR->GetZMODEL(), m_pObjCHAR->Get_fAttackSPEED()  );
			break;		
...
void CEndurancePack::Draw()
{
	...
	if( m_pObjCHAR->IsA(OBJ_USER) )
	{
		...
		if( iMax > 0 && iValue > 0 )///Summon Guage
		{
			...
			int iBackImageNo   =  CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI00_GUAGE_BACKGROUND" );
			int iVioletImageNo =  CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI00_GUAGE_VIOLET" );

			pDrawObj->Draw( ptDraw.x, ptDraw.y, UI_IMAGE_RES_ID, iBackImageNo );
			pDrawObj->Draw( ptDraw.x, ptDraw.y, iWidth, UI_IMAGE_RES_ID, iVioletImageNo );
		...
		{
			int iBackImageNo  = CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI00_GUAGE_BACKGROUND" );
			int iGreenImageNo = CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI00_GUAGE_GREEN" );
			int iRedImageNo   = CResourceMgr::GetInstance()->GetImageNID( UI_IMAGE_RES_ID, "UI00_GUAGE_RED" );
			...
			if( pEngineItem )
			{
				...
				iWidth = 100 * iValue / 10000;

				pDrawObj->Draw( ptDraw.x, ptDraw.y,			UI_IMAGE_RES_ID, iBackImageNo );
				pDrawObj->Draw( ptDraw.x, ptDraw.y, iWidth, UI_IMAGE_RES_ID, iGreenImageNo );
				RECT rcDraw = { 0, 0, 100, 20 };

				drawFontf( g_GameDATA.m_hFONT[ FONT_NORMAL_OUTLINE ], true, &rcDraw, g_dwWHITE, DT_CENTER, "%d", iValue );
			...
			if( iMax > 0 )
			{
				...
				pDrawObj->Draw( ptDraw.x, ptDraw.y, UI_IMAGE_RES_ID, iBackImageNo );
				pDrawObj->Draw( ptDraw.x, ptDraw.y, iWidth, UI_IMAGE_RES_ID, iRedImageNo );
				RECT rcDraw = { 0, 0, 100, 20 };
				...
				case ING_DEC_MOV_SPD:
					{
						...
						// 이동 속도 모션 스피드 설정...
						// 06. 10. 12 - 김주현 : State가 중복이 될수 있을까..-_-? 제자리에 서있는 상태에서 이속 버프를 스킬로 해제 시키면
						// 아바타가 한걸음 이동하는것처럼 보인다;; Get_STATE & CS_MOVE를 아래와 같이 수정했다.
...
void CEndurancePack::ClearRWeaponSkillEffect()
{
	...
	DeleteEntityByStateType( ING_DUMMY_DAMAGE ); // 06. 09. 28 - 김주현 :  발키리 서포트 제거 추가.
#endif
}
...
bool CEndurancePack::GetFlagEntitySleep()
{
	return m_bSleep;
}
void CEndurancePack::SetFlagEntitySleep(bool bSleep)
{
	m_bSleep = bSleep;
}
