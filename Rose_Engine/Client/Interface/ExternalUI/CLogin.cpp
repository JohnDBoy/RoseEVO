...
{ 
	...
	m_listiter = m_warning.begin();
}

void CLogin::AddWaning()
{
	m_warning.clear();
	m_warning.push_back("\"공부와 일에 휴식이 필요하듯이, 게임 이용에도 적절한 휴식이 필요합니다.\"");
	m_warning.push_back("\"장시간의 게임 이용은 건강을 해칠 수 있습니다.\"");
}

std::string CLogin::GetWarning()
{
...
void CLogin::SendLoginReq( )
{
	...
	{
		CGame::GetInstance().SetActiveXConnect( true );
		g_pNet->Send_cli_LOGIN_REQ2( g_GameDATA.m_dwSeq, g_GameDATA.m_szSessionKey );
		return;
	}
#endif
...
void CLogin::Show()
{
	...
	*/
	...
	///사용자 등급
#if defined( __KOREA ) || defined ( __NEWKOREA )
...
//	::setTransformSprite( mat );
	...
	drawFont( g_GameDATA.m_hFONT[ FONT_NORMAL_OUTLINE ], true, 0,0, g_dwWHITE, CStr::Printf("RoseOnline [Ver. %s]",CGame::GetInstance().GetCurVersion().c_str() ));
//	#endif

	#ifdef __KOREA_NEW
	//한국은 로그인 화면에서 장시간 플레이에 대한 경고문을 출력한다.
	D3DXMatrixTranslation( &mat, 55, g_pCApp->GetHEIGHT() - m_iHeightCopyRight - 20, 0.0f);
	::setTransformSprite( mat );
	drawFont( g_GameDATA.m_hFONT[ FONT_NORMAL_OUTLINE ], true, 0,0, g_dwWHITE, GetWarning().c_str());
	#endif
