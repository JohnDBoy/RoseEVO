...
void CIconItem::Draw()
{
	...
	if( Item.IsEnableDupCNT() && GetQuantity())/// Quantity Item일경우
	{
		...
		g_DrawImpl.Draw( (int)m_ptPosition.x , (int)m_ptPosition.y + 2, iWidth , UI_IMAGE_RES_ID, iImageIndex ,D3DCOLOR_ARGB(128,255,255,255));
...
void CIconItem::GetToolTip( CInfo& strToolTip,  DWORD dwDialogType, DWORD dwInfoType )
{
	assert( m_pItem );
