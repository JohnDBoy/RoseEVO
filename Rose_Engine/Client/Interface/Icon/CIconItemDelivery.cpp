...
{
	m_border_grahicid = CResourceMgr::GetInstance()->GetImageNID(UI_IMAGE_RES_ID ,"UI13_BORDER_SELECTED_ITEM" );
	if( pszFrom  )
		m_from = pszFrom;
	else
		m_from.clear();
...
void CIconItemDelivery::Draw()
{
	CIconItem::Draw();
	if( s_selected_icon == GetCItem()->GetIndex())
		g_DrawImpl.Draw( (int)m_ptPosition.x, (int)m_ptPosition.y, UI_IMAGE_RES_ID  , m_border_grahicid );
}
