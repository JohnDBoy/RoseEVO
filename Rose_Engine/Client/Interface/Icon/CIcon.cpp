...
CIcon::CIcon(void)
{
	...
	m_dwIconDrawColor_Ori	= D3DCOLOR_ARGB( 255, 255, 255, 255 );
	m_dwIconDrawColor_Gray	= D3DCOLOR_ARGB( 255, 128, 128, 128 );
	m_bShow				= true;
}
...
void CIcon::SetDrawColor_Ori( DWORD color )
{
	m_dwIconDrawColor_Ori = color;
}

DWORD CIcon::GetDrawColor()
{
	return m_dwIconDrawColor;
}
...
{
	if( IsEnable() )
		m_dwIconDrawColor = m_dwIconDrawColor_Ori;
	else
		m_dwIconDrawColor = m_dwIconDrawColor_Gray;
}
...
void CIconStatus::SetVisibility( int iVisibility )
{
...
void CIconStatus::Draw()
{
	...
	g_DrawImpl.Draw( (int)m_ptPosition.x, (int)m_ptPosition.y, m_iModuleID, m_iIconGraphicID, D3DCOLOR_RGBA( 255,255,255,m_iVisibility ));
#else
