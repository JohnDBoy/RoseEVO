...
CIconSkill::CIconSkill( CSkill * pSkill )
{
	m_iSkillSlot = -1;
	m_pSkill = pSkill;	
}
...
{	
	...
	if( m_pSkill  )
	{
		m_pSkill->DrawIcon( m_ptPosition.x, m_ptPosition.y, GetDrawColor() );
		return;
	}
	/// get skill.
...
void CIconSkill::ExecuteCommand()
{
	...
	if( pSkill  )
	{
		if( pSkill->IsEnable() )
		{
			static unsigned long lastSkillTime = 0;
			unsigned long curTime = ::timeGetTime();
			if (lastSkillTime == 0 || (curTime - lastSkillTime >= 200))
			{
				pSkill->Execute();
				lastSkillTime = curTime;
			}
		}
		else
		{
...
{	
	if( m_pSkill )
	{
		return SKILL_NAME( m_pSkill->GetSkillIndex() );
	}
		...
		case SKILL_ACTION_ENFORCE_BULLET:///5,		///< 강화총알 변경 발사. ( 아이스 애로우.. 실제 화살이 변하는.. )
			{
				...
				AddSkillDistanceScope( iSkillNo, ToolTip, false ); // 3,4,5번 타입에도 사정거리 추가.
...
void CIconSkill::AddSkillName( int iSkillNo ,CInfo& ToolTip, bool bAddLevel )
{
	...
	if( bAddLevel )
	{
		char* pszBuf = CStr::Printf( "%s[%s:%d] (%d)", SKILL_NAME(iSkillNo ),STR_LEVEL, SKILL_LEVEL(iSkillNo ), iSkillNo);
		ToolTip.AddString( pszBuf ,g_dwYELLOW, g_GameDATA.m_hFONT[ FONT_NORMAL_BOLD] );
	}
	else
	{
		char* pszBuf = CStr::Printf( "%s (%d)", SKILL_NAME(iSkillNo ), iSkillNo);
		ToolTip.AddString( pszBuf, g_dwYELLOW, g_GameDATA.m_hFONT[ FONT_NORMAL_BOLD]);
	}
#else
	if( bAddLevel )
...
bool CIconSkill::GetSkillIncreaseAbility_Value( int iSkillNo, int iColumn, std::string& strOut ,bool bAddTypeName )
{
	if( iColumn >= SKILL_INCREASE_ABILITY_CNT )
		return false;
...
bool CIconSkill::GetSkillIncreaseAbility_Rate( int iSkillNo, int iColumn, std::string& strOut ,bool bAddTypeName )
{
	if( iColumn >= SKILL_INCREASE_ABILITY_CNT )
		return false;
...
void CIconSkill::AddSkillStatus( int iSkillNo, CInfo& ToolTip  )
{
	...
	for(int i=0; i<SKILL_INCREASE_ABILITY_CNT; i++)
	{
		iStateNo = SKILL_STATE_STB(iSkillNo ,i);
		...
		if( iStateNo )
		{
			...
			{
				if( GetSkillIncreaseAbility_Value( iSkillNo, i, strOut , false ) )
				{
					strTemp = pszHeader;
				...
				if( GetSkillIncreaseAbility_Rate( iSkillNo, i, strOut , false ) )
				{
					strTemp = pszHeader;
					strTemp.append( STATE_NAME( iStateNo ) );
