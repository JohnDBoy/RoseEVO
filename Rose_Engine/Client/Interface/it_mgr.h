...
class IT_MGR
{
	...
	friend class CITStateMakeSocket;
...
public:
	...
	/*-----------------------------------------------------------------------------*/
	/// 메세지 박스 여는 methods
	// 06. 08. 22 - 김주현 :   1. bUseTimer :: 타이머 사용여부, 2. dwTimer :: 타이머 작동 시간, 
	// 3. bCheckUseOKButton :: 타이머의 시간이 0이 된후 true면 OK command를 날린다, false면 Cancel command를 날린다.
	void OpenMsgBox( const char* szMsg, 
		...
		int iMsgType = 0, const char * szXmlName = "MsgBox", bool bUseTimer = false, DWORD dwTimer = 30000, int iCheckUseOKButton = 0
		);
	
	void	OpenMsgBox_CartRide( const char* szMsg, 
		short int iButtonType = CMsgBox::BT_OK , bool	bModal = true,
		unsigned int iInvokerDlgID =  0, CTCommand* pCmdOk = NULL, CTCommand* pCmdCancel = NULL ,
		int iMsgType = 0, WORD parm1 = 0, WORD parm2 = 0, bool bUseTimer = false, DWORD dwTimer = 30000
		);
	...
	CTDialog*	OpenDialog( int iDlgType, bool bToggle = true , int iPosX = -1, int iPosY = -1 );	/// 다이얼로그를 연다
	void		CloseDialog( int iDlgType );														/// 다이얼로그를 닫는다.

	CTDialog*	FindDlg( DWORD dwDlgType );												/// 해당 ID의 다이얼로그를 찾아서 포인터를 리턴
	void		DeleteDlg( CTDialog* pDlg );											/// 해당 ID의 다이얼로그를 찾아서 제거.
	...
	void		InitInterfacePos(CTDialog * pDlg);														/// 인터페이스 초기화, 해상도가 바뀌거나 인터페이스를 최초 위치로 옮기고 싶을때 사용하는 Method
	...
	void		BeginMakeSocket();														/// 소켓 생성 시작
	void		EndRepair();															/// 수리상태 끝
	void		EndAppraisal();															/// 아이템감정 끝
	void		EndMakeSocket();														/// 소켓 생성 상태 끝
