...
#include "itstate/CITStateMakeSocket.h"
...
IT_MGR::IT_MGR()
{
	...
	m_pStates[ STATE_MAKESOCKET ]		= new CITStateMakeSocket;
...
//	AppendDlg( DLG_TYPE_PRIVATECHAT, pPrivateChatDlg, pPrivateChatDlg->GetControlID () );
	...
	// 월드맵
	CWorldMapDLG * pWorldMapDLG = new CWorldMapDLG;
	pWorldMapDLG->Create("DlgWorldMap");
	AppendDlg( DLG_TYPE_WORLDMAP, pWorldMapDLG, pWorldMapDLG->GetControlID() );
...
/*#ifdef __TEST// 타겟 오브젝트 다이알로그
	...
	pQuickBAR->Create("DlgQuickBarExt");
...
void IT_MGR::DeleteDlg( CTDialog* pDlg )
{
	list_dlgs_itor  itorDlgs;
	for( itorDlgs = m_Dlgs.begin(); itorDlgs != m_Dlgs.end(); ++itorDlgs )
	{
...
void IT_MGR::AppendChatMsg( const char* pszMsg, int iType ,DWORD forceapply_color)
{
	...
	case CHAT_TYPE_SHOUT:
		...
		iFilterType = CChatDLG::FILTER_SHOUT;
...
#if !defined(__SRVDISCONNECTRELOGIN) && !defined(_DEBUG)
//#ifndef 
		delete pCmd;
#endif
...
void IT_MGR::OpenMsgBox(	const char* szMsg, 
						short int iButtonType , bool	bModal,
						unsigned int iInvokerDlgID, CTCommand* pCmdOk, CTCommand* pCmdCancel,
						int iMsgType, const char * szXmlName, bool bUseTimer , DWORD dwTimer, int iCheckUseOKButton)
{
	...
	Data.pCmdOk				= pCmdOk;
	Data.strMsg				= szMsg;
	Data.strXml				= szXmlName;
	Data.bUseTimer			= bUseTimer;
	Data.dwTimer			= dwTimer;
	Data.iCheckUseOKButton	= iCheckUseOKButton;
...
void IT_MGR::OpenMsgBox_CartRide( const char* szMsg, short int iButtonType, bool bModal, unsigned int iInvokerDlgID , CTCommand* pCmdOk, CTCommand* pCmdCancel, int iMsgType,
								 WORD parm1, WORD parm2, bool bUseTimer, DWORD dwTimer )
{
	CreateMsgBoxData Data;
	Data.bModal			= bModal;
	Data.iButtonType	= iButtonType;
...
void IT_MGR::InitInterfacePos(CTDialog * pDlg)
{
	POINT ptNew = { 0 , 0 };
...
CTDialog * IT_MGR::OpenDialog( int iDlgType, bool bToggle, int iPosX , int iPosY )
{
	CTDialog* pDlg = NULL;
...
CPrivateChatDlg* IT_MGR::GetPrivateChatDlg( DWORD dwUserTag )
{
	...
	{
		pDlg = *iter;
		if( pDlg->GetDialogType() == DLG_TYPE_PRIVATECHAT )
		{	
			//홍근: ControlID로 비교 하지 말고 클래스내의 UserTag값으로 비교하게 하자
			if(  ((CPrivateChatDlg*)pDlg)->GetUserTag() == dwUserTag )			
			{
...
void IT_MGR::BeginMakeSocket()
{
	CTCommand* pCmd = new CTCmdChangeStateInterface( STATE_MAKESOCKET );
	AddTCommand( DLG_TYPE_MAX, pCmd );
}
...
void IT_MGR::AddNotifybutton( int iEventIndex )
{
	...
	int iNormalGraphicID = CResourceMgr::GetInstance()->GetImageNID( IMAGE_RES_NEWUI, "ALARM_N" );
	int iOverGraphicID   = CResourceMgr::GetInstance()->GetImageNID( IMAGE_RES_NEWUI, "ALARM_O" );
	int iDownGraphicID   = CResourceMgr::GetInstance()->GetImageNID( IMAGE_RES_NEWUI, "ALARM_W" );
	int iBlinkGraphicID  = CResourceMgr::GetInstance()->GetImageNID( IMAGE_RES_NEWUI, "ALARM_D" );
	int iModuleID		 = IMAGE_RES_NEWUI;
