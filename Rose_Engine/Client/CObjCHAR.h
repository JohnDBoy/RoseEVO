...
struct tagDAMAGE {
	...
	uniDAMAGE	m_wDamage;			//홍근 : 맥스 데미지 수정
} ;
...
/*
	// 06. 04. 18 - 김주현 : 카트 탑승 / 교환 / 친구 추가 / 파티 초대 관련 메세지 오버플로우 에러 수정위한 멤버변수.
	   FindMsgBox / IsOpenedMsgBox 로 체크를 해보았으니;; 최초에 게임창이 활성화 되지 않은 상태에서 메세지 러쉬를 당하게 되면
	   체크를 못하게 됩니다. 그래서 따로 변수를 만들어서 체크합니다;
*/
	bool						m_bCartInvitationWnd;	   //카트 관련 메세지가 활성화 되어있는가?

	int							GetMinStateValue( int iIng_Type, int iValue );	//홍근 : 어빌값의 최소값 적용.
...
protected :
   ...
   CObjCHAR_Collision2NPC      *m_pCollision2NPC; //NPC 충돌
...
public:
	...
	long						m_lDeadTIME;
	void						Apply_DAMAGE (CObjCHAR *pAtkOBJ, uniDAMAGE sDamage);	//홍근 : 맥스 데미지 수정
