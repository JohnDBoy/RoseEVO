...
float CPatchManager::Pick_POSITION( D3DXVECTOR3 &PosPICK )
{
	...
	while( pCurNode ) 
	{
		if ( pCurNode->DATA->IsIntersect(fCurDis, PosPICK) ) {
			fMinDis = fCurDis;
//			::InputSceneOBBNode(pCurNode->DATA->GetVISIBLE());
		}
...
//	::InputSceneSphere(buffer_pos[0], buffer_pos[1], buffer_pos[2], 100.0f);
