...
bool CGame::Load_BasicDATA()
{
...
#ifdef __VIRTUAL_SERVER
	//06. 08. 09 - 김주현 : 아이템드랍 클라에서 사용하지 않는데 로딩한다! 
	//__VIRTUAL_SERVER - 가상서버발동, 예전에 서버가 구축되기전에 클라혼자서 지지고 볶기 위해서 만든 전처리기.
	...
	g_TblBREAK.Load		( "3DDATA\\STB\\LIST_Break.STB",	true, false  );///05/11/03 : 김주현  분해 관련 STB 로딩 추가.
#endif
...
//	g_pAVATAR->SetAllPARTS( m_SelectedAvataInfo.m_nPartItemIDX );
	...
	for( int i = 0 ; i < MAX_MAINTAIN_STATUS; ++i )
	{
		...
		{
#ifdef __EXPERIENCE_ITEM // 더미 값에 어빌타입을 넣어 구별한다.
			if(i > 3)
			{
	...
	for( int i = 0; i < MAX_LEARNED_SKILL_CNT; i++ )
	{
		...
		if( iSkill != 0 )
		{
			...
			{
				//m_SelectedAvataInfo.m_dwCoolTime[iDelayType] : iDelayType가 스킬 사용한 시간.
...
void CGame::ScreenCAPTURE (SYSTEMTIME SystemTime)
{
	if ( ::beginScene() )
	{
		...
		::drawFontf( g_GameDATA.m_hFONT[ FONT_NORMAL_OUTLINE ], true, 820,720, D3DCOLOR_XRGB( 255, 255, 255),		
			...
			g_pAVATAR ? g_pAVATAR->Get_NAME() : "UNKNOWN",							// 케릭 이름
...
//07. 06. 14 - 김주현 : ActiveX를 사용할시 케릭터 선택창에서 서버 선택창으로 이동시 디스컨넥트 문제 수정을 위해서
//ActiveX 사용 여부를 판단하기 위해 추가.
