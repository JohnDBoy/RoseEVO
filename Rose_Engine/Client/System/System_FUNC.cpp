					   ...
					   int iFaceItem, int iKnapsack, int iRWeapon, int iLWeapon )
{
	...
	//조성현 2005 12 - 17 test
	CGameDataCreateAvatar::GetInstance().m_CreateAvatar = pAvt->GetModelNode(); 
...
SYSTEM_SCRIPT
int SC_QF_getClanVAR ( int iVarNO )
{
	return QF_getClanVAR ( iVarNO );
}
...
{
	g_pTimeLuaFuncs->AddTimeLuaFunction( funcName, time, 0 );
}

SYSTEM_SCRIPT
void SC_TimeLuaObject ( ZSTRING funcName, int time )
{
	g_pTimeLuaFuncs->AddTimeLuaFunction( funcName, time, 1 );
}
