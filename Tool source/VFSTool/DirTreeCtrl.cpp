...
CDirTreeCtrl * g_pCtrl_DirTree = NULL;

CDirTreeCtrl::CDirTreeCtrl()
{
	m_strRoot = "";      // Auf Leer setzen
	g_pCtrl_DirTree = this;
}

CDirTreeCtrl::~CDirTreeCtrl()
{
	m_imgList.Detach();
	g_pCtrl_DirTree = NULL;
}
...
BOOL CDirTreeCtrl::DisplayDrives()
{
	...
	while( *pDrive )
	{
		...
		if ( FindSubDir ( pDrive ) ) { InsertItem ( "", 0, 0, hParent ); }
...
void CDirTreeCtrl::OnTvnSelchanged (NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMTREEVIEW pNMTreeView = reinterpret_cast<LPNMTREEVIEW>(pNMHDR);
	
	CString Path = GetFullPath (pNMTreeView->itemNew.hItem);
	
	::g_pFV_Files->DisplayFiles (Path);
...
/**
 * 현재 선택된 항목의 디렉토리 경로를 알아낸다
 */
CString CDirTreeCtrl::GetSelectedPath(void)
{
	CString strPath;

	if(IsSelected ())
	{ strPath = GetFullPath (GetSelectedItem ()); }
 ...
 * 선택된 항목이 있는가
 */
bool CDirTreeCtrl::IsSelected (void)
{
	if(GetSelectedItem ()) { return true; }
